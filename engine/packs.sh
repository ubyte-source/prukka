...
# The Tatoeba model zips ship no license file, so every converted MT pack
# records the CC-BY attribution itself — a pack must stay self-describing
# offline, exactly like the voice packs' bundled MODEL_CARD. The same short
# form goes into each MT meta so the catalog carries it too.
MT_LICENSE="CC-BY-4.0 (Helsinki-NLP Opus-MT / Tatoeba Challenge)"
write_mt_license() {
  local dest="$1" url="$2"
  cat > "$dest/LICENSE" <<EOF
This directory redistributes an Opus-MT translation model converted from:
  $url
License: CC-BY 4.0 (https://creativecommons.org/licenses/by/4.0/)
Attribution: Helsinki-NLP and the Tatoeba Challenge (Tiedemann & Thottingal,
...
write_mt_license "$OUT/mt-it-en/models/mt-it-en" "$MT_MODEL_URL"
write_mt_license "$OUT/mt-en-it/models/mt-en-it" "$MT_EN_IT_MODEL_URL"
write_meta mt-it-en "{\"id\":\"mt-it-en\",\"kind\":\"mt\",\"from\":\"it\",\"to\":\"en\",\"license\":\"$MT_LICENSE\"}"
write_meta mt-en-it "{\"id\":\"mt-en-it\",\"kind\":\"mt\",\"from\":\"en\",\"to\":\"it\",\"license\":\"$MT_LICENSE\"}"
...
while read -r iso1 _iso3 voice vdir enx_url enx_sha xen_url xen_sha onnx_sha json_sha; do
  ...
  write_mt_license "$OUT/mt-en-$iso1/models/mt-en-$iso1" "$enx_url"
  write_mt_license "$OUT/mt-$iso1-en/models/mt-$iso1-en" "$xen_url"
  write_meta "mt-en-$iso1" "{\"id\":\"mt-en-$iso1\",\"kind\":\"mt\",\"from\":\"en\",\"to\":\"$iso1\",\"license\":\"$MT_LICENSE\"}"
  write_meta "mt-$iso1-en" "{\"id\":\"mt-$iso1-en\",\"kind\":\"mt\",\"from\":\"$iso1\",\"to\":\"en\",\"license\":\"$MT_LICENSE\"}"
