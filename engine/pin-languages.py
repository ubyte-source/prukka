...
rhasspy/piper-voices index, medium quality first (see pick_voice for why
medium, not high, is preferred).
"""
...
def pick_voice(voices: dict, iso1: str):
    """Return (name, dir) of the preferred Piper voice for a language."""
    # A preference order, not a quality ranking: the shipped en/it voices
    # (pins.sh) are both "medium", so pinning every pivot language to medium
    # keeps all lanes on one voice size/latency class; the rest is only a
    # fallback order for languages without a medium voice.
