...
convert_marian() {
  ...
  local src="$WORK/marian-$pair"
  download "$url" "$src.zip" "$want"
  mkdir -p "$src" && (cd "$src" && unzip -q "$src.zip")
  # Only the parent: ct2-opus-mt-converter refuses an existing --output_dir.
  mkdir -p "$(dirname "$dest")"
  "$WORK/venv/bin/ct2-opus-mt-converter" \
    --model_dir "$src" --output_dir "$dest" --quantization int8
  cp "$src/source.spm" "$src/target.spm" "$dest/"
