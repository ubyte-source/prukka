...
// Beam 2 buys most of beam search's quality over greedy decoding at a
// fraction of the latency; the streaming dubbing lane is latency-bound, so
// wider beams are not worth their cost.
constexpr std::size_t kBeamSize = 2;
...
int main(int argc, char** argv) {
  ...
  while (std::getline(std::cin, line)) {
    ...
    // Marian/Opus-MT expects an explicit end-of-source token. Every pinned
    // model is a single-pair model, so no >>lang<< target tag is prepended.
