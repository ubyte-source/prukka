...
# CI builder's; checks.sh is the one deployment floor shared with the native
# driver builds, and clang and cmake both honor the export it makes.
[ "$OS" = macos ] && . "$HERE/../drivers/macos/common/checks.sh"
...
relocate() {
  ...
  # $ORIGIN is a literal ELF token the loader expands, never a shell variable,
  # so it has to stay single-quoted.
...
# Third-party trees (whisper.cpp, CTranslate2, SentencePiece) compile with
# warnings silenced: their diagnostics are not actionable here, and thousands
# of lines of vendor -Wunused chatter bury the build's real signal. CMake
# appends these env flags to its per-toolchain defaults (cl accepts -w as
# /w), and CMP0092=NEW below keeps MSVC from injecting /W3 first — which
# would add a D9025 override warning per translation unit. Prukka's own code
# (the mt wrapper, the Go tree, the drivers) keeps warnings fully fatal; the
# direct clang++/cl invocations below never read these variables.
export CFLAGS="${CFLAGS:-} -w" CXXFLAGS="${CXXFLAGS:-} -w"
# VS 2026's cl still reports template-instantiation-context warnings from
# the STL past /w (observed: C4244/C4267/C4566 out of xutility/xmemory on
# CTranslate2 and SentencePiece types); /wd suppression is absolute, so pin
# the observed IDs off explicitly.
if [ "$OS" = windows ]; then
  export CFLAGS="$CFLAGS /wd4244 /wd4267 /wd4566" CXXFLAGS="$CXXFLAGS /wd4244 /wd4267 /wd4566"
fi
...
WHISPER_FLAGS=(-DGGML_NATIVE=OFF -DWHISPER_BUILD_SERVER=ON -DBUILD_SHARED_LIBS=ON -DGGML_CCACHE=OFF)
...
cmake -Wno-dev -Wno-deprecated --no-warn-unused-cli ${CMAKE_GEN[@]+"${CMAKE_GEN[@]}"} -S "$WORK/whisper.cpp" -B "$WORK/whisper.cpp/build" \
...
download_whisper_model() {
  ...
  download "$WHISPER_MODELS_URL/$model" "$WORK/$model" "$want"
...
# CTranslate2 appends /W4 to CMAKE_CXX_FLAGS on MSVC, after the -w set
# above — re-enabling its warning storm and emitting a D9025 override
# warning for every translation unit. Neutralize the append at the source;
# the tree is pinned to one commit, so the substitution is stable.
[ "$OS" = windows ] && sed -i 's|/W4 /d2FH4-|/w /d2FH4-|' "$WORK/ct2/CMakeLists.txt"
...
cmake -Wno-dev -Wno-deprecated --no-warn-unused-cli ${CMAKE_GEN[@]+"${CMAKE_GEN[@]}"} -S "$WORK/ct2" -B "$WORK/ct2/build" \
...
# CTranslate2 loads OpenBLAS at runtime on Windows; without its DLL beside
# ctranslate2.dll the bundle installs cleanly and then fails to translate on the
# customer's machine, so a missing DLL fails the build here.
if [ "$OS" = windows ]; then
  OPENBLAS_DLL="$(find "$VCPKG_WIN/bin" -iname '*openblas*.dll' 2>/dev/null | head -1)"
  [ -n "$OPENBLAS_DLL" ] || { echo "OpenBLAS DLL not found under $VCPKG_WIN/bin" >&2; exit 1; }
  cp -a "$OPENBLAS_DLL" "$OUT/lib/"
fi
...
cmake -Wno-dev -Wno-deprecated --no-warn-unused-cli ${CMAKE_GEN[@]+"${CMAKE_GEN[@]}"} -S "$WORK/spm" -B "$WORK/spm/build" \
...
if [ "$OS" = windows ]; then
  # //utf-8 sets source AND execution charset: a CTranslate2 header carries
  # a unicode literal that codepage 1252 cannot represent (C4566).
  cl //nologo //std:c++17 //O2 //EHsc //MD //utf-8 \
...
if [ "$RUNTIME_ONLY" -eq 0 ]; then
  ...
  convert_marian "$MT_MODEL_URL" "$MT_MODEL_SHA256" "it-en" "$OUT/models/mt-it-en"
  convert_marian "$MT_EN_IT_MODEL_URL" "$MT_EN_IT_MODEL_SHA256" "en-it" "$OUT/models/mt-en-it"
fi
...
if [ "$PIPER_ZIP" -eq 1 ]; then
  download "$PIPER_RELEASE_URL/piper_$PIPER_TAG.zip" "$WORK/piper.zip" "$PIPER_SHA"
  (cd "$WORK" && unzip -q piper.zip)
else
  download "$PIPER_RELEASE_URL/piper_$PIPER_TAG.tar.gz" "$WORK/piper.tar.gz" "$PIPER_SHA"
...
if [ "$OS" = macos ]; then
  ...
  download "$PIPER_PHONEMIZE_RELEASE_URL/piper-phonemize_$PP_TAG.tar.gz" \
    "$WORK/piper-phonemize.tar.gz" "$PP_SHA"
