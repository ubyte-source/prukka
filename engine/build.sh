...
# fetch downloads one pinned artifact, retrying transient upstream failures
# and naming the URL when it still fails — a bare `curl -fs` dies mute with
# exit 22 and leaves CI logs undebuggable.
fetch() {
  local url="$1" dest="$2"
...
download_whisper_model() {
  ...
  fetch "$WHISPER_MODELS_URL/$model" "$WORK/$model"
...
if [ "$PIPER_ZIP" -eq 1 ]; then
  fetch "$PIPER_RELEASE_URL/piper_$PIPER_TAG.zip" "$WORK/piper.zip"
  verify_sha256 "$WORK/piper.zip" "$PIPER_SHA"
  (cd "$WORK" && unzip -q piper.zip)
else
  fetch "$PIPER_RELEASE_URL/piper_$PIPER_TAG.tar.gz" "$WORK/piper.tar.gz"
...
if [ "$OS" = macos ]; then
  ...
  fetch "$PIPER_PHONEMIZE_RELEASE_URL/piper-phonemize_$PP_TAG.tar.gz" "$WORK/piper-phonemize.tar.gz"
...
download_voice() {
  ...
  fetch "$PIPER_VOICES_URL/$path/$name.onnx" "$OUT/models/tts/$name.onnx"
  verify_sha256 "$OUT/models/tts/$name.onnx" "$onnx_sha"
  fetch "$PIPER_VOICES_URL/$path/$name.onnx.json" "$OUT/models/tts/$name.onnx.json"
