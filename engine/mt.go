...
func runMT(args []string) (err error) {
	...
	dir := engineDir()
	modelDir := filepath.Join(dir, "models", "mt-"+from+"-"+to)
...
func startMTProc(dir, modelDir string) (*mtProc, error) {
	...
	return &mtProc{cmd: cmd, stdin: stdin, stdout: stdout, out: scanner}, nil
}
...
func (p *mtProc) closeInput() error {
	p.stdinOnce.Do(func() { p.stdinErr = p.stdin.Close() })

	return p.stdinErr
}

func (p *mtProc) kill() error {
