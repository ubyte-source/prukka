...
[![License](https://img.shields.io/badge/License-GPL--3.0--or--later%20%7C%20GPL--2.0--only-green.svg)](#license)

## Current status

- `prukka setup` installs every runtime dependency on macOS, Linux and
  Windows: the pinned, checksum-verified static FFmpeg, the managed native
  speech tools (whisper.cpp, CTranslate2/Opus-MT, Piper) and the model packs
  the configuration needs. The speech tools and packs are downloaded from the
  prukka release whose tag matches the daemon version and SHA-256 verified
  against `prukka-engine-catalog.json`, an asset of that same release. FFmpeg is
  a separate supply chain: setup downloads the platform pin declared in
  `internal/media/ffmpeg/builds.go` directly from its upstream vendor — BtbN on
  Linux and Windows, Martin Riedl on macOS — and verifies it against the
  SHA-256 recorded there. See [Managed FFmpeg runtime](docs/FFMPEG.md).
...
- The managed speech runtime ships prebuilt for macOS (amd64, arm64), Linux
  (amd64, arm64) and Windows (amd64); `engine/build.sh` builds all of them and
  the release carries one bundle per platform.
...
New to the code? Start with the reading path in
[Architecture](docs/ARCHITECTURE.md#reading-path): where audio enters, the six
files that carry it to a translated voice, and the question each one answers.
The rest of that document covers package responsibilities, the media flow and
...
- macOS or Linux on a supported architecture to run the commands below — they
  are Make-driven and assume a POSIX host, so on Windows use WSL2. Windows is a
  supported *target* (CI cross-builds and tests it, and the release ships
  `prukka_windows_amd64.zip`), not a development host; and
...
`mt` and `tts` subcommands over stdio; `engine/` holds the build recipe
(`build.sh`, `packs.sh`, `pins.sh` and their helper scripts, `mt.cpp`, patches)
that builds the native
tools (whisper.cpp, CTranslate2/SentencePiece, Piper) and model packs published
as release assets, plus the `prukka-engine-manifest.json` that the build
copies into every bundle. `prukka setup` downloads and verifies that managed
bundle;
...
A tolerated set of inactive fields (provider base URLs, remote-model tuning,
formats, rates and timeouts) loads without error but configures no runtime
feature; `prukka doctor` flags each one, and the next successful settings
save removes them. The
...
Control mutations, configuration reads, local-device enumeration, engine state
and `/api/v1/doctor` diagnostics require the install token.
Session list/event payloads expose a sanitised source label instead of source
credentials, paths or query strings. Session, stats and language reads, like
HLS, direct audio and WebVTT reads, do not require the token and rely on the
HTTP listener remaining inside the intended trust boundary.
...
The application and macOS/Windows drivers are GPL-3.0-or-later. Linux kernel modules
