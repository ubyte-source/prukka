...
bundled-driver gates. It is a subset of CI, not a mirror of it — CI *also*
blocks on `buf lint`, `govulncheck` across three build contexts, the
six-target cross-build matrix, the three acceptance demos and the full web set
(`make web`, `web-audit`, `licenses-check`, `web-e2e`). Those stay out of
`verify` because they need a network tool install or repeat a check it already
runs. Run what your change touches before pushing.
