...
- The maintainer provides `.golangci.yml` and the pinned linter version. For
  the coding agent this configuration is **read-only and non-negotiable**:
  never edit, replace, weaken, add exclusions to, re-pin, or bypass it with
  CLI flags.
...
- Enforcement:
  ...
  - Suppression gate: CI fails if a diff introduces `nolint`/`#nosec`
    directives or changes the linter version pin.
  - `CODEOWNERS`: `.golangci.yml`, `LINTER.sha256` and `tools/` require
    maintainer review.
...
```bash
make lint            # the pinned linter over three GOOS
...
- **Bench Gates**: the hot-path zero-alloc and load gates are blocking (`make bench`, `make load`)
...
make load            # the load gate (needs a key + ffmpeg)
