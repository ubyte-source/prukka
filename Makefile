...
.PHONY: verify
verify: mod-check lint-all suppression-check test bench cover-gate loadgen-test bundled-check gen-check proto-breaking ## Run the local blocking gate set: module graph, lint, tests, benches, coverage, generated code and the proto/driver gates.
