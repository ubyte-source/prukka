...
func TestPartialPacerPacesByStride(t *testing.T) {
	...
	pacer := &partialPacer{rate: rate, stride: sttPartialStride, run: func(
...
func TestDiscardedNoiseWindowCutsPartialEpoch(t *testing.T) {
	...
	run := 0
	pacer := &partialPacer{
		rate: rate, stride: tuning.partialStride,
...
func TestSegmentWorkerKeepsCaptureSideUnblocked(t *testing.T) {
	...
	release := make(chan struct{})
	first := true
	worker := newSegmentWorker(func(speechSegment) error {
		if first {
			first = false
			close(started)
			<-release
...
func TestQueuedFinalsBlockLaterPartialInference(t *testing.T) {
	...
	worker := newSegmentWorker(runner.run)
	...
	pacer := &partialPacer{
		rate: 1000, stride: 20 * time.Millisecond, blocked: worker.hasPending,
