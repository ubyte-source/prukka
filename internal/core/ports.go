...
// SampleRate is the system's canonical PCM sample rate in hertz: 16 kHz mono,
// the rate the speech engine expects and every media stage assumes. It lives
// here, with the PCM contract it belongs to, so the media adapters that only
// need the rate do not import the DSP pipeline for it.
...
type PCM struct {
	...
	Rate int           // sample rate in Hz; SampleRate is the internal reference
...
// is written directly to its target timeline rather than retained here; the
// target language is the sink's identity (engine.Output keys sinks by Lang),
// not segment state.
...
// Frames delivers labeled PCM from an opened source. Every source owns a
// releasable resource (a file, a capture pipe, a subprocess), so lifecycle is
// part of the port: Close must be idempotent and safe to call concurrently
// with an in-flight Next, which it interrupts.
