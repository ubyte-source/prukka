...
func TestRuntimeLifecycle(t *testing.T) {
	...
	rt := session.NewRuntime(store, lanes.starter, nil, nil, slog.New(slog.DiscardHandler))
...
func TestRuntimeRestartWaitsForTeardown(t *testing.T) {
	...
	rt := session.NewRuntime(store, probe.starter, nil, nil, slog.New(slog.DiscardHandler))
...
func TestRuntimeShutdownCleansEveryStoredSession(t *testing.T) {
	...
	rt := session.NewRuntime(store, lanes.starter,
		func(slug string) { cleaned <- slug }, nil, slog.New(slog.DiscardHandler))
...
func TestRuntimeDeleteCleansSelfEndedLane(t *testing.T) {
	...
	rt := session.NewRuntime(store, finished, cleanup, nil, slog.New(slog.DiscardHandler))
...
func TestRuntimeSurvivesFailingLanes(t *testing.T) {
	...
	rt := session.NewRuntime(store, failingStarter, nil, nil, slog.New(slog.DiscardHandler))
...
func TestRuntimeLogDoesNotExposeFailureSecrets(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(t.Context())
	rt := session.NewRuntime(store, starter, nil, nil, logger)
	done := make(chan error, 1)
	go func() { done <- rt.Run(ctx) }()
...
func TestRuntimeReportsRunningThenNaturalFinish(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(t.Context())
	rt := session.NewRuntime(store, starter, nil, nil, slog.New(slog.DiscardHandler))
	done := make(chan error, 1)
	go func() { done <- rt.Run(ctx) }()
...
// TestStatusEventsDoNotRestartTheLane asserts quiescence, which a sleep can
// only approximate: inside a bubble synctest.Wait returns when every other
// goroutine is durably blocked, so "the runtime is done reacting" is an
// exact question instead of "nothing happened in 50 wall-clock ms here".
