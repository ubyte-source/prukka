...
func TestReconfigureUsesLatestConcurrentSessionState(t *testing.T) {
	...
	// Reactivity allows one transient stale incarnation that its own event
	// supersedes; the contract is quiescence with the LATEST definition last.
	settled := starter.starts.Load()
	time.Sleep(50 * time.Millisecond)
	if got := starter.starts.Load(); got != settled {
		t.Fatalf("lanes kept churning after quiescence: %d -> %d starts", settled, got)
	}
	if got := starter.latest.Load(); got != "fr" {
		t.Fatalf("a stale incarnation started after the latest: langs %v", got)
	}
	if got := starter.updatedStarts.Load(); got > 3 {
