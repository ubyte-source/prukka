...
// teardownLog records which teardown hook the runtime chose per slug.
...
func newTeardownLog() *teardownLog {
	return &teardownLog{dropped: map[string]int{}, scrubbed: map[string]int{}}
}

func (l *teardownLog) Drop(slug string) {
	l.mu.Lock()
	l.dropped[slug]++
	l.mu.Unlock()
}

func (l *teardownLog) Scrub(slug string) {
	l.mu.Lock()
	l.scrubbed[slug]++
	l.mu.Unlock()
}

func (l *teardownLog) drops(slug string) int {
	l.mu.Lock()
	defer l.mu.Unlock()

	return l.dropped[slug]
}

func (l *teardownLog) scrubs(slug string) int {
	l.mu.Lock()
	defer l.mu.Unlock()

	return l.scrubbed[slug]
}

// reconfigureRecorder counts starts beside the teardown hooks the runtime ran.
...
func newReconfigureRecorder() *reconfigureRecorder {
	return &reconfigureRecorder{teardownLog: newTeardownLog(), starts: map[string]int{}}
}
...
func (r *reconfigureRecorder) assertSelective(t *testing.T) {
	...
	for _, slug := range []string{"running", "failed", "finished"} {
		if got := r.drops(slug); got != 0 {
			t.Fatalf("reconfigure dropped %q %d times; a restart clears outputs by scrubbing", slug, got)
		}
	}
	if r.scrubs("running") != 1 || r.scrubs("failed") != 1 || r.scrubs("finished") != 0 {
		t.Fatalf("scrubs = running:%d failed:%d finished:%d, want running/failed only",
			r.scrubs("running"), r.scrubs("failed"), r.scrubs("finished"))
	}
}
...
// assertSettledOnLatest pins the quiescence contract: one transient stale
// incarnation is allowed, but the latest definition must be started last and
// nothing may move after that.
...
// TestFailedLaneRetriesOnTheReconcileTick drives both clocks the runtime owns
// — Run's reconcileEvery ticker and reconcile's retryAt gate — on a synctest
// bubble's fake clock, so the wiring itself is under test.
func TestFailedLaneRetriesOnTheReconcileTick(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		store := NewStore()
		rt := NewRuntime(&RuntimeDeps{Store: store,
			Start: func(context.Context, *Session, func()) error {
				if starts.Add(1) == 2 {
					close(retried)
				}
		...
		// The deadline is load-bearing: a runtime that never retries leaves the
		// ticker firing forever, a live hang the bubble cannot report as a
		// deadlock.
		select {
		case <-retried:
...
func TestReconcileHealsMissedEvents(t *testing.T) {
	...
	rt := NewRuntime(&RuntimeDeps{Store: store, Start: starts.starter, Log: slog.New(slog.DiscardHandler)})
...
func TestReconcileRestartsRecreatedSession(t *testing.T) {
	...
	// No event is applied, and every public field is identical.
	rt.reconcile(ctx)
...
func TestLaneStarterCannotMutateReconcileSnapshot(t *testing.T) {
	...
	store := NewStore()
	rt := NewRuntime(&RuntimeDeps{Store: store,
		Start: func(ctx context.Context, s *Session, running func()) error {
			s.Langs[0] = "fr"
			close(started)
			running()
			<-ctx.Done()
...
func TestReconcileRetriesFailedLaneWhenDue(t *testing.T) {
	...
	var starts atomic.Int32
	rt := NewRuntime(&RuntimeDeps{Store: store,
		Start: func(ctx context.Context, _ *Session, running func()) error {
			if starts.Add(1) == 1 {
				return errors.New("temporary failure")
			}
			running()
...
// TestReconfigureUsesLatestConcurrentSessionState runs in a bubble because its
// assertions are about settling. Nothing here blocks on a bare channel, so the
// fake clock never advances and the reconcile ticker stays out of the scenario.
func TestReconfigureUsesLatestConcurrentSessionState(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		rt := NewRuntime(&RuntimeDeps{Store: store, Start: starter.starter, Log: slog.New(slog.DiscardHandler)})
...
func TestReconfigureSignalsCoalesce(t *testing.T) {
	...
	rt := NewRuntime(&RuntimeDeps{Store: NewStore(), Log: slog.New(slog.DiscardHandler)})
	for range 10 {
		rt.Reconfigure()
	}
...
func TestRetryScrubsWhileDeleteCleans(t *testing.T) {
	...
	if outputs.scrubs("scrub-vs-clean") == 0 || outputs.drops("scrub-vs-clean") != 0 {
		t.Fatalf("retry used scrubs=%d drops=%d, want scrub only",
			outputs.scrubs("scrub-vs-clean"), outputs.drops("scrub-vs-clean"))
	}
	...
	if outputs.drops("scrub-vs-clean") == 0 {
...
func TestUpdateKeepsEventLoopReactiveWhileDraining(t *testing.T) {
	...
	rt := NewRuntime(&RuntimeDeps{Store: store, Start: starter, Log: slog.New(slog.DiscardHandler)})
