...
// TestSubscriberOverflowDropsInsteadOfBlockingTheStore covers notify running
// under the store's write lock: a blocking send would freeze Create/Get/List
// for every client for as long as one SSE reader stalls.
func TestSubscriberOverflowDropsInsteadOfBlockingTheStore(t *testing.T) {
	...
	// cap(events) is the subscriber buffer; two more force the drop path.
...
// createSessions performs n store mutations off the test goroutine, so a
// blocking notify fails the caller's timeout instead of wedging the test.
...
// expectClosed drains any buffered events, then asserts the channel closes.
