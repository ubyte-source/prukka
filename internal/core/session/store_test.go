...
func TestGetReturnsIsolatedCopy(t *testing.T) {
	...
	st := session.NewStore()
	input := demo("demo")
	input.Subs = session.SubsVTT
	input.DubLangs = session.DubOnly("it")
	if err := st.Create(input); err != nil {
		t.Fatalf("Create returned error: %v", err)
	}
	input.Langs[0] = "fr"
	input.Subs = session.SubsOff
	...
	first.Langs[0] = "xx"
	first.Subs = session.SubsOff
	first.DubLangs.Langs()[0] = "xx"
...
func TestUpdateLangsPrunesTheDubbedSubset(t *testing.T) {
	...
	if err := s.ApplyFlags(map[string]string{session.FlagDubLangs: "IT,en"}); err != nil {
	...
	if kept := got.DubLangs.Langs(); len(kept) != 1 || kept[0] != "it" {
		t.Fatalf("dub selection = %v, want the removed language pruned", kept)
	}
}
...
func TestDeleteRemovesOnlyAReciprocalPair(t *testing.T) {
	...
	inbound.Pair = "meeting-out"
	outbound := demo("meeting-out")
	outbound.Pair = "meeting"
	unrelated := demo("other")
	unrelated.Pair = "meeting"
...
// TestSubscriberOverflowDropsInsteadOfBlockingTheStore: notify runs with the
// store's write lock held, so its only defense against a stalled subscriber is
// dropping past the buffer — a blocking send would freeze Create/Get/List for
// every client for as long as one SSE reader stalls. Excess events are
// dropped, and re-List (the documented resynchronization) still sees the truth.
func TestSubscriberOverflowDropsInsteadOfBlockingTheStore(t *testing.T) {
	t.Parallel()

	st := session.NewStore()
	events := st.Subscribe(t.Context())

	// cap(events) is the subscriber buffer; two more mutations than it holds
	// force the drop path while the subscriber reads nothing.
	mutations := cap(events) + 2
	select {
	case err := <-createSessions(st, mutations):
		if err != nil {
			t.Fatalf("Create: %v", err)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("store mutations blocked on an undrained subscriber")
	}

	if drained := drainPending(events); drained != cap(events) {
		t.Fatalf("drained %d events, want exactly the %d buffered — excess must drop", drained, cap(events))
	}
	if got := len(st.List()); got != mutations {
		t.Fatalf("List = %d sessions, want %d — re-List must report the dropped truth", got, mutations)
	}
}

// createSessions performs n store mutations off the test goroutine: if notify
// ever blocks, the caller's timeout fails cleanly instead of wedging the test
// while the store's write lock is held.
func createSessions(st *session.Store, n int) <-chan error {
	done := make(chan error, 1)
	go func() {
		for i := range n {
			if err := st.Create(demo(fmt.Sprintf("overflow-%02d", i))); err != nil {
				done <- err
...
// drainPending counts the events already buffered on the subscriber channel.
func drainPending(events <-chan session.Event) int {
	drained := 0
	for {
		select {
		case <-events:
...
func TestSubscriberEventsDoNotAlias(t *testing.T) {
	...
	candidate.Subs = session.SubsVTT
	candidate.DubLangs = session.DubOnly("it")
	if err := st.Create(candidate); err != nil {
		t.Fatalf("Create returned error: %v", err)
	}
	...
	firstEvent.Session.Subs = session.SubsOff
	firstEvent.Session.DubLangs.Langs()[0] = "xx"
...
func assertOwnedSession(t *testing.T, label string, got *session.Session) {
	...
	if got.Subs != session.SubsVTT {
		t.Fatalf("%s subtitle mode aliases caller data: %+v", label, got)
	}
	if kept := got.DubLangs.Langs(); len(kept) != 1 || kept[0] != "it" {
		t.Fatalf("%s dub selection aliases caller data: %v", label, kept)
	}
}
