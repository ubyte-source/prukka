...
// TestApplyFlagsParsesTheWireVocabulary: the open map is read once, here, and
// every option lands on a typed field.
...
// TestApplyFlagsRefusesOptionsNoLaneReads: the vocabulary is closed, so an
// option with no consumer — "voices" was accepted, stored and echoed for
// exactly that long — is rejected at the boundary instead of becoming a
// contract with no behavior.
...
// TestFlagsWireEchoesOnlyWhatTheSessionChose: a read renders the typed
// options back onto the same open map, and an untouched option stays absent.
func TestFlagsWireEchoesOnlyWhatTheSessionChose(t *testing.T) {
	...
	// dub_langs is the one option whose empty value carries meaning, so it
	// survives the round trip an empty value would otherwise lose.
...
// TestDubbedLangsResolvesTheTriState: the lane, the store and the validator
// all read dub_langs through this one resolution, so the tri-state — absent,
// empty selection, explicit list — is pinned here.
