...
// deleted sessions leave their outputs to the lane teardown alone.
func NewRuntime(store *Store, start LaneStarter, cleanup CleanupFunc, log *slog.Logger) *Runtime {
	return &Runtime{
		store:   store,
		start:   start,
		cleanup: cleanup,
...
// RetryFailed makes failed lanes eligible for immediate reconciliation.
// It is non-blocking so configuration RPCs never wait on media teardown.
func (r *Runtime) RetryFailed() {
	select {
	case r.retry <- struct{}{}:
	default:
	}
...
func (r *Runtime) makeFailedDue() {
	r.mu.Lock()
	defer r.mu.Unlock()

	for slug, reg := range r.lanes {
		if reg.state == laneExited {
