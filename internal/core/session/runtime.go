...
func (r *Runtime) syncCurrent(ctx context.Context, slug string) {
	...
	if exists && (reg.spec.revision != s.revision || !sameDefinition(reg.spec, &s)) {
		// Cancel without waiting: the replacement lane drains the predecessor
		// and scrubs in its own goroutine, keeping the event loop reactive.
		r.stop(slug)
...
func (r *Runtime) reconfigure(ctx context.Context) {
	for _, slug := range r.reconfigurable() {
		...
		// A deletion racing this restart is healed by its own event: the
		// loop stays reactive, so the delete stops the replacement promptly.
		r.stop(slug)
		r.launch(ctx, &s, func() { r.scrubOutputs(slug) })
	}
}
...
// teardown so old cleanup never destroys new resources. preStart, when set,
// runs in that same slot — after the drain, before the new lane.
func (r *Runtime) launch(ctx context.Context, s *Session, preStart func()) {
	...
	if !r.store.bindRuntime(slug, spec.revision, gen) {
		// The generation lost the store race before ever running, but its
		// done channel is already the link successors wait on: hand it to an
		// owner that closes it only after the predecessor drains. Closing it
		// here would short-circuit the chain and let a successor start over
		// a still-live teardown.
		cancel()
		r.wg.Add(1)

		go r.abandonLane(prev, done, slug, gen)
	...
	go r.runLane(laneCtx, cancel, prev, done, slug, gen, spec.revision, runSpec, preStart)
}

// abandonLane retires a lane that never ran, in runLane's exact teardown
// order, so the drain chain stays intact for whoever waits on done.
func (r *Runtime) abandonLane(prev <-chan struct{}, done chan<- struct{}, slug string, gen uint64) {
	defer r.wg.Done()
	defer close(done)
	defer r.forget(slug, gen)
...
func laneDisposition(reg laneReg, exists bool, s *Session, now time.Time) (same, retrying bool) {
	...
	// A draining lane is a corpse, never "this lane already runs": a launch
	// during its drain chains a replacement behind reg.done instead.
...
func (r *Runtime) runLane(
	...
	generation, revision uint64, spec *Session, preStart func(),
) {
	defer r.wg.Done()
	defer close(done)
	...
	// Runs after the predecessor drained and before this lane starts — the
	// slot restart scrubs need, without blocking the event loop for the drain.
	if preStart != nil {
		preStart()
	}
...
// stopAndWait serves the terminal paths (deletion, vanished session) on the
// event loop deliberately: clean() must never race the registrations of a
// recreated session with the same slug, and blocking here is what serializes
// them. Restarts do not wait — their scrub rides the replacement lane.
