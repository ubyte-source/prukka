...
// Delete removes a session and, when that session and another name each
// other in Pair, the reciprocal one too, notifying subscribers once per
// removal. A caller tracking WithMaxSessions capacity therefore cannot
// assume one Delete frees exactly one slot.
