...
// Off is the token that disables a media option. It stays untyped so the one
// word reads the same in every mode that admits it.
const Off = "off"

// SubsMode selects how a session renders captions. The zero value means the
// session did not choose: the control boundary fills it from the daemon
// configuration before the store sees the definition.
...
// ParseSubs validates CLI, API or configuration input as a SubsMode. It is
// the one place the caption vocabulary is spelled out.
...
// DubMode says whether a session synthesizes speech at all. The zero value
// dubs, because dubbing is what a session is for; DubOff reduces it to
// captions.
type DubMode string
...
// BedMode is the original-audio level under the dubbed voice, either Off or
// a decibel figure. It is named for the option rather than the level because
// core.BedLevel is the function that turns it into one; the zero value asks
// the lane for its configured fallback.
type BedMode string
...
// dubs every target language, present names exactly the listed subset, and
// present-but-empty means captions only. A bare slice cannot hold that
// distinction across a clone or a JSON round trip, so the chosen bit is its
// own field.
...
// AllDubLangs dubs every target language: the selection a session that named
// no subset carries.
...
// ParseDubLangs reads the wire's comma-separated subset through the one
// language registry. An empty list is legal and means captions only.
...
// retain drops languages that are no longer targets, so removing a language
// from a live session cannot leave a selection naming one it no longer
// carries.
...
// validate reports whether the selection names only distinct target
// languages of s.
...
// Wire flag keys. The v1 Session keeps map<string,string> flags open by
// design so a new option needs no schema change, which makes these names the
// boundary contract: ApplyFlags and FlagsWire are the only code that spells
// them, and everything past them reads a typed field.
const (
	FlagSubs     = "subs"
	FlagDub      = "dub"
...
// ApplyFlags parses the wire's open flag map onto the session's typed
// options. An unknown key is refused rather than stored, so a client cannot
// register an option no lane will ever read. Checks that span fields —
// dub_langs against the target list, pair against the slug — belong to
// validate, because a map has no order.
...
func (s *Session) applyFlag(key, value string) error {
	...
	case FlagDubLangs:
		// The one option whose empty value is itself a choice: present and
		// empty means captions only, so it must not take assign's
		// "value omitted, keep the default" path.
...
// "not set" so a client may send the key without choosing.
...
// FlagsWire renders the typed options back onto the v1 flag map. An option
// left at its zero value is omitted, so a read echoes what the session
// actually chose.
...
// validateMedia checks the typed options against each other and the target
// list.
...
// validateModes re-checks the per-value vocabularies ApplyFlags already
// enforced, because a Session may also be built in process and must not
// reach the store undecided in a way no parser ever saw.
...
// validatePair accepts a one-sided pair: the counterpart is created
// afterwards, and only a reciprocal link cascades on delete (Store.Delete).
...
// DubSelection holds a slice, so the struct is not comparable with ==.
...
// none when dubbing is off, every target when no subset was named, otherwise
// the named subset. It is the one answer the lane, the store and the control
// plane share.
