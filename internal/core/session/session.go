...
// Subs render modes; Off is also the token that disables dub and bed. These
// name the flag vocabulary so the control, lane and CLI layers never restate
// the literals.
const (
	Off      = "off"
	SubsVTT  = "vtt"
...
func validateMediaFlag(key, value string) error {
	switch key {
	case "subs":
		return requireFlagValue(key, value, "", Off, SubsVTT, SubsBurn)
	case "dub":
