...
// Session describes one unit of work. Language tags, the profile and the
// media options are validated at the boundary; code holding a Session trusts
// them and reads a field rather than a string key.
type Session struct {
	Slug       string
	Profile    Profile
