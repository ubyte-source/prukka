...
// redactSource replaces the exact configured source before the generic URL
// pass runs: an error may embed the source with characters the URL-token
// scan would split on, and the raw file path a file: source names must fall
// with it.
func redactSource(detail, source string) string {
	if source == "" {
		return detail
	}
	detail = strings.ReplaceAll(detail, source, redact.URL(source))
