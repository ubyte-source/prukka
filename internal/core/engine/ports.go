...
// Stable. SourceEnd, when HasSourceEnd is true, is the source-clock instant
// covered by this update. Native streaming adapters set it from the exact PCM
// boundary reported by their helper so inference delay cannot move captions.
...
type Transcription interface {
	...
	// returns an error once the session is closed or ctx is done.
...
// The two ways a provider can break the AudioStream contract, as matchable
// sentinels so a consumer can tell them apart with errors.Is.
var (
...
func (s *AudioStream) Err() error {
	s.once.Do(func() {
		if s.result == nil {
			s.err = errNoTerminalResult

			return
		}
		result, ok := <-s.result
		if !ok {
			s.err = errResultChannelClosed
