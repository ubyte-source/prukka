...
// commitCase drives one transcript stream and asserts the exact clauses each
// update yields.
...
// the loss bounded to the words the re-tokenization swallowed.
...
// TestCommitBroadcastHoldsDeeperTailThanCall separates the two wait-k
// policies by behavior: nine identical unpunctuated words leave the broadcast
// stable end at 7 (holdTail 2), one short of a forced maxClause cut, so
// nothing commits — while the call policy's shallower hold (holdTail 1) would
// already release an eight-word clause. The final proves the held words still
// commit rather than being silently dropped.
