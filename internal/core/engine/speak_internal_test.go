...
func testVoiceJob() voiceJob {
	return voiceJob{
		// Anchored one millisecond in the past: Windows's coarse monotonic
		// clock can report zero elapsed within one tick, and the metric
		// assertion wants a provably positive latency on every platform.
		endpointAt: time.Now().Add(-time.Millisecond),
		seg: &core.TranslatedSegment{
			Session: "call", Text: "translated speech", ScheduleAt: 500 * time.Millisecond,
		},
	}
}
