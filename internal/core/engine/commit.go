...
// then re-anchors the emitted frontier inside the new tokenization.
...
// content, not an index: a preserved spoken prefix keeps the old mark, else
// the frontier moves to the rightmost reappearance of the pending words —
// when alignment is ambiguous we skip rather than duplicate, because a
// duplicate is audible while a skipped region was already covered by audio.
