...
func TestHolderReloadSerializesWithUpdate(t *testing.T) {
	...
	// transaction finish under it. This one sleep cannot become a synctest
	// bubble: sync.Mutex.Lock is explicitly not durably blocking, so
	// synctest.Wait would return while the reload is still contending and
	// prove nothing about the ordering under test.
