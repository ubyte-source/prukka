...
// isolateDefaultConfig points the platform default config location at an empty
// temp dir, so Load("") cannot read the developer's own config.yaml.
...
func invalidProviderCases() []invalidConfigCase {
	return []invalidConfigCase{
		{
			name: "zero dispatch workers",
			body: "providers:\n  dispatch:\n    workers: 0\n",
			want: "providers.dispatch.workers",
		},
		{
			name: "oversized dispatch queue",
			body: "providers:\n  dispatch:\n    queue: 65537\n",
			want: "providers.dispatch.queue",
		},
		{
			name: "zero active lanes",
			body: "providers:\n  dispatch:\n    max_lanes: 0\n",
...
func TestLaneFingerprintTracksOnlyLaneRelevantChanges(t *testing.T) {
	...
	// These seed new sessions only; a live lane keeps running.
...
// FuzzLoad pins that a Config the strict loader accepts holds every invariant
// validate promises.
...
// assertValidatedConfig re-derives what validate promises on an accepted Config.
...
// cleanOrigin mirrors the promised origin shape: http(s), a host, nothing else.
...
func assertDefaultsInvariants(t *testing.T, cfg *config.Config) {
	...
	// The accepted value must BE the parser's own: downstream never re-parses.
	if mode, err := session.ParseSubs(string(cfg.Defaults.Subs)); err != nil || mode != cfg.Defaults.Subs {
		t.Fatalf("accepted defaults.subs %q is not the parsed mode %q: %v", cfg.Defaults.Subs, mode, err)
	}
	if level, err := session.ParseBed(string(cfg.Defaults.Bed)); err != nil || level != cfg.Defaults.Bed {
		t.Fatalf("accepted defaults.bed %q is not the parsed level %q: %v", cfg.Defaults.Bed, level, err)
	}
...
// baseLang reports whether l is a normalized concrete base language.
