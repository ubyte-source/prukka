...
// ModelForCall returns the low-latency override when configured; a bundle
// with a single STT model deliberately serves calls with that model.
...
func loadFile(path string) (*Config, error) {
	...
	case err == nil:
		...
		if !useDefaultCallModel {
			// A file that names a primary STT model without a call override
			// deliberately serves calls with that model. Empty and unrelated
			// partial configs retain the bundled, quality-first call default
...
// validateSubs checks the subtitle mode through the domain's own parser, so
// the vocabulary is defined once.
func validateSubs(mode string) error {
	if _, err := session.ParseSubs(mode); err != nil {
