...
func (m observedMetrics) PostCommitLatency(kind string, d time.Duration) {
...
func (s stagedTakeSynth) Speak(
	...
	go func() {
		...
		for _, data := range s.chunks {
			select {
			case audio <- core.PCM{Data: data, Rate: core.SampleRate, Ch: 1}:
			case <-ctx.Done():
				result <- ctx.Err()
...
func TestSpeakPlacesCompleteTakeAtLatestPlayoutFloor(t *testing.T) {
	...
	lane := testVoiceLane(stagedTakeSynth{
		delivered: delivered,
		release:   release,
		chunks:    chunks,
	})
	...
	go func() { done <- lane.speak(t.Context(), "en", track, testVoiceJob()) }()
	<-delivered
	...
	bed.Append(0, make([]int16, int(floor.Seconds())*core.SampleRate))
	cursor := pipeline.NewMixer(bed, track, math.Inf(-1)).Cursor()
	window := make([]int16, core.SampleRate/10)
...
func TestSpeakDoesNotPublishPartialTakeOnTerminalFailure(t *testing.T) {
	...
	err := lane.speak(t.Context(), "en", track, testVoiceJob())
...
func testVoiceLane(synth Synthesizer) *Lane {
	return &Lane{
...
func testVoiceJob() voiceJob {
	return voiceJob{
		// One millisecond in the past: Windows's coarse monotonic clock can
		// report zero elapsed within a tick.
		endpointAt: time.Now().Add(-time.Millisecond),
		seg: &core.TranslatedSegment{
			Slug:       "call",
			Text:       "translated speech",
			ScheduleAt: 500 * time.Millisecond,
		},
	}
}
