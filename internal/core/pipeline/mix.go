...
type Mixer struct {
	...
	live       bool // anchor at the live edge instead of the retained base
}
...
// Cursor returns an independent renderer over the same tracks. Each output
// owns its clock, buffers and sidechain envelope, so concurrent consumers
// receive the complete timeline instead of advancing one shared cursor.
...
// Live returns a cursor anchored at the live edge rather than the oldest
// retained instant: a device consumer attaching mid-session must speak
// now, not replay the retention window.
func (m *Mixer) Live() *Mixer {
	cursor := m.Cursor()
	cursor.live = true
...
func (m *Mixer) Pull(n int) (core.PCM, bool) {
	pcm, status := m.pull(n, nil)

	return pcm, status == PullReady
}

// PullInto mixes the next window into dst. The returned PCM aliases dst;
// callers can reuse the same buffer for every frame.
func (m *Mixer) PullInto(dst []int16) (core.PCM, bool) {
	pcm, status := m.pull(len(dst), dst)
...
func (m *Mixer) playoutStart(n int) (time.Duration, bool) {
	if m.live {
		return m.bed.liveStart(n)
	}
