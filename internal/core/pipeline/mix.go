...
func (m *Mixer) beginPlayoutLocked() bool {
	if !m.registered {
		...
		if m.accepted {
			// Only the voice track carries a trim fence: the bed is shared
			// by every language's mixer, so one cursor must not reach into it.
			m.voice.acquirePlayout()
		}
	}
...
// alignPlayoutClock seeds the playout clock on the first pull and keeps it
// inside the retained window afterwards. The seed is the newest instant the
// ready gate has already released, one quantum back: a cursor attached
// mid-session — a late listener, a re-pushed device, a rebuilt lane — then
// plays at the session's own delay instead of at the retention floor, tens of
// seconds stale. Thereafter the clock advances one quantum per pull and never
// chases the write head; only trim moving the base past a stalled cursor
// snaps it forward, because audio already dropped cannot be rendered and a
// silent window must not pass as ready.
func (m *Mixer) alignPlayoutClock(start time.Duration, n int) {
	if m.started {
		m.clock = max(m.clock, start)

		return
	}

	m.clock = start
	if edge, ok := m.bed.dueEdge(); ok {
		m.clock = max(start, edge-durationFor(n))
	}
	m.started = true
}
...
func (m *Mixer) render(dst []int16) core.PCM {
	n := len(dst)
	if len(m.bedBuf) < n {
	...
	out := core.PCM{Data: dst, Rate: core.SampleRate, Ch: 1, PTS: m.clock}
	m.mixInto(out.Data, bed, vox)
...
// |vox|/32767 > 0.01, the float comparison in exact integer form).
