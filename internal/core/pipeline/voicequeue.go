...
func (q *VoiceQueue) NextInto(dst []int16) (core.PCM, PullStatus) {
	...
	return core.PCM{Data: dst, Rate: SampleRate, Ch: 1, PTS: pts}, PullReady
}
...
func (q *VoiceQueue) BeginPlayout() bool {
	...
	if q.registered && q.accepted && !q.released {
		return true
	}
	if !q.registered || q.released {
		if q.group.acquire() {
			q.registered, q.accepted, q.released = true, true, false
		} else {
			q.registered, q.accepted = true, false
		}
	}

	return q.accepted && !q.released
}
