...
// minPlayoutCushion is the least distance a live consumer keeps from the
// write head when the session delay is smaller (zero-delay calls).
const minPlayoutCushion = 300 * time.Millisecond
...
func (t *Track) ready(from time.Duration, samples int, tailEnd time.Duration) bool {
	...
	if !t.finished.Load() && from+durationFor(samples)+max(t.delay, minPlayoutCushion) > end {
		return false
	}
...
func (t *Track) liveStart(samples int) (time.Duration, bool) {
	t.mu.Lock()
	defer t.mu.Unlock()
	...
	guard := max(t.delay, minPlayoutCushion)
	headroom := max(durationFor(samples), minPlayoutCushion)

	return max(t.base, end-guard-headroom), true
}
