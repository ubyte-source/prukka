...
// acquirePlayout arms the trim fence for one live cursor: while any consumer
// is registered, audio its render clock has not passed is never trimmed.
...
// releasePlayout retires one cursor. When the last one leaves, retention
// alone bounds memory again — floor keeps guarding placement, never trim,
// or a departed listener would pin every later take in memory for good.
...
// dueEdge is the newest instant the ready gate has already released — the
// inverse of its own due arithmetic. False while the track carries no anchor
// or no playout mapping, where the retained base is the only honest seed.
...
func (t *Track) trim() {
	...
	// Never trim across a LIVE consumer's playout fence: cutting unplayed
	// audio would slide the base past its clock and mute the track for good.
	// Once the last consumer releases, the fence is moot — floor still guards
	// placement in Append, while retention alone bounds memory.
