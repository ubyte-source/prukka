...
// Live retention window plus trim hysteresis; keeps Append amortized O(1).
const (
...
// when the session delay is smaller.
const playoutCushion = 300 * time.Millisecond

// Track assembles one language's dubbed audio on the source clock: segments
// spill right rather than overwrite, gaps are silence, only the retention
// window stays in memory. Safe for concurrent use.
type Track struct {
	anchoredAt time.Time
	clock      playoutClock
	buf        []int16
	// trim and ready do cross-field arithmetic atomics cannot express; the six
	// acquisitions per mixer pull cost ~150ns of a 100ms quantum.
	mu         sync.Mutex
...
// acquirePlayout arms the trim fence: while any consumer is registered, audio
// its render clock has not passed is never trimmed.
func (t *Track) acquirePlayout() {
	t.mu.Lock()
	t.consumers++
	t.mu.Unlock()
}

// releasePlayout retires one cursor; once the last leaves, retention alone
// bounds memory again.
func (t *Track) releasePlayout() {
	t.mu.Lock()
	t.consumers--
	t.mu.Unlock()
}

// ConfigurePlayout maps the delayed media clock onto real time; the lane
...
// Finish marks the track complete so readiness covers the buffered tail
// instead of stalling on the playout cushion.
func (t *Track) Finish() {
	t.finish()
}

// dueEdge is the newest instant the ready gate has already released; false
// while the track carries no anchor or no playout mapping.
func (t *Track) dueEdge() (time.Duration, bool) {
	t.mu.Lock()
	defer t.mu.Unlock()

	if t.buf == nil || !t.configured {
		return 0, false
	}

	return t.first + t.clock.now().Sub(t.anchoredAt) - t.delay, true
}
...
func (t *Track) ready(from time.Duration, samples int, tailEnd time.Duration) bool {
	...
	// The delay doubles as the live-edge cushion; a zero-delay call still needs
	// a floor, or the consumer races the writer chunk by chunk.
...
func (t *Track) trim() {
	...
	// Never trim across a live consumer's fence: cutting unplayed audio slides
	// the base past its clock and mutes the track for good.
	if t.consumers > 0 && t.floor > 0 && t.floor >= t.base {
