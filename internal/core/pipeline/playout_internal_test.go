...
func TestMixerWaitsForPlayoutAndKeepsVoiceWithinDelay(t *testing.T) {
	...
	voice := NewTrack()
	cursor := NewMixer(bed, voice, -15).Cursor()

	if _, ok := pull(cursor, samplesFor(100*time.Millisecond)); ok {
	...
	out, ok := pull(cursor, samplesFor(100*time.Millisecond))
	if !ok {
...
func TestLateCursorSeedsAtTheLiveEdgeNotTheRetentionFloor(t *testing.T) {
	...
	// The newest due instant is (elapsed - delay).
...
// TestTrimPastAStalledCursorSnapsItBackIntoTheRetainedWindow: without the snap
// a stalled cursor renders silence the ready gate still calls ready, and
// nothing ever closes the gap.
func TestTrimPastAStalledCursorSnapsItBackIntoTheRetainedWindow(t *testing.T) {
	...
	// The bed carries no trim fence, so retention can overtake a stalled cursor.
	...
	// Two more minutes past retention (30s + delay, 15s slack) while stalled.
...
func TestMixerDoesNotAdvancePastTheLiveEdge(t *testing.T) {
	...
	if _, ok := pull(cursor, chunk); ok {
		t.Fatal("mixer fabricated a window beyond the live edge")
	}

	// A complete window alone is not enough without a live cushion.
...
func TestMixerMutedBedPassesOnlyTheVoice(t *testing.T) {
	...
	cursor := NewMixer(bed, voice, math.Inf(-1)).Cursor()
	out, ok := pull(cursor, samplesFor(100*time.Millisecond))
	if !ok {
...
func TestTrimNeverCrossesTheLivePlayoutFence(t *testing.T) {
	...
	// Played up to 10s, then queue past retention (30s keep + 15s slack).
