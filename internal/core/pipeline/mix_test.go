...
func TestMixerDucksBedUnderVoice(t *testing.T) {
	...
	bed.Append(8*time.Second, tone(20000, 3*core.SampleRate))
	voice.Append(9*time.Second, tone(12000, core.SampleRate))

	m := pipeline.NewMixer(bed, voice, -15)

	out, ok := pull(m.Cursor(), 3*core.SampleRate)
	if !ok || out.PTS != 8*time.Second {
		t.Fatalf("Pull PTS = %v (ok=%v), want the bed anchor 8s", out.PTS, ok)
	}

	if early := out.Data[core.SampleRate/2]; early < 19000 {
		t.Fatalf("bed before voice = %d, want ≈ full 20000", early)
	}

	// Past the 50 ms attack: the bed at −15 dB (≈3557) under the 12000
	// voice → ≈15557.
	mid := out.Data[core.SampleRate+core.SampleRate/2]
	if mid < 15000 || mid > 16200 {
		t.Fatalf("voiced mix = %d, want ≈15557 (ducked bed + voice)", mid)
	}

	if late := out.Data[3*core.SampleRate-100]; late < 19000 {
...
func TestMixerZeroFillsPastTheLiveEdge(t *testing.T) {
	...
	bed := pipeline.NewTrack()
	bed.Append(0, tone(1000, core.SampleRate/10))

	cursor := pipeline.NewMixer(bed, pipeline.NewTrack(), -15).Cursor()

	out, ok := pull(cursor, core.SampleRate/5)
	if !ok {
	...
	second, _ := pull(cursor, core.SampleRate/10)
...
func TestMixerCursorsRenderTheFullTimelineIndependently(t *testing.T) {
	...
	bed := pipeline.NewTrack()
	bed.Append(3*time.Second, tone(1200, core.SampleRate))
...
func TestMixerDrainsTailOnlyAfterFinish(t *testing.T) {
	...
	bed := pipeline.NewTrack()
	// 200 ms of audio: shorter than the 300 ms cushion, so it sits inside it.
	bed.Append(0, tone(1000, core.SampleRate/5))
	bed.ConfigurePlayout(0)

	cursor := pipeline.NewMixer(bed, pipeline.NewTrack(), -15).Cursor()

	if _, ok := pull(cursor, 160); ok {
...
func TestBedAnchorsAtTheDelayedClock(t *testing.T) {
	...
	speech := make([]int16, core.SampleRate)
	for i := range speech {
		speech[i] = 1000
	}
	...
	probe := make([]int16, core.SampleRate)
