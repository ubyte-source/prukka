...
func BenchmarkFrameMixEncode(b *testing.B) {
	...
	if _, ok := pullInto(mixer, samples); !ok {
...
func BenchmarkFrameDecode(b *testing.B) {
	...
	for b.Loop() {
		...
		// Sink one element so the inlined stores cannot be proven dead —
		// without it this benchmark measures an empty loop.
