...
func TestSamplesInQuantumConvertsAlignedDurations(t *testing.T) {
	t.Parallel()

	// 16 kHz reference rate: 20 ms and 100 ms are the pipeline's real quanta.
	cases := []struct {
		name    string
		quantum time.Duration
		want    int
	}{
		{name: "call sink window", quantum: 20 * time.Millisecond, want: 320},
		{name: "capture chunk", quantum: 100 * time.Millisecond, want: 1600},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()

			if got := pipeline.SamplesInQuantum(tc.quantum); got != tc.want {
				t.Fatalf("SamplesInQuantum(%v) = %d, want %d", tc.quantum, got, tc.want)
			}
		})
	}
}

// TestSamplesInQuantumPanicsOnInvalidQuanta: the function's whole contract is
// that a misconfigured quantum is a wiring-time programming error — it must
// panic, never round.
func TestSamplesInQuantumPanicsOnInvalidQuanta(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{name: "zero", quantum: 0},
		{name: "negative", quantum: -20 * time.Millisecond},
		{name: "misaligned", quantum: pipeline.SamplePeriod + time.Nanosecond},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			...
			defer func() {
				if recover() == nil {
					t.Fatalf("SamplesInQuantum(%v) did not panic", tc.quantum)
				}
			}()
			pipeline.SamplesInQuantum(tc.quantum)
		})
	}
}

func TestPeakS16HandlesFullSignedRange(t *testing.T) {
	t.Parallel()
	...
	for _, test := range []struct {
		...
		{name: "empty", want: 0},
		{name: "silence", samples: []int16{0, 0}, want: 0},
		{name: "positive", samples: []int16{1, 1234, 12}, want: 1234},
		{name: "negative", samples: []int16{-1, -2345, -12}, want: 2345},
		{name: "minimum", samples: []int16{-32768, 32767}, want: 32768},
	} {
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()

			if got := pipeline.PeakS16(test.samples); got != test.want {
				t.Fatalf("PeakS16(%v) = %d, want %d", test.samples, got, test.want)
			}
		})
	}
}
