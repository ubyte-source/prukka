...
// Auto is the input sentinel that selects source-language auto-detection.
const Auto = "auto"
...
type Language struct {
	Tag    core.Lang // canonical base tag
	Name   string    // English name
	Native string    // native name shown in dropdowns
}
...
// All returns every registered language in stable English-name order; the
...
// Parse validates input as a BCP-47 tag ("it", "de-CH"), normalizing case and
// separators; the Auto keyword returns core.LangAuto, which callers needing a
// concrete language must reject explicitly.
...
// lookup scans the registry for a base tag.
...
// unknownError builds the error, attaching a "did you mean" hint when one exists.
