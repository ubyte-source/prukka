...
// The lock lives in the kernel, keyed by the open descriptor: a second
// descriptor on the same file must be refused while the first holds the lock
// and admitted the moment the holder closes — which is exactly how a crashed
// holder is recovered from without any staleness clock.
func TestLockFileExcludesSecondDescriptorUntilClose(t *testing.T) {
	t.Parallel()
