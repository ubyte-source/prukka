...
// TestCatalogURLRejectsInsecureOverride: the catalog is trusted by transport
// alone, so a plaintext mirror override must fail at resolution.
...
// A rejected URL may carry userinfo or a presigned query, so the rejection must
// name the endpoint through the shared reduction, never echo the raw URL.
// Fixtures are assembled from parts so no literal is a credential URL.
