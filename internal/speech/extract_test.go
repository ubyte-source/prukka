...
// against its OWN parent, so a per-entry textual check admits the chain while the
// kernel walks one directory further out at every hop.
...
// body behind it, so a multi-gigabyte fixture costs nothing. The tar writer is
// left unclosed: closing it would demand the declared bytes.
...
// The two byte caps would each catch the other's fixture, so both assert their
// own message and neither can be deleted behind the other's refusal.
...
func TestExtractArchiveBoundsOneEntry(t *testing.T) {
	...
	// Over the per-file cap but under the total, so only the per-file guard can
	// refuse it.
...
// FuzzExtractArchive: two invariants survive ANY input, accepted or refused —
// nothing materializes outside the extraction root, and the files that did land
// never sum past maxArchiveTotal. The root sits two directories beneath the
// walked base, so an escape of up to two hops lands where the walk sees it.
...
// assertExtractionContained walks base and fails on anything the archive placed
// outside dir. It re-checks the expansion cap on what DID land: the extractor
// reads declared header sizes, and only the on-disk sum proves actual bytes.
...
// extraction; anything else out there escaped.
...
// regularSize reports a regular file's on-disk size, zero for anything else.
