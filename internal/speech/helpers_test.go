// Shared fixtures for the speech installer tests; no Test functions live here.
...
func tarGz(tb testing.TB, entries []tarEntry) []byte {
	tb.Helper()
	...
	for _, e := range entries {
		...
		if err := tw.WriteHeader(header); err != nil {
			tb.Fatalf("write header %s: %v", e.name, err)
		}
		if header.Typeflag == tar.TypeReg {
			if _, err := tw.Write(e.body); err != nil {
				tb.Fatalf("write body %s: %v", e.name, err)
			}
		}
	}
...
// nativeHelpersPresent accepts the bundle on every OS.
...
// catalogDoc renders a valid catalog document for the current platform.
