...
// `prukka setup` consume it.
const (
	// CatalogSchema and CatalogVersion pin the producer/consumer wire contract.
...
// Pack kinds, the capability units the dashboard composes into languages.
const (
	PackSTT   = "stt"
	PackMT    = "mt"
	PackVoice = "voice"

	// PackIDSTTCore is the one mandatory pack, needed whatever the language
	// selection.
...
// runtimeTarget is the comparable identity of one runtime slot.
type runtimeTarget struct {
	os   string
	arch string
}

func (t runtimeTarget) String() string { return t.os + "/" + t.arch }
...
func (c *Catalog) validateRuntimes() error {
	...
	seen := make(map[runtimeTarget]bool, len(c.Runtimes))
	for i := range c.Runtimes {
		r := &c.Runtimes[i]
		if r.OS == "" || r.Arch == "" {
			return errors.New("engine catalog runtime is missing os or arch")
		}
		target := runtimeTarget{os: r.OS, arch: r.Arch}
		if seen[target] {
			return fmt.Errorf("engine catalog lists runtime %s twice", target)
		}
		seen[target] = true
		if err := validateArtifact("runtime "+target.String(), r.URL, r.SHA256, r.Size); err != nil {
			return err
		}
	}
...
// validateShape pins each pack to its canonical, kind-derived identity.
...
func MTPackID(from, to string) string { return enginebundle.MTPackName(from, to) }
...
// baseLang admits only concrete registered base tags: no region, no auto.
