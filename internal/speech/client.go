...
// empty) has no published release and requires the override. The catalog is
// the only download trusted by transport alone, so the override must pass
// the same predicate every artifact URL does — a plaintext mirror fails at
// resolution instead of silently downgrading that trust.
...
// lists: https everywhere, with plain http admitted only toward a local test
// server. Callers name the URL's role when wrapping. The rejection travels
// to `prukka setup` output, the daemon log and the catalog error surface,
// and the URLs it rejects may carry userinfo or a presigned query, so it
// names the endpoint only through redact.URL — and drops the *url.Error
// wrapper, which stores the whole raw URL on every parse failure.
