...
func TestWriterReporterToleratesANilWriter(t *testing.T) {
	t.Parallel()

	report := WriterReporter(nil)

	report(Progress{Phase: PhaseDownload, Item: "voice-it", DoneBytes: 1, TotalBytes: 2})
	report(Progress{Phase: PhaseDone, Item: "voice-it"})
}
