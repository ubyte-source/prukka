...
func TestResolveReturnsManagedBinary(t *testing.T) {
	...
	if _, ensureErr := installer.EnsureRuntime(context.Background(), catalog, nil); ensureErr != nil {
...
func TestResolveRejectsIncompleteInstall(t *testing.T) {
	...
	if strings.Contains(err.Error(), stateDir) {
		t.Fatalf("error exposes the install path: %v", err)
	}
}
