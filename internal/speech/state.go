...
func readState(path string) (*State, error) {
	...
	if s.Schema != stateSchema || s.Version != stateVersion {
		return nil, fmt.Errorf("%s declares %q v%d, want %q v%d", stateName, s.Schema, s.Version, stateSchema, stateVersion)
	}
	if s.Protocol != SupportedProtocol {
		return nil, fmt.Errorf("%s records protocol %d; this daemon needs %d", stateName, s.Protocol, SupportedProtocol)
	}
