...
// records which runtime and packs produced the bundle.
const (
...
// validatePackInventory holds every file a persisted inventory claims to
// modelFileEntry.
...
// validatePackFile holds one inventory entry's name and current type to the
// shared rule.
...
// bundleEntryType reports the file type an inventory entry currently has in the
// bundle. An absent entry reports a regular file's type, because a partially
// deleted pack must stay loadable for the reinstall that repairs it.
...
func removeQuietly(path string) {
	besteffort.Ignore(os.Remove(path))
}
