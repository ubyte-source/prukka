...
func readState(path string) (*State, error) {
	...
	if err := validatePackInventory(filepath.Dir(path), s); err != nil {
...
// validatePackInventory re-applies the pack-file rule (modelFileEntry) to a
// persisted inventory at the CONSUME side. The bundle operations that spend
// these names already ride *os.Root, so a hostile entry cannot escape at use
// time; this check is defense in depth at the inventory layer: the list must
// never NAME what staging refused, or a state file edited or corrupted
// between runs would re-admit it. An entry named but absent has no object to
// type-check — absence is the reinstall path's business — while the name
// rule still holds, and a probe the kernel cannot resolve inside the engine
// root condemns the entry that asked for it.
func validatePackInventory(dir string, s *State) error {
	...
	for i := range s.Packs {
		...
		for _, file := range pack.Files {
			if err := validatePackFile(root, file); err != nil {
...
// validatePackFile probes one inventory entry through the engine root — the
// same containment every bundle operation rides, so a name only resolvable
// by leaving the root fails here the way it would there — and then holds
// the entry's name and current type to the shared rule.
func validatePackFile(root *os.Root, file string) error {
	typ, err := bundleEntryType(root, file)
	if err != nil {
		return fmt.Errorf("file %q: %w", file, err)
	}

	return modelFileEntry(file, typ)
}

// bundleEntryType reports the file type an inventory entry currently has in
// the bundle. An absent entry reports a regular file's type — the one shape
// the rule has nothing against — because a partially deleted pack must stay
// loadable for the reinstall that repairs it.
func bundleEntryType(root *os.Root, file string) (fs.FileMode, error) {
	info, err := root.Lstat(filepath.Join(bundleDirName, filepath.FromSlash(file)))
