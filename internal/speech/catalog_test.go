...
// catalogFixture is a mutable, typed valid document.
...
func TestParseCatalogRejectsInvalidDocuments(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "mt regional language",
			mutate: func(f *catalogFixture) {
				f.packs[1]["from"] = "it-IT"
				f.packs[1]["id"] = "mt-it-IT-en"
			},
		},
		{name: "mt auto language",
			mutate: func(f *catalogFixture) {
				f.packs[1]["from"] = "auto"
				f.packs[1]["id"] = "mt-auto-en"
			},
		},
		{name: "voice path escape",
			mutate: func(f *catalogFixture) {
...
// FuzzParseCatalog: the parser must never panic, a refusal must never echo the
// document wholesale (catalog URLs may carry userinfo or a presigned query), and
// an accepted document must hold every invariant the installer then trusts.
func FuzzParseCatalog(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if parseErr != nil {
			// Long enough that a verbatim reappearance cannot be the one character
			// encoding/json legitimately names in a syntax error.
