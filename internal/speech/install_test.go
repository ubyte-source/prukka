...
// Two ordinary catalog packs compose into a write outside the engine root: every
// hop is lexically legal against its OWN parent, WalkDir lstats so the links
// enter the file list as plain files, and the kernel resolves one directory
// further out per hop.
...
// and is published by swapping the whole tree, so no per-file filter sees it.
...
// is itself a link must not be followed out of the engine root.
func TestCarryModelsRefusesASymlinkedModelsPath(t *testing.T) {
	...
	// An interrupted carry whose successor bundle points models/ at the state
	// directory, one level above the engine root.
...
// A model pack carries data: a symlink, device node or socket has no place in
// it, even pointing at an innocent sibling.
...
func TestRemovePackDeletesOwnedFiles(t *testing.T) {
	...
	// The prune stops at models/, which belongs to the bundle, not to any pack.
...
// A successor runtime can keep models/ a real directory and plant the chain
// beneath it, which carryModels' Lstat skip lets through. Deleting along that
// chain must be refused by the same kernel handle the publish uses.
...
// plantEscapingChain plants the hop chain inside a models directory; the kernel
// resolves each hop one directory further out, so models/h3 is the state dir.
...
// recordPackFiles rewrites one pack's file list in the live inventory; the files
// a removal deletes are named by the state, not by the archive.
...
func TestEnsureRuntimeUpgradeCarriesModels(t *testing.T) {
	...
	// A new runtime artifact upgrades the bundle; installed models must survive.
...
// A crashed holder must not outlive its process: the leftover lock FILE alone
// must never read as busy.
func TestOperationLockRecoversInstantlyFromCrashedHolder(t *testing.T) {
	t.Parallel()

	f := newInstallFixture(t, map[string][]byte{"stt-core": packArchive(t, "models/stt/a.bin")})

	// What a crash leaves behind: the lock file exists, freshly touched, with no
	// live process holding the kernel lock on it.
...
func TestRecoverBundleRestoresInterruptedSwap(t *testing.T) {
	...
	// A crash between the two swap renames: retired, no successor published.
...
// An interrupted model carry leaves both bundles present with some models still
// in old; recovery must re-home them, never lose them or leave old behind.
func TestRecoverBundleFinishesInterruptedModelCarry(t *testing.T) {
	...
	// A crash mid-carry: a retired copy still holding a model the live bundle
	// does not have.
	if err := os.MkdirAll(filepath.Join(old, modelsDirName), 0o700); err != nil {
