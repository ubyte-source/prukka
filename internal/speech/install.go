...
// Managed layout under the daemon state directory: engine/bundle/ is the live
// bundle root PRUKKA_ENGINE_ROOT points at, engine/state.json the installed
// inventory, engine/.stage-* per-operation staging.
const (
...
// The deadlines both engine surfaces share: the daemon's engine RPCs and
// `prukka setup`.
const (
	// CatalogTimeout bounds one catalog fetch.
...
// NewInstaller wires an installer under stateDir.
...
// EnsureRuntime installs or upgrades the platform runtime, reporting whether
// anything changed; installed model packs survive an upgrade.
...
// incompatible inventory reinstalls from scratch (nil state, no error).
...
// installed, or whose files are already gone, is a no-op.
...
// empty, through the same *os.Root that published them.
...
// swapBundle publishes a staged runtime, carrying the previous bundle's models
// over only AFTER the successor is published, so the sole copy is never moved
// out of the live tree. Any step can fail — notably the Windows retire rename,
// which a running daemon's open helpers block with a sharing violation — and
// recoverBundle then finishes the swap on the next operation.
...
func (i *Installer) recoverBundle() {
	...
	if _, err := os.Stat(bundle); errors.Is(err, fs.ErrNotExist) {
		besteffort.Ignore(os.Rename(old, bundle))
...
// rootDirNames lists one directory's entry names through a root. The handle is
// released before the names are returned: the caller renames the entries out of
// this very directory, which an open handle blocks on Windows.
...
// publishPackFiles moves staged pack files into the live bundle. Rename
// replaces the destination atomically on all three platforms, so a reinstall
// never leaves the daemon a window in which the model is absent.
...
// stagedModelFiles lists a staged pack's files, holding every entry to
// modelFileEntry.
...
// modelFileEntry is the pack-file rule: a regular file — never a symlink or
// device node — under a clean relative models/ path. It must be applied on BOTH
// sides of the inventory, by stagedModelFiles on publish and by readState on
// load, or a hand-edited state file re-admits what staging refused. The runtime
// bundle is the exception: it does ship symlinks, the version links of its
// shared libraries, and is published by swapping the whole tree rather than
// through a Files list.
...
// cleanRelativeSlashPath reports whether name is the canonical slash form the
// installer records: relative, no backslash, no empty, "." or ".." element.
...
// packFilesPresent reports whether every file of an installed pack is still on
// disk, so a partially deleted pack reinstalls instead of no-oping. The final
// component is not followed: a link where a published file belongs counts as
// absent.
...
// daemon must never stage over each other. The kernel releases the lock on
// unlock and on process death alike, so there is no staleness clock and no
// window in which two processes both reclaim it.
func (i *Installer) lock() (func(), error) {
	...
	// Every locked operation self-heals an interrupted swap first, and only a
	// locked one: the read path never repairs what it cannot serialize.
	i.recoverBundle()
...
// reporting ErrBusy against a live holder. The file is never unlinked:
// remove-and-recreate would let two processes lock two different inodes for the
// same path and both believe they hold the operation lock.
...
// cleanupStages removes leftovers of interrupted operations old enough to be
// certainly abandoned.
...
// nativeHelpers are the compiled tools a runtime bundle must contain.
func nativeHelpers() []string {
	...
	if runtime.GOOS == hostos.Darwin {
		// ffmpeg's raw AVFoundation input is silent under a launchd daemon, so
		// the bundle ships a native capture helper instead.
...
func removeTreeQuietly(path string) {
	...
	besteffort.Ignore(os.RemoveAll(path))
}

// pruneEmptyParents removes the directories a removal emptied, from dir up to
// (and excluding) stop. The bound is a path boundary, not a string prefix, and
// the first directory that is not empty ends the climb.
