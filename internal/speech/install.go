...
type Installer struct {
	...
	progress Reporter
	root     string
	// lockRefresh is how often a held operation lock is touched; zero uses
	// the default. A held lock must be refreshed well within staleStageAge so
	// a long but live operation is never mistaken for a crashed holder.
...
func (i *Installer) swapBundle(stage string) error {
	...
	if err := carryModels(old, bundle); err != nil {
		return err
	}
...
// daemon must never stage over each other. The holder keeps the lock fresh
// while it works, so a long but live operation is never stolen; a crashed
// holder stops refreshing and its lock decays after staleStageAge.
...
// acquireLockFile takes the exclusive lock file, reclaiming one abandoned by a
// crashed holder — a file left untouched past staleStageAge.
...
// refreshLock pushes the held lock's mtime forward until the returned stop is
// called, so a long but live operation is never mistaken for a crashed holder.
// A crashed holder stops refreshing and its lock decays after staleStageAge.
func (i *Installer) refreshLock(path string) func() {
	interval := i.lockRefresh
	if interval <= 0 {
		interval = staleStageAge / 4
	}
	...
	go func() {
		...
		for {
			...
			case <-stop:
				return
			case now := <-ticker.C:
				// A Chtimes failure means the lock file is gone (teardown or
				// an external removal): nothing left to keep fresh.
