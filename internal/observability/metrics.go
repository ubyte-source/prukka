...
// latencyBuckets span sub-frame work up through the multi-second broadcast
// paths.
var latencyBuckets = []float64{0.05, 0.1, 0.25, 0.5, 1, 1.8, 3.5, 5, 7, 9, 14, 20}

// Metrics holds every Prometheus collector on a dedicated registry, never the
// global default.
type Metrics struct {
	reg *prometheus.Registry

	postCommitLatency  *prometheus.HistogramVec
	sttInference       *prometheus.HistogramVec
	sessionsRegistered prometheus.Gauge
	sessionsByState    *prometheus.GaugeVec
}

// SessionCounts is a bounded aggregate of the session registry.
...
func NewMetrics() *Metrics {
	...
	m := &Metrics{
		reg: reg,
		postCommitLatency: prometheus.NewHistogramVec(prometheus.HistogramOpts{
			Name: "prukka_post_commit_latency_seconds",
			Help: "Pipeline latency from source-clause commit to caption publication " +
				"or placement of a complete synthesized take.",
	...
	reg.MustRegister(m.postCommitLatency, m.sttInference, m.sessionsRegistered, m.sessionsByState)
...
// PostCommitLatency records the post-commit pipeline path — excluding device
// playback and the capture/STT interval — where kind is caption or voice.
func (m *Metrics) PostCommitLatency(kind string, d time.Duration) {
	m.postCommitLatency.WithLabelValues(kind).Observe(d.Seconds())
}
