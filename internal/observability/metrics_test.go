...
func TestMetricsRecordAndExpose(t *testing.T) {
	...
	m.PostCommitLatency("caption", 2*time.Second)
	m.STTInference("partial", 125*time.Millisecond)
	...
	for _, want := range []string{
		// The HELP line is scraped prose: an SRE reads it in Grafana's metric
		// browser, so it moves with the semantics the histogram measures.
		"# HELP prukka_post_commit_latency_seconds Pipeline latency from " +
			"source-clause commit to caption publication or placement of a " +
			"complete synthesized take.",
		`prukka_post_commit_latency_seconds_count{kind="caption"} 1`,
		`prukka_stt_inference_seconds_count{kind="partial"} 1`,
	...
	for _, stale := range []string{"prukka_stage_latency", "prukka_fallback_active", "prukka_e2e_latency"} {
		if strings.Contains(out, stale) {
			t.Errorf("unexpected collector %q in metrics output", stale)
		}
	}
}
