...
// daemon's live-reconfiguration signal, fired after a pack removal that may
// strand a running lane; baseline absorbs a strictly-additive capability
// change (a pack install) into the reconfigure baseline without restarting
// live lanes. Either may be nil.
func NewEngine(
	installer EngineInstaller, source CatalogSource, holder *config.Holder,
	change, baseline func(), log *slog.Logger,
) *Engine {
	return &Engine{
		installer:   installer,
		source:      source,
