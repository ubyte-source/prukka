...
// after a pack operation changed the capability configuration.
...
func (e *Engine) finish(kind, packID string, err error) {
	...
	if err == nil && kind != engineOpInstallRuntime {
		e.changed()
	}
}
...
// extendConfigForPack maps one pack onto its configuration capability.
func extendConfigForPack(c *config.Config, pack *speech.Pack) {
	local := &c.Providers.Local
	switch pack.Kind {
	case speech.PackMT:
		for _, pair := range local.MT.Pairs {
			if string(pair.From) == pack.From && string(pair.To) == pack.To {
				return
			}
		}
		local.MT.Pairs = append(local.MT.Pairs, config.TranslationPair{
			From: core.Lang(pack.From), To: core.Lang(pack.To),
		})
	case speech.PackVoice:
		for i := range local.TTS.Voices {
			if string(local.TTS.Voices[i].Language) == pack.Lang {
				local.TTS.Voices[i].Voice = pack.Voice

				return
			}
		}
		local.TTS.Voices = append(local.TTS.Voices, config.VoiceModel{
			Language: core.Lang(pack.Lang), Voice: pack.Voice,
		})
	}
}

// retireConfigForPack removes one pack's configuration capability.
func retireConfigForPack(c *config.Config, pack *speech.InstalledPack) {
	local := &c.Providers.Local
	switch pack.Kind {
	case speech.PackMT:
		kept := local.MT.Pairs[:0]
		for _, pair := range local.MT.Pairs {
			if string(pair.From) != pack.From || string(pair.To) != pack.To {
		...
		local.MT.Pairs = kept
	case speech.PackVoice:
		kept := local.TTS.Voices[:0]
		for _, voice := range local.TTS.Voices {
			if string(voice.Language) != pack.Lang {
				kept = append(kept, voice)
			}
		}
		local.TTS.Voices = kept
	}
}
