...
// server can accept another, under the same bounded drain the servers get.
// Cancellation reaches the installer's deferred unlock, so a daemon restart
// does not inherit a stale install lock. Nil when no engine surface is
// wired (tests); then there is nothing to drain.
