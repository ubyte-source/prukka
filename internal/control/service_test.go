...
// emptyInstaller reports an installed engine with no packs, and accepts
// every operation without doing one.
...
// newTestEngine gives the service a working engine surface. Without it
// GET /api/v1/engine cannot be exercised end to end, which is how the route
// stayed outside the token-gate tests.
func newTestEngine(t *testing.T) *control.Engine {
	...
	engine := control.NewEngine(emptyInstaller{}, emptyCatalog{}, holder, nil, slog.New(slog.DiscardHandler))
	t.Cleanup(func() {
...
// TestCreateSessionLeavesTheRequestUntouched: the session must not keep the
// request's flag map by reference — seeding defaults would then write into a
// map the caller still owns. Parsing the open map into typed fields at the
// boundary is what keeps the request read-only.
