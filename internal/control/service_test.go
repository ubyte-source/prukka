...
func (p *stubPusher) Push(slug string, tag core.Lang, target string, subs session.SubsMode) error {
	p.attempts++
	if p.failed {
		return fmt.Errorf("warming up: %w", core.ErrNotReady)
	}

	p.calls = append(p.calls, [4]string{slug, string(tag), target, string(subs)})

	return nil
}

// newTestService wires a Service with an inert doctor, fixed defaults and the
// given pusher.
...
func newTestServiceWithCapability(
	...
	return control.NewService(&control.ServiceDeps{
		Store:   store,
		Version: "test",
		Doctor:  func() []doctor.Check { return nil },
		Defaults: func() control.SessionDefaults {
		...
		Devices: func(context.Context) []discover.Device {
...
// emptyInstaller reports an installed engine with no packs and accepts every
// operation without doing one.
type emptyInstaller struct{}

func (emptyInstaller) State() (*speech.State, error) { return &speech.State{}, nil }

func (emptyInstaller) EnsureRuntime(context.Context, *speech.Catalog, speech.Reporter) (bool, error) {
	return false, nil
}

func (emptyInstaller) InstallPack(context.Context, *speech.Catalog, string, speech.Reporter) error {
	return nil
}

func (emptyInstaller) RemovePack(string) error { return nil }

// emptyCatalog offers nothing, which is a valid catalog.
type emptyCatalog struct{}

func (emptyCatalog) Catalog(context.Context) (*speech.Catalog, error) { return &speech.Catalog{}, nil }

// newTestEngine gives the service a working engine surface.
func newTestEngine(t *testing.T) *control.Engine {
	t.Helper()
	...
	engine := control.NewEngine(&control.EngineDeps{
	...
	t.Cleanup(func() {
		if shutdownErr := engine.Shutdown(context.Background()); shutdownErr != nil {
			t.Errorf("engine shutdown: %v", shutdownErr)
		}
	})
...
// TestCreateSessionLeavesTheRequestUntouched: seeding defaults must not write
// into the flag map the caller still owns.
func TestCreateSessionLeavesTheRequestUntouched(t *testing.T) {
	t.Parallel()

	svc := newTestService(t, session.NewStore(), &stubPusher{})
	flags := map[string]string{session.FlagSource: "it"}
	request := &v1.CreateSessionRequest{Session: &v1.Session{
		Slug:      "read-only-request",
		Profile:   "broadcast",
		SourceUrl: "file:///tmp/x.wav",
		Langs:     []string{"en"},
		Flags:     flags,
	...
	if _, err := svc.CreateSession(t.Context(), request); err != nil {
		t.Fatalf("CreateSession returned error: %v", err)
	}

	if !maps.Equal(flags, map[string]string{session.FlagSource: "it"}) {
		t.Fatalf("CreateSession mutated the caller's flags: %v", flags)
	}
}
...
func TestCreateCallSessionDefaultsToZeroDelay(t *testing.T) {
	...
	// An omitted delay on a live call must not postpone the interpreter by the
	// configured eight seconds.
	reply, err := svc.CreateSession(t.Context(), &v1.CreateSessionRequest{Session: &v1.Session{
		Slug:      "live-call",
		Profile:   "call",
...
func TestPushLetsTheMediaRegistryDecideStartingReadiness(t *testing.T) {
	...
	_, err := svc.Push(t.Context(), &v1.PushRequest{
		Slug:      "starting",
		Lang:      "en",
		TargetUrl: "rtmp://live.example/out",
		Subs:      "off",
	})
...
func assertTerminalPushBlocked(t *testing.T, slug string, starter session.LaneStarter) {
	...
	runtime := session.NewRuntime(&session.RuntimeDeps{Store: store, Start: starter, Log: slog.New(slog.DiscardHandler)})
...
func startRunningSession(t *testing.T, store *session.Store, candidate *session.Session) {
	...
	ctx, cancel := context.WithCancel(t.Context())
	runtime := session.NewRuntime(&session.RuntimeDeps{
		Store: store,
		Start: func(ctx context.Context, _ *session.Session, running func()) error {
			running()
			close(started)
			<-ctx.Done()
