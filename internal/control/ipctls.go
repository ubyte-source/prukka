...
func ensureIPCKeypair(stateDir string) error {
	...
	// Order is not load-bearing once both halves are published whole: the guard
	// above demands both files, so whichever rename lands last is the commit
	// point either way, and neither name can hold torn content in between.
...
// publishSecret stages one PEM inside the state directory and publishes it
// under its final name with a rename. Staging buys what os.WriteFile cannot:
// the fresh temporary carries its own 0600 mode, so a name already on disk
// with looser permissions cannot leave the private key readable; the rename
// replaces a symlink planted at that name instead of writing through it into
// the attacker's file; and it publishes whole content, so a mint interrupted
// between the two halves leaves one complete file and one absent — which the
// both-files-exist guard remints — rather than a truncated certificate that
// satisfies the guard forever while tls.LoadX509KeyPair fails on every start.
// The rename itself is deliberately not fsynced: a keypair lost to a power cut
// is simply reminted, whereas torn content would not be, and Sync below is
// what rules that out.
