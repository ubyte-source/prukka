...
func serveSSE(
	...
	// read guards, so it is only wired for a client that proves the token.
	// Without an engine or without the token, engineEvents stays nil — a
	// permanent no-op in streamSSE's select.
