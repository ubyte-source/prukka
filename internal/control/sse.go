...
func sseHandler(store *session.Store, log *slog.Logger, dubbed DubbedLanguagesFunc) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		serveSSE(w, r, store, log, dubbed)
	})
}
...
func serveSSE(
	...
	flusher.Flush()
	streamSSE(w, flusher, events, store, log, dubbed)
}
...
func streamSSE(
	w io.Writer, flusher http.Flusher, events <-chan session.Event, store *session.Store, log *slog.Logger,
	dubbed DubbedLanguagesFunc,
) {
