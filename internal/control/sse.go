...
// wireSession is the JSON shape shared by SSE snapshots and events; its keys
// must match the gateway's protojson names for v1.Session.
...
func serveSSE(
	...
	// missed. The engine sub-stream carries the install and provider detail
	// the token-gated /api/v1/engine read guards, so it needs the token;
	// without one engineEvents stays nil, a no-op in streamSSE's select.
	events := store.Subscribe(r.Context())
	var engineEvents <-chan wireEngineEvent
	if engine != nil && engineStreamAuthorized(r, token) {
...
// dashboard passes the control token as a query parameter.
...
// streamSSE forwards events and periodic snapshots until the subscription
// closes; the snapshots repair anything the bounded subscriber buffer dropped.
func streamSSE(
	...
	for {
		...
		case e, open := <-events:
			...
			flusher.Flush()
		case progress, open := <-engineEvents:
			if !open {
				return
			}
