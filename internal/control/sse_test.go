...
// TestEngineStreamAuthorizedGatesOnQueryToken: session snapshots stay reachable
// without a token, engine progress does not.
func TestEngineStreamAuthorizedGatesOnQueryToken(t *testing.T) {
	t.Parallel()
	...
	cases := []struct {
		...
		{"matching token authorizes", "?token=s3cret", "s3cret", true},
		{"wrong token is refused", "?token=nope", "s3cret", false},
		{"missing token is refused", "", "s3cret", false},
		{"empty token matches empty query", "", "", true},
	}
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			r := httptest.NewRequestWithContext(t.Context(), "GET", "/api/v1/events"+tc.query, http.NoBody)
			if got := engineStreamAuthorized(r, tc.token); got != tc.want {
				t.Fatalf("engineStreamAuthorized(%q, token=%q) = %v, want %v", tc.query, tc.token, got, tc.want)
			}
		})
	}
}
...
// gateway's protojson field names: the two are separate on purpose (presence
// semantics differ), and the web client reads both.
func TestWireSessionKeysMatchTheProtoContract(t *testing.T) {
	...
	proto := &v1.Session{
		...
		Error:                "x",
		Langs:                []string{"en"},
		EffectiveDubbedLangs: []string{"en"},
		DelaySeconds:         &delay,
		Flags:                map[string]string{"source": "it"},
	}
