...
// TestEngineStreamAuthorizedGatesOnQueryToken checks that only a matching
// query token unlocks the engine sub-stream (EventSource cannot set a header),
// while session snapshots — served regardless — stay reachable without one.
