...
func install(ctx context.Context, opts *Options) error {
	...
	bootoutErr := runQuiet(exec.CommandContext(ctx, "launchctl", "bootout", agentTarget()))
...
func bootstrap(ctx context.Context, path string) error {
	...
	for range bootstrapAttempts {
		if err = runQuiet(exec.CommandContext(ctx, "launchctl", "bootstrap", domain, path)); err == nil {
			return nil
		}
...
func restart(ctx context.Context) error {
	...
	kickErr := runQuiet(exec.CommandContext(ctx, "launchctl", "kickstart", "-k", agentTarget()))
	if kickErr == nil {
		return nil
	}
...
func remove(ctx context.Context) error {
	...
	if bootoutErr != nil {
		...
		if printErr := runQuiet(exec.CommandContext(ctx, "launchctl", "print", agentTarget())); printErr == nil {
			return bootoutErr
		}
	}
