...
// directory, is enabled in the user manager and, with --now, replaces any
// live instance — `enable --now` alone would leave a running daemon on the
// previous ExecStart, which macOS and Windows both refuse to do.
...
// TestServiceVerbsRefuseRoot: under sudo every verb must refuse before
// touching systemd — the manager and unit directory would resolve from
// root's identity, so status would answer "not installed" for a live user
// service and `sudo prukka update` would claim success while the old
