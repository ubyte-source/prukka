...
// euid is read through a variable so tests can exercise the guard without
// running as root.
var euid = os.Geteuid

// rootGuard refuses every service verb under sudo: systemctl --user and
// unitPath would resolve the user manager and unit directory from root's
// identity and silently act on the wrong (empty) service.
...
func install(ctx context.Context, opts *Options) error {
	...
	// leave it running the previous ExecStart, --config and binary image until
	// the next logout. restart starts a dead unit and replaces a live one, so
	// it covers both cases — the same replace-the-live-instance step launchctl
	// bootout and the scheduler's End take on the sibling platforms.
	return restart(ctx)
}
...
// disableOutcome resolves a failed disable: systemd also refuses the verb
// for a unit that was never loaded, which is harmless exactly when no unit
// is left running.
