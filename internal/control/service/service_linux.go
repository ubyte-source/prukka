...
func install(ctx context.Context, opts *Options) error {
	...
	if opts.Now {
		return procio.RunQuiet(exec.CommandContext(ctx, "systemctl", "--user", "enable", "--now", unit))
	}

	return procio.RunQuiet(exec.CommandContext(ctx, "systemctl", "--user", "enable", unit))
}
