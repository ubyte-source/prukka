...
// TestStopEngineJoinsTheInFlightInstall: the server owns the engine's
// drain — its shutdown path must end an in-flight install and wait for the
// terminal record, so the daemon's errgroup transitively joins the one
// goroutine the servers themselves do not carry.
func TestStopEngineJoinsTheInFlightInstall(t *testing.T) {
	t.Parallel()
	...
	server := &Server{service: NewService(&ServiceDeps{Engine: engine}), log: slog.New(slog.DiscardHandler)}
	...
	if phase := reply.GetEngine().GetOperation().GetPhase(); phase != enginePhaseError {
		t.Fatalf("operation not joined by the server drain: phase %q", phase)
	}
}
...
func TestStopServersEndsOpenEventStreams(t *testing.T) {
	...
	// Premise: graceful stop alone never returns while the watcher is
	// connected.
	...
	// stopServers, in contrast, gives up on the graceful drain after its
	// budget.
