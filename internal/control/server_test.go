...
// startServer boots the full control plane and waits until it answers.
func startServer(t *testing.T) (base, token string, stop func() error) {
	...
	server := control.NewServer(&control.ServerDeps{
		Config:  cfg,
		Store:   store,
		Service: newTestService(t, store, &stubPusher{}),
		Data:    control.DataPlane{Docs: fakeCaptions{}, Streams: fakeStreams{}, Media: fakeMedia{}},
	...
	go func() { done <- server.RunAfterBind(ctx, nil) }()
...
func waitHealthy(t *testing.T, base string) {
	...
	testkit.Eventually(t, 10*time.Second, func() bool {
		return do(t, http.MethodGet, base+"/healthz", "", "") == http.StatusOK
	}, "server became healthy")
}

// request performs one control-plane call: a non-empty token adds the bearer
// header, json marks the body, and a non-nil out decodes a 200 reply.
func request(t *testing.T, method, url, token, body string, jsonBody bool, out any) int {
	t.Helper()
	...
	if out != nil && reply.StatusCode == http.StatusOK {
...
// do sends one request and returns the status, draining and closing the body;
// a connection error reports 0 so health polling can retry.
func do(t *testing.T, method, url, token, body string) int {
	t.Helper()

	return request(t, method, url, token, body, false, nil)
}
...
func getJSON(t *testing.T, url string, out any) {
	...
	if status := request(t, http.MethodGet, url, "", "", false, out); status != http.StatusOK {
...
func writeJSON(t *testing.T, method, url, token, body string, out any) int {
	...
	return request(t, method, url, token, body, true, out)
}
...
func TestServerServesTheWholeSurface(t *testing.T) {
	...
	assertUnknownFieldsRejected(t, base, token)
...
// assertUnknownFieldsRejected: a misspelled field is a typo, not a request, so
// a mutation route must name it instead of applying the rest of the document.
func assertUnknownFieldsRejected(t *testing.T, base, token string) {
	t.Helper()
	...
	cases := []struct {
		...
		{http.MethodPost, "/api/v1/sessions",
			`{"slug":"typo","profile":"broadcast","sourceUrl":"file:///tmp/x.wav","langs":["it","en"],"subz":"vtt"}`},
		{http.MethodPatch, "/api/v1/sessions/demo", `{"addLangz":["fr"]}`},
		{http.MethodPut, "/api/v1/config", `{"defaults":{"delaySecond":7}}`},
	}

	for _, tc := range cases {
		if got := writeJSON(t, tc.method, base+tc.path, token, tc.body, nil); got != http.StatusBadRequest {
			t.Fatalf("%s %s with an unknown field = %d, want 400", tc.method, tc.path, got)
		}
	}
}

// assertGatewayReads exercises public reads and the token-protected ones.
func assertGatewayReads(t *testing.T, base, token string) {
	...
	for _, path := range []string{"/api/v1/config", "/api/v1/devices", "/api/v1/doctor", "/api/v1/engine"} {
...
// assertSessionLifecycle drives create → list → patch over REST and pins the
// tokenless rejection.
...
// assertDataPlane exercises captions, live audio, the HLS master and the 404.
...
// assertSSEDelivers reads the snapshot the server sends first.
