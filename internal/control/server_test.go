...
// request performs one control-plane call. A non-empty token adds the
// bearer header, json marks the body, and a non-nil out decodes a 200
// reply. Connection failures report status 0 so health polling can retry;
// callers that need a hard failure assert on the status.
...
// assertUnknownFieldsRejected: a misspelled field is a typo, not a request.
// Every mutation route must name it instead of applying the rest of the
// document, the way the session flag map and the config file already do.
