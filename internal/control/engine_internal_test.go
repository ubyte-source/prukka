...
func (f *fakeEngineInstaller) InstallPack(_ context.Context, _ *speech.Catalog, id string, _ speech.Reporter) error {
	if f.gate != nil {
		<-f.gate
	}
	f.mu.Lock()
...
func newTestEngine(t *testing.T, installer EngineInstaller, source CatalogSource) *Engine {
	...
	return NewEngine(installer, source, engineTestHolder(t), nil, nil, slog.New(slog.DiscardHandler))
}
