...
func engineTestCatalog() *speech.Catalog {
	return &speech.Catalog{
		...
		Packs: []speech.Pack{
			...
			{
				ID:     "voice-it",
				Kind:   speech.PackVoice,
				Lang:   "it",
				Voice:  "models/tts/custom-it.onnx",
				URL:    "https://x/v",
				SHA256: "0",
				Size:   1,
			},
...
// TestPerArtifactDoneKeepsTheOperationRunning: the installer reports done once
// per artifact, and only finish may end an operation.
...
// TestShutdownCancelsAndJoinsTheInFlightInstall: Shutdown must reach a blocked
// install through the operation context and return only after the terminal
// phase, then refuse further operations.
