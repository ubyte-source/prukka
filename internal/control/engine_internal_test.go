...
func (f *fakeEngineInstaller) EnsureRuntime(context.Context, *speech.Catalog, speech.Reporter) (bool, error) {
	return true, nil
}

// InstallPack blocks on the gate, when one is set, until the gate closes or
// the operation context ends — the same two exits the real installer has.
func (f *fakeEngineInstaller) InstallPack(ctx context.Context, _ *speech.Catalog, id string, _ speech.Reporter) error {
	if f.gate != nil {
		select {
		case <-f.gate:
		case <-ctx.Done():
...
func newTestEngine(t *testing.T, installer EngineInstaller, source CatalogSource) *Engine {
	...
	return NewEngine(installer, source, engineTestHolder(t), nil, slog.New(slog.DiscardHandler))
}
...
// TestPerArtifactDoneKeepsTheOperationRunning: the installer reports done
// once per artifact, before the operation's config step has run. That
// report must neither free the single operation slot nor reach subscribers
// as a terminal frame — finish alone ends an operation, and only then does
// the slot reopen while the record keeps its terminal phase.
func TestPerArtifactDoneKeepsTheOperationRunning(t *testing.T) {
	t.Parallel()
	...
	engine.Progress(speech.Progress{Phase: speech.PhaseDone, Item: "voice-it"})

	if err := engine.begin(engineOpInstallPack, "mt-it-en"); status.Code(err) != codes.Aborted {
		t.Fatalf("per-artifact done freed the operation slot: %v", err)
	}
	select {
	case event := <-events:
		t.Fatalf("per-artifact done reached subscribers: %+v", event)
	default:
	}
	...
	if phase := reply.GetEngine().GetOperation().GetPhase(); phase != speech.PhaseDone {
		t.Fatalf("terminal record phase = %q, want %q", phase, speech.PhaseDone)
	}

	if err := engine.begin(engineOpInstallPack, "mt-it-en"); err != nil {
		t.Fatalf("slot not released by finish: %v", err)
	}
	engine.finish(engineOpInstallPack, "mt-it-en", nil)
}

// TestShutdownCancelsAndJoinsTheInFlightInstall: the engine owns its
// operation goroutines. Shutdown must reach a blocked install through the
// operation context and return only after the operation reported its
// terminal phase — the join that lets the installer release its on-disk
// lock before the process exits — then refuse further operations.
func TestShutdownCancelsAndJoinsTheInFlightInstall(t *testing.T) {
	t.Parallel()
	...
	event := waitEngineEvent(t, events, enginePhaseError)
	if event.Error == "" {
		t.Fatalf("terminal frame carries no error: %+v", event)
	}
