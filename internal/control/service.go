...
// DoctorFunc runs environment checks for the Doctor RPC.
type DoctorFunc func() []doctor.Check

// Pusher starts an external push of one language's output; subs is the
// overlay mode the session asked for.
type Pusher interface {
	Push(slug string, lang core.Lang, target string, subs session.SubsMode) error
}
...
// DevicesFunc enumerates the machine's media devices for pickers; nil means
// no enumeration on this build.
...
// SessionCapabilityFunc rejects a valid-looking session the configured engine
// cannot execute; nil accepts every definition.
...
// ServiceDeps carries the control service's collaborators.
...
// applyCallInvariants coerces broadcast-tuned options that break a live call:
// no subtitles, and no bed — the sidechain would otherwise release it to full
// volume between takes.
func applyCallInvariants(sess *session.Session) {
	if sess.Profile != session.ProfileCall {
		return
	}

	sess.Subs = session.SubsOff
	sess.Bed = session.Off
}
...
func (s *Service) Push(_ context.Context, req *v1.PushRequest) (*v1.PushResponse, error) {
	...
	if req.GetSubs() != "" {
		if _, subsErr := session.ParseSubs(req.GetSubs()); subsErr != nil {
			return nil, status.Error(codes.InvalidArgument, subsErr.Error())
		}
	}
	...
	subs := session.SubsMode(req.GetSubs())
	if pushErr := s.pusher.Push(req.GetSlug(), target, req.GetTargetUrl(), subs); pushErr != nil {
		if errors.Is(pushErr, core.ErrNotReady) {
			return nil, notReadyStatus(pushErr.Error())
		}
...
func runtimePushStatus(runtime session.RuntimeStatus) error {
	...
	case session.StateStarting, session.StateRunning:
		return nil
	default:
		return notReadyStatus("session runtime is not ready")
	}
}

// PushNotReadyReason tags a retryable not-ready push so clients can tell it
// apart from the transport-level Unavailable a stopped daemon also returns.
const PushNotReadyReason = "MEDIA_NOT_READY"

func notReadyStatus(msg string) error {
	st := status.New(codes.Unavailable, msg)
	if detailed, err := st.WithDetails(&errdetails.ErrorInfo{Reason: PushNotReadyReason}); err == nil {
		st = detailed
	}
...
// ListDevices implements prukka.v1.Control, best-effort: an empty list sends
// pickers back to manual entry.
...
// ListLanguages implements prukka.v1.Control: the one registry, over the wire.
...
// StreamEvents implements prukka.v1.Control until the client disconnects.
