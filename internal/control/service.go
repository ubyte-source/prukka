...
// SessionDefaults seeds fields the caller left unset on a new session. The
// media options are typed because the daemon configuration validated them
...
// ServiceDeps carries the control service's collaborators. Like the package's
// other multi-dependency constructors (httpDeps), a struct keeps each wire
// named at the call site instead of ten positional arguments.
...
// apart from transport-level Unavailable — a stopped daemon also surfaces as
// codes.Unavailable, and retrying that for the full budget would just spin.
