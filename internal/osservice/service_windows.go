...
// taskName identifies the per-user logon task in the scheduler; running as the
// logged-in user keeps the state directory, control pipe and media devices
...
// endThenRun stops any running instance and starts a fresh one, waiting for
// the old daemon's control pipe to vanish in between: with
// MultipleInstancesPolicy IgnoreNew, a /Run issued while the scheduler still
// counts the ending instance is silently dropped.
func endThenRun(ctx context.Context) error {
	// Ending fails when the task is not running; that error only matters if
	// the fresh start also fails.
	endErr := procio.RunQuiet(exec.CommandContext(ctx, "schtasks", "/End", "/TN", taskName))

	for range endSettleAttempts {
		if _, statErr := os.Stat(paths.IPCPath()); errors.Is(statErr, fs.ErrNotExist) {
			break
		}
...
// status reports the task's registration and, since schtasks localizes its
// state column, reads liveness from the daemon's control pipe.
func status(ctx context.Context) (string, error) {
	if !installed(ctx) {
		return "not installed", nil
	}

	if _, statErr := os.Stat(paths.IPCPath()); statErr != nil && errors.Is(statErr, fs.ErrNotExist) {
...
func rendered(opts *Options) (path, content string, err error) {
	...
	// line would vanish.
	args := append(daemonArgs(opts), "--log-file", filepath.Join(paths.StateDir(), "daemon.log"))
