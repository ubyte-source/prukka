...
// fakeLaunchctl plants the shared fake tool under the launchctl name, points
// HOME at a temp dir so plists land there, and removes the retry pacing.
func fakeLaunchctl(t *testing.T, failingVerb string) string {
	t.Helper()

	log := plantFakeTool(t, "launchctl")
...
func TestInstallWritesAgentPlist(t *testing.T) {
	...
	if _, readErr := os.ReadFile(filepath.Clean(log)); !errors.Is(readErr, fs.ErrNotExist) {
		t.Fatalf("launchctl was invoked without --now: %v", readErr)
	}
}

// assertDaemonLogCaptured: the plist must route both streams and the log
...
// TestBootstrapSurfacesPermanentErrorsImmediately: a non-EIO failure must not
// burn the ten-second retry loop.
...
// TestRemoveDeletesPlistAndBootsOut: a second removal is a no-op.
