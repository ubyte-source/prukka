...
// The daemon must run as the logged-in user: the control socket and its token
// file sit in the user's state directory, invisible to a root daemon.
const errRoot = "the macOS service is a per-user launch agent — run `prukka service` commands without sudo"

// rootGuard refuses every service verb under sudo: launchctl would resolve
// gui/$UID to root's empty domain and silently act on the wrong agent.
...
// bootstrapAttempts bounds the bootstrap retries.
const bootstrapAttempts = 20

// bootstrapRetryDelay is a variable so tests do not sleep.
...
func install(ctx context.Context, opts *Options) error {
	...
	// launchd refuses group/world-writable plists.
	...
	// A live instance makes bootstrap fail with "already bootstrapped"; boot
	// it out first. That error only matters if bootstrap also fails.
...
// bootstrap loads the agent, retrying while launchd settles after a bootout;
// a permanent failure surfaces immediately.
...
// transientBootstrap recognizes the EIO launchd reports while a booted-out
// instance is still tearing down, and for several seconds after Background
// Task Management sees the binary change — the only failure worth retrying.
...
// restart relaunches the daemon; a service installed but never bootstrapped
// gets bootstrapped instead.
...
func remove(ctx context.Context) error {
	...
	// Bootout runs even without a plist on disk: a loaded agent whose plist
	// was deleted by hand must still be stopped. It fails when the agent was
	// never bootstrapped, so it is only surfaced if the removal also fails.
	bootoutErr := procio.RunQuiet(exec.CommandContext(ctx, "launchctl", "bootout", agentTarget()))
	...
	if bootoutErr != nil {
		// If launchd can still print the target, deleting the plist did not
		// stop it.
