// Package osservice installs, removes and inspects the per-user service
...
// RestartHint is the command that relaunches the daemon service, spelled with
...
type Options struct {
	...
	ExecPath string
	// ConfigPath optionally pins --config for the daemon; the supervisor has
	// its own working directory, so it must be absolute.
	ConfigPath string
...
// Restart relaunches the daemon service.
