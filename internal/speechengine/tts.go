...
// protocol; raw stdout carries no request boundary in the pinned release.
...
func startPiperCommand(cmd *exec.Cmd, workDir string) (*piperProc, error) {
	...
	proc := &piperProc{
		stdioProc: newStdioProc(cmd, stdin),
...
// after scanning ends: os/exec forbids Wait before StdoutPipe reads complete,
// and Wait closes the read pipe. Sends never block — the protocol is lockstep,
// so a full buffer means an unsolicited line.
