...
// RunTTS serves the text-to-speech stdio protocol over stdin/stdout, resolving
// the Piper voice and phonemizer from the engine bundle.
...
func parseTTSOptions(args []string) (voice string, rate int, err error) {
	...
	flags.StringVar(&voice, nativewire.FlagModel, "", "TTS voice model path")
	flags.IntVar(&rate, nativewire.FlagRate, rate, "PCM sample rate")
...
func synthesizeLine(synth ttsSynthesizer, rate int, line []byte, encoder *json.Encoder, out *bufio.Writer) error {
	...
	if voiceRate != rate {
		pcm = resamplePCM(pcm, voiceRate, rate)
	}
	...
	for len(pcm) > 0 {
		...
		if err := encoder.Encode(nativewire.AudioReply{Audio: base64.StdEncoding.EncodeToString(raw)}); err != nil {
			return err
		}
		pcm = pcm[count:]
	}
...
func writeTTSDone(encoder *json.Encoder, out *bufio.Writer) error {
	if err := encoder.Encode(nativewire.AudioReply{Done: true}); err != nil {
		return err
	}
...
func startPiperProc(dir, voice, workDir string) (*piperProc, error) {
	cmd := exec.CommandContext(context.Background(), filepath.Join(dir, enginebundle.Piper()),
		"--model", voice, "--json-input", "--quiet")
	cmd.Env = libraryEnv(os.Environ(), filepath.Join(dir, enginebundle.PiperDir))
...
func startPiperCommand(cmd *exec.Cmd, workDir string) (*piperProc, error) {
	...
	proc := &piperProc{
		stdioProc: &stdioProc{cmd: cmd, stdin: stdin},
		input:     json.NewEncoder(stdin), acks: acks,
		done: make(chan struct{}), workDir: workDir,
	}
	go proc.pumpAcks(scanner, acks)

	return proc, nil
}

// pumpAcks owns Piper's stdout and is the sole caller of cmd.Wait, strictly
// after scanning ends: Wait closes the read pipe once the child exits, so a
// concurrent Wait could discard an ack Piper wrote just before dying
// (os/exec forbids Wait before StdoutPipe reads complete). Sends never block:
// the protocol is lockstep, so a full buffer means an unsolicited line, and
// dropping it keeps close()'s join on done bounded.
func (p *piperProc) pumpAcks(scanner *bufio.Scanner, acks chan<- piperAck) {
	for scanner.Scan() {
		select {
		case acks <- piperAck{path: scanner.Text()}:
		default:
		}
	}
	if err := scanner.Err(); err != nil {
		select {
		case acks <- piperAck{err: err}:
		default:
		}
	}
	close(acks)
