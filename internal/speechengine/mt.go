...
func RunMT(args []string) (err error) {
	...
	return streamMT(os.Stdin, os.Stdout, proc.translate)
}

// streamMT runs the lockstep one-line-in, one-line-out protocol loop. out must
// be unbuffered: the daemon reads each reply before sending the next request,
// and a buffered writer would deadlock both sides.
func streamMT(in io.Reader, out io.Writer, translate func(string) (string, error)) error {
	scanner := bufio.NewScanner(in)
	scanner.Buffer(make([]byte, 0, 64<<10), 1<<20)
	encoder := json.NewEncoder(out)

	for scanner.Scan() {
		text, err := decodeTextRequest(scanner.Bytes())
		if err != nil {
		...
		if err := encoder.Encode(nativewire.TextLine{Text: translated}); err != nil {
			return err
		}
	}
...
func parseMTOptions(args []string) (from, to string, err error) {
	...
	flags.StringVar(&from, nativewire.FlagFrom, "", "source language")
	flags.StringVar(&to, nativewire.FlagTo, "", "target language")
...
// mtProc is the warm compiled translator: one source line in, one out.
type mtProc struct {
	*stdioProc
...
func startMTProc(dir, modelDir string) (*mtProc, error) {
	cmd := exec.CommandContext(context.Background(), filepath.Join(dir, enginebundle.MT),
		"--model", modelDir,
	...
	return &mtProc{stdioProc: newStdioProc(cmd, stdin), stdout: stdout, out: scanner}, nil
}

// translate sends one source line and reads its translation; blank input is
// answered locally, without perturbing the helper's lockstep.
