...
// streamMT runs the lockstep one-line-in, one-line-out protocol loop. The
// daemon reads each reply before sending the next request, so out must be the
// unbuffered stream: json.Encoder never flushes, and a buffered writer here
