...
func TestPartialAudioContextKeepsTheCallHeadroomRatio(t *testing.T) {
	t.Parallel()

	if got := partialAudioContext(defaultSTTTuning(), false); got != callAudioContext {
		t.Fatalf("default-window partial context = %d, want %d", got, callAudioContext)
	}
	atRatio := defaultSTTTuning()
	atRatio.MaxWindow = callAudioContext * whisperEncoderPosition / 2
	if got := partialAudioContext(atRatio, false); got != callAudioContext {
		t.Fatalf("half-span-window partial context = %d, want %d", got, callAudioContext)
	}
	wide := defaultSTTTuning()
	wide.MaxWindow = atRatio.MaxWindow + time.Millisecond
	if got := partialAudioContext(wide, false); got != 0 {
		t.Fatalf("wide-window partial context = %d, want the full context", got)
	}
	if got := partialAudioContext(defaultSTTTuning(), true); got != 0 {
		t.Fatalf("fast-decode partial context = %d, want the server's own bound", got)
	}
}
...
func TestEnergyEndpointerCutsAfterSpeechAndSilence(t *testing.T) {
	...
	// Exactly the silence hang: the cut lands on the frame that completes it.
...
func TestUnsafeFinalTranscriptIsDroppedWithoutKillingSTTLane(t *testing.T) {
	...
	transcriber := whisperSegmentTranscriber{
		...
		lang:         "it",
		rate:         16000,
		finalTimeout: finalTimeoutFor(defaultSTTTuning()),
	}
...
// callProfileSTTTuning mirrors lane.sttTuning for session.ProfileCall.
...
// newSampleCountingWhisperStub derives each transcript from the posted WAV's
// sample count, so identical segments produce identical lines.
...
// syncLineBuffer collects protocol lines written by inference goroutines.
...
// waitForSTTStreamLines withholds the next chunk until the expected lines
// appeared; a timeout falls through so the closing assertions report which.
...
func newStreamTestTranscriber(
	...
	return &whisperSegmentTranscriber{
		...
		base:         server.URL,
		lang:         "it",
		rate:         sttStreamTestRate,
		fastDecode:   true,
		finalTimeout: finalTimeoutFor(tuning),
	}
}

// TestStreamSTTEmitsPartialsBetweenCallEndpoints requires at least two partials
// per endpoint interval: below two, LocalAgreement-2 commits nothing early.
...
// assertPartialBoundariesAdvance pins the ordering: end_samples never move
// backward, and every partial exceeds the last final.
...
// assertFinalsMatchFinalsOnlyRun replays the identical byte stream with
// finals-only transcription and requires byte-identical final lines.
...
func finalsOnlySTTRun(t *testing.T, server *httptest.Server, stream [][]byte) []string {
	...
	decoder := &s16StreamDecoder{}
	// One io.Pipe write is consumed in 8192-byte reads; the endpointer frames per
	// push, so the replay must deliver the identical read sequence.
...
// TestTranscriptWireShape pins protocol v2: a partial carries "partial", a
// final "text"+"final", and both carry end_samples.
