...
// The daemon's lockstep client blocks on each reply line, so the framing — one
// compact JSON object, one newline, no buffering — is the contract.
...
// The scanner's error tail must surface, not a silent clean EOF.
...
func TestMTProcTranslateProtocol(t *testing.T) {
	...
	proc := &mtProc{stdioProc: newStdioProc(nil, nopWriteCloser{Writer: requests}), out: bufio.NewScanner(reader)}
...
func TestMTProcSkipsBlankInput(t *testing.T) {
	...
	requests := &bytes.Buffer{}
	proc := &mtProc{stdioProc: newStdioProc(nil, nopWriteCloser{Writer: requests})}
...
func TestMTProcBoundsWriteAndResponse(t *testing.T) {
	...
	t.Run("blocked write", func(t *testing.T) {
		...
		proc := &mtProc{stdioProc: newStdioProc(nil, writer)}
		start := time.Now()
	...
	t.Run("missing response", func(t *testing.T) {
		...
		proc := &mtProc{
			stdioProc: newStdioProc(nil, nopWriteCloser{Writer: io.Discard}),
			stdout:    reader,
	...
	t.Run("short write", func(t *testing.T) {
		...
		proc := &mtProc{stdioProc: newStdioProc(nil, shortWriteCloser{})}
