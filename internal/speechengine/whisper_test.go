...
// TestRealWhisperDecodesFinalAfterPartial is an opt-in semantic regression for
// the native bundle boundary. Unit tests pin the safe argv, while this test
// proves a live-window partial decode leaves the actual whisper.cpp server
// clean before the same process decodes the endpointed final.
//
