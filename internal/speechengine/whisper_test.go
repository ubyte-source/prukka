...
func TestWhisperRequestSinglePassDisablesTemperatureFallback(t *testing.T) {
	t.Parallel()

	request, err := whisperRequest(
		t.Context(), "http://127.0.0.1:43123/token", nil,
		whisperInferenceOptions{singlePass: true},
	)
	if err != nil {
		t.Fatal(err)
	}
...
func TestWhisperRequestBoundsTheAudioContextPerRequest(t *testing.T) {
	t.Parallel()

	request, err := whisperRequest(
		t.Context(), "http://127.0.0.1:43123/token", nil,
		whisperInferenceOptions{audioContext: callAudioContext},
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := request.FormValue("audio_ctx"); got != "512" {
		t.Fatalf("audio_ctx = %q, want 512", got)
	}
}

func TestWhisperRequestZeroOptionsKeepServerDefaults(t *testing.T) {
	t.Parallel()

	request, err := whisperRequest(
		t.Context(), "http://127.0.0.1:43123/token", nil, whisperInferenceOptions{},
	)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, field := range []string{"temperature_inc", "audio_ctx"} {
		if _, overridden := request.MultipartForm.Value[field]; overridden {
			t.Fatalf("zero options serialized %s", field)
		}
	}
}
...
func TestWaitReadyReportsExitedProcess(t *testing.T) {
	...
	process := newChildProcess(nil)
	process.waitErr = errors.New("boom")
	close(process.done)
...
// whisperHealthCase is one probe of the readiness endpoint's accept rule.
type whisperHealthCase struct {
	body        string
	contentType string
	name        string
	server      string
...
// whisperHealthCases enumerates what the probe accepts and what it refuses.
func whisperHealthCases() []whisperHealthCase {
	return []whisperHealthCase{
		{
			name:        "ready",
			status:      http.StatusOK,
			server:      whisperServerName,
			contentType: contentTypeJSON,
			body:        whisperHealthReady,
			ready:       true,
		},
		{
			name:        "loading",
		...
		},
		{
			...
			server:      whisperServerName,
			contentType: contentTypeJSON,
			body:        strings.Repeat("x", whisperHealthMaxBytes+1),
			wantErr:     true,
		},
	}
}
...
func TestValidateWhisperHealthIsStrictAndBounded(t *testing.T) {
	...
	for _, test := range whisperHealthCases() {
...
// TestRealWhisperDecodesFinalAfterPartial proves a live-window partial decode
// leaves the real whisper.cpp server clean before the endpointed final. Run
// with PRUKKA_TEST_ENGINE_BUNDLE pointing at a complete engine bundle.
func TestRealWhisperDecodesFinalAfterPartial(t *testing.T) {
	...
	if err := transcriber.partial(segment); err != nil {
		t.Fatalf("live partial inference: %v", err)
	}
	if err := transcriber.transcribe(segment); err != nil {
		t.Fatalf("final inference after partial: %v", err)
	}

	decoder := json.NewDecoder(&output)
	partial := decodeTranscriptLine(t, decoder)
	if partial.Partial == nil || strings.TrimSpace(*partial.Partial) == "" {
		t.Fatal("live partial inference returned no speech")
	}
	final := decodeTranscriptLine(t, decoder)
	if final.Text == nil || strings.TrimSpace(*final.Text) == "" {
		t.Fatal("final inference after partial returned no speech")
	}
	if len(*final.Text) > 512 {
		t.Fatalf("final inference expanded %d bytes from two seconds of speech: %q", len(*final.Text), *final.Text)
	}
	t.Logf("final transcript after partial: %q", *final.Text)
}

func decodeTranscriptLine(t *testing.T, decoder *json.Decoder) nativewire.Transcript {
	t.Helper()
...
func prepareWhisperFixture(t *testing.T, bundle, voice string) (pcm []int16, rate int) {
	...
	if rate != 16000 {
		pcm = resamplePCM(pcm, rate, 16000)
		rate = 16000
	}
