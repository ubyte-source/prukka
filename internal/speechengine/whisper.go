...
const (
	...
	// callAudioContext covers 10.24 s of Whisper mel context, headroom above
	// the 5 s call endpoint.
	callAudioContext = 512
	// whisperEncoderPosition is the source audio one encoder position spans.
	whisperEncoderPosition = 20 * time.Millisecond
	whisperTextMaxBytes    = 4 << 10
)

// errUnsafeWhisperTranscript marks a successful inference whose text must not
// leave the native helper; it is distinct from transport and model failures, so
// one malformed result drops instead of tearing down the STT lane.
...
// newChildProcess adopts a started helper; the caller must run the single
// goroutine that publishes waitErr and closes done.
func newChildProcess(cmd *exec.Cmd) *childProcess {
	c := &childProcess{cmd: cmd, done: make(chan struct{})}
	c.stopOnce = sync.OnceValue(c.retire)
...
const (
	...
	// whisperReadyTimeout is one total startup budget across retries.
...
// racing process can take it; each such loss retries on a fresh port.
...
func whisperServerArgs(
	...
	if fastDecode {
		// A bounded audio context covers the 5 s call endpoint without Whisper's
		// fixed 30 s encoder work, and greedy best-of-one keeps latency
		// deterministic. Keep token timestamps: whisper.cpp's no-timestamps mode
		// with a bounded audio context can poison the next decode after a cancel.
...
func (p *childProcess) stop() error { return p.stopOnce() }

func (p *childProcess) retire() error {
	...
	if killed {
		...
		// POSIX, EINVAL on Windows), so the real status is in waitErr.
		killErr = nil
	}
...
// validateWhisperTranscript is a final fail-closed boundary before text fans
// out into translation and synthesis: a size cap plus the adjacent n-gram loop
// a poisoned decoder produces, which can synthesize minutes of audio from one
// bounded call window.
...
// whisperInferenceOptions are one request's decode overrides; the zero value
// keeps every server default.
type whisperInferenceOptions struct {
	// audioContext bounds this request's encoder positions; zero keeps the
	// server's context.
	audioContext int
	// singlePass disables the temperature-fallback ladder for this request.
	singlePass bool
}
...
func writeWhisperForm(
	...
	if options.audioContext > 0 {
		if fieldErr := form.WriteField("audio_ctx", strconv.Itoa(options.audioContext)); fieldErr != nil {
			return fieldErr
		}
	}
...
func decodeWhisperResponse(resp *http.Response) (text, language string, err error) {
	...
	if declaredJSON || looksLikeObject {
		// Declared or apparent JSON must BE valid JSON: a malformed object is a
		// server fault, never a transcript.
		if !json.Valid(raw) {
...
func probeWhisperHealth(ctx context.Context, client *http.Client, healthURL string) (bool, error) {
	...
	if err != nil {
		// Not bound yet; keep polling. The swallow lives in its own function, or
		// nilerr flags returning a nil error while err is non-nil here.
