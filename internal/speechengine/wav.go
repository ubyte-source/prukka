...
// checkRIFFEnvelope validates the 12-byte RIFF/WAVE header against the
// source's total size: a still-growing file reads as incomplete, a
// mismatched declared size as corrupt.
...
// resampleTaps is the length of the kernel evaluated at every output instant.
// 63 taps of a Blackman-windowed sinc place the first fold frequency deep in
// the stopband; fewer taps widen the transition band back into the speech
// range the kernel exists to protect.
const resampleTaps = 63

// resampleCenter is the tap the kernel is centered on, so the constant group
// delay a symmetric FIR introduces is removed rather than heard as a shift.
...
// the input timeline. Band-limiting and re-spacing MUST be the same operation.
// A low-pass followed by a 2-tap linear interpolator looks equivalent and is
// not: linear interpolation reconstructs a piecewise-linear signal whose first
// image at from-freq survives only ~13 dB down, and re-sampling that
// reconstruction folds the image into the audible band. The energy doing the
// damage never leaves the low-pass PASSBAND, so no amount of stopband
// attenuation touches it — 7 kHz of a 22.05 kHz Piper take lands at 950 Hz and
// 6 kHz at 50 Hz, the buzz under every /s/, /ʃ/ and /f/. The kernel runs once
// per take, against a Piper inference of hundreds of milliseconds.
func resamplePCM(in []int16, from, to int) []int16 {
	...
	// The output instant is carried as base + phase/phases in exact integers
	// rather than as a float position, so a long take cannot accumulate drift.
	base, phase := 0, 0
	for i := range out {
...
// rational, so reducing it by its GCD makes the set exact and finite —
// 22050/16000 reduces to 441/320, so 320 rows cover every instant the output
// can land on and no phase is ever approximated.
func newResampleKernel(from, to int) resampleKernel {
	...
	// The corner is the anti-alias filter's, in INPUT cycles per sample: this
	// kernel ABSORBS that filter rather than replacing it, so a rate reduction
	// must still band-limit to the target's Nyquist, not to the source's.
...
func (k resampleKernel) at(in []int16, base, phase int) int16 {
	...
	for tap, weight := range k.row(phase) {
		// Hold the edge samples: a synthesized take opens and closes in
		// silence, so extension cannot manufacture a transient.
...
// tapered by a Blackman window whose ~74 dB stopband is what turns a fold into
// an attenuation. Two details carry the result:
//
// The window is centered on the same fractional instant as the sinc, not on
// the tap indices. Tapering around the integer center instead leaves the taper
// asymmetric about the instant being reconstructed, which costs 37 dB of image
// rejection (-46 dB instead of -110 dB at 22.05→16 kHz).
//
// Each row is normalized on its OWN sum, which varies with frac. One shared
// normalizer would trade the aliasing for a gain that changes from output
// sample to output sample — heard as modulation instead of as a fold.
func fillSincRow(row []float64, cutoff, frac float64) {
	...
	for tap := range row {
		...
		// The taper spans one tap more than the row, putting its zeros just
		// outside the outermost taps: a fractional shift of up to a whole
		// sample can then never push a tap past a zero into a hard edge.
...
func encodeWAV(pcm []int16, rate int) []byte {
	...
	// Encode the samples straight into the payload region: one allocation, no
	// intermediate copy on the per-inference STT path.
	for i, v := range pcm {
