...
// TestSynthesizeLineBoundsTheSynthesizedText: the cap on the TTS side is the
// last bound before Piper, and the STT cap does not imply it — translation
// sits between them and expands text. An oversized clause must fail before
// the synthesizer is entered, or one request occupies it for the length of
// its take and buries live speech behind stale playout.
...
// fakePiperExitAfterAck makes the fake helper exit cleanly right after
// acknowledging the request that carries it, modeling a Piper that dies with
// its last ack already on the wire.
...
// A dying Piper's last acknowledgement must not be lost: the stdout pump is
// the sole caller of cmd.Wait and calls it only after scanning ends, so an
// ack written just before exit is delivered instead of surfacing as EOF.
func TestPiperAckWrittenBeforeExitIsDelivered(t *testing.T) {
	...
	closed = true
	// close joined the pump, so the channel is already closed; a clean exit
	// must not have manufactured a trailing error acknowledgement.
...
// unsolicited output lines are dropped, keeping shutdown bounded even against
// a misbehaving Piper.
