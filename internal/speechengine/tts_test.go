...
// TestSynthesizeLineBoundsTheSynthesizedText: the STT cap does not imply this
// one — translation sits between them and expands text.
func TestSynthesizeLineBoundsTheSynthesizedText(t *testing.T) {
	t.Parallel()

	synth := &fakeTTSSynth{pcm: []int16{0, 1000}, rate: 16000}
	request, err := json.Marshal(map[string]string{"text": strings.Repeat("x", ttsTextMaxBytes+1)})
	if err != nil {
	...
	err = synthesizeLine(synth, 16000, request, json.NewEncoder(writer), writer)
	if err == nil || !strings.Contains(err.Error(), "tts: text exceeds") {
		t.Fatalf("synthesizeLine error = %v, want the text cap named", err)
	}
	if synth.text != "" {
		t.Fatalf("synthesizer received %d bytes, want an untouched synthesizer", len(synth.text))
	}
}
...
func TestPiperRequestUsesJSONLAndValidatesAcknowledgement(t *testing.T) {
	...
	close(acks)
	proc := &piperProc{
		stdioProc: newStdioProc(nil, nil),
	...
	close(wrong)
	mismatch := &piperProc{stdioProc: newStdioProc(nil, nil), input: json.NewEncoder(io.Discard), acks: wrong}
...
func TestPiperRequestTimeoutUnblocksAStalledWrite(t *testing.T) {
	...
	proc := &piperProc{stdioProc: newStdioProc(nil, writer), input: json.NewEncoder(writer), acks: acks}
...
// fakePiperExitAfterAck makes the fake helper exit right after acknowledging
// the request that carries it.
const fakePiperExitAfterAck = "__fake_piper_exit_after_ack__"

func startFakePiperProcess(t *testing.T, root, workDir string, extraEnv ...string) *piperProc {
	t.Helper()
...
// symlinkTestExecutable exposes the running test binary under a fixed name. A
// symlink, not a hardlink: it is an independent filesystem object, so t.TempDir
// cleanup can unlink it on Windows while the target still runs; a hardlink
// shares the running image and is refused with "Access is denied".
...
// A dying Piper's last acknowledgement must not be lost: the stdout pump calls
// cmd.Wait only after scanning ends, so the ack is delivered, not EOF.
func TestPiperAckWrittenBeforeExitIsDelivered(t *testing.T) {
	root := t.TempDir()
	workDir := filepath.Join(root, "audio")
	if err := os.Mkdir(workDir, 0o700); err != nil {
		t.Fatal(err)
	...
	pcm, rate, err := proc.synthesize(fakePiperExitAfterAck)
	if err != nil || rate != 22050 || !reflect.DeepEqual(pcm, []int16{1, -1}) {
		t.Fatalf("synthesize before helper exit = %v, %d, %v", pcm, rate, err)
	}
	if closeErr := proc.close(); closeErr != nil {
		t.Fatalf("close after helper exit: %v", closeErr)
	}
	closed = true
	// close joined the pump, so the channel is already closed.
	if ack, ok := <-proc.acks; ok {
		t.Fatalf("stray acknowledgement after clean shutdown: %+v", ack)
	}
}

// close joins the stdout pump, so the pump must never park on the ack buffer:
// unsolicited lines are dropped, keeping shutdown bounded.
func TestPiperCloseStaysBoundedDespiteUnsolicitedOutput(t *testing.T) {
	root := t.TempDir()
	workDir := filepath.Join(root, "audio")
	if err := os.Mkdir(workDir, 0o700); err != nil {
		t.Fatal(err)
	}
	proc := startFakePiperProcess(t, root, workDir, "PRUKKA_FAKE_PIPER_NOISY=1")
	...
	case closeErr := <-closeDone:
		if closeErr != nil {
			t.Fatalf("close with a noisy helper: %v", closeErr)
		}
	case <-time.After(piperStopGrace + 3*time.Second):
		t.Fatal("close never returned; the ack pump is parked on an unsolicited line")
	}
}
...
func runFakePiper() (err error) {
	...
	if os.Getenv("PRUKKA_FAKE_PIPER_NOISY") == "1" {
		for _, line := range []string{"unsolicited line one", "unsolicited line two"} {
...
func serveFakePiper(root *os.Root) error {
	...
	for input.Scan() {
		stop, err := serveFakePiperRequest(root, input.Bytes())
		if err != nil {
			return err
		}
		if stop {
...
func serveFakePiperRequest(root *os.Root, line []byte) (bool, error) {
	var request struct {
	...
	if request.Text == "" || request.OutputFile == "" {
		return false, errors.New("fake Piper received an incomplete request")
	}
	...
	if _, err := fmt.Fprintln(os.Stdout, request.OutputFile); err != nil {
		return false, err
	}

	return request.Text == fakePiperExitAfterAck, nil
}
