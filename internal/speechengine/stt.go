...
func (t *whisperSegmentTranscriber) decode(
	...
	// whisper marks near-silent windows with bracketed non-speech tokens
	// ("[BLANK_AUDIO]", "(music)"); a window of nothing but markers must not
	// enter the stream as speech.
...
// non-speech markers ("[BLANK_AUDIO]", "(music) (applause)"): every balanced
// [...] or (...) run is dropped and the remainder decides. The strip is only
// a decision aid — markers mixed with speech keep the text intact, because a
// window may open AND close on stings around a real sentence.
