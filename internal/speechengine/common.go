...
const (
	...
	// languageAuto asks whisper to detect the language, and marks a hint unset.
	languageAuto = "auto"
)
...
// engineDir is the directory holding the bundled tools and models:
// PRUKKA_ENGINE_ROOT when set, otherwise this executable's directory.
func engineDir() string {
	if root := strings.TrimSpace(os.Getenv(nativewire.EngineRootEnv)); root != "" {
		return root
	}
...
// only "auto" or a bare ISO-639-1 code, so "it-CH" must reach it as "it".
