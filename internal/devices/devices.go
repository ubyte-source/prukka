...
// missingFilesNextStep is the remediation when a marker outlived the files it
// records.
const missingFilesNextStep = "installed files are gone — run: prukka devices install"
...
// Install puts the embedded drivers in place, skipping those already current.
...
// Status reports each device against the bundled payloads and the recorded
// installs.
func Status(ctx context.Context) ([]Result, error) {
	return status(ctx)
}

// executable names the running binary for user-facing commands: PATH rarely
// carries it right after install, and sudo resets PATH anyway.
...
func removeAt(kind Kind, path string) (Result, error) {
	...
	if recordedSum(kind) == "" && errors.Is(statErr, fs.ErrNotExist) {
...
func clearInstallRecords() error {
	...
	if err := os.Remove(parent); err != nil && !errors.Is(err, fs.ErrNotExist) {
