...
// moduleStatus classifies one device: the marker says what was built, and the
// extra tree says whether the module survived to the next boot. An unbundled
// build has no sources and therefore no kernel to check against — asking
// uname there would add an exec and a hard-error path to every doctor run —
// so it stays marker-only.
...
// current reports whether every module already carries this digest and its
// built module is still in the running kernel's extra tree. The marker alone
// is not enough: one that outlived its files would skip the rebuild and leave
// the machine with no devices and no next step. root is a seam for tests.
...
// kernel's extra tree. Only this kernel counts: a .ko surviving under an
// older kernel is exactly what the marker's kernel coupling exists to catch.
...
// the install writes it and the skip gate looks for it there.
