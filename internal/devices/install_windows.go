...
// webcamNextStep words the one action Windows needs; the controller is
// resident, so the camera lives only while it runs.
...
// uninstallWebcam lets the controller remove the live Media Foundation camera
// and its COM registration before staged files disappear; the registry
// fallback repairs installs whose controller vanished.
func uninstallWebcam(ctx context.Context) error {
	...
	if err := os.Remove(dll); err != nil && !errors.Is(err, fs.ErrNotExist) {
...
// status reports the staged camera against the bundled payload; the audio
// devices always read manual.
func status(context.Context) ([]Result, error) {
	...
	if webcam.State == StateInstalled {
		if _, statErr := os.Stat(filepath.Join(webcamDir(), webcamCtl)); statErr != nil {
			webcam = Result{Kind: Webcam, State: StateMissing, NextStep: missingFilesNextStep}
		}
	}
