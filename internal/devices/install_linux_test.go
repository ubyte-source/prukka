...
// plantModules writes one built module per kind under a kernel's extra tree.
...
// TestCurrentRequiresTheBuiltModules: a marker that outlived its .ko must
// make the set stale, or install skips the rebuild and the machine is left
// with markers, no devices and nothing to run.
...
// TestStatusReportsAModuleWhoseFileIsGone: status must not report a device
// whose module was pruned as installed — doctor folds that verdict into an
// all-clear and the user gets no next step.
