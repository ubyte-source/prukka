...
func testArchive(t *testing.T, name, body string) []byte {
	...
	header := &tar.Header{Name: name + "/bin", Typeflag: tar.TypeReg, Mode: 0o755, Size: int64(len(body))}
...
func TestExtractRejectsEscapingEntries(t *testing.T) {
	...
	err := extract(buf.Bytes(), t.TempDir())
...
func TestExtractRejectsArchiveBomb(t *testing.T) {
	...
	if err := extract(buf.Bytes(), t.TempDir()); err == nil || !strings.Contains(err.Error(), "entries") {
...
func TestExtractRejectsDuplicateEntries(t *testing.T) {
	...
	if err := extract(buf.Bytes(), t.TempDir()); err == nil || !strings.Contains(err.Error(), "duplicate") {
...
func linkArchive(t *testing.T, name, link, followedFile string) []byte {
	...
	if err := tw.WriteHeader(&tar.Header{Name: name, Typeflag: tar.TypeSymlink, Mode: 0o777, Linkname: link}); err != nil {
