...
// extension-approval flow. NextStep spells the path — a root or SSH
// install could not raise the GUI prompt from here anyway.
...
// webcamResult overlays the signing gap on a marker-derived webcam state,
// so install's verdict and status's verdict come from one place: the app
// bundle being in place proves nothing about activation, so installed
// reads manual and a skipped re-install repeats the step instead of going
// silent. Missing and outdated pass through, keeping their remediation.
...
// status reports each device against the bundled payloads; the webcam
// verdict flows through the same overlay install uses, so both verbs
// change together when signing lands.
func status(context.Context) ([]Result, error) {
	return statusIn(halDir, appsDir)
}

// statusIn reports status against explicit install roots, which is what the
// tests can write to; production always passes the system directories.
...
// deviceStatus classifies one device: the marker says what was installed,
// the destination says whether it survived — the same pair installPayload's
// own skip gate compares. The presence check runs before the webcam overlay,
// so a deleted app bundle reads missing with a repair step instead of the
// signing note for an extension that is no longer on disk.
