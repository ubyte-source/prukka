...
// staged files; PRUKKA_STATE overrides it for tests.
...
// programDataRoot is <ProgramData>\Prukka, the machine-wide home shared with
// the elevated webcam installer that copies the DLL there.
...
// recordedSum reads the digest stored at install time, or "" if never
// installed.
...
func dropMarker(kind Kind) error {
	if err := os.Remove(markerPath(kind)); err != nil && !errors.Is(err, fs.ErrNotExist) {
...
// markerState classifies a kind against the marker an install would record
// now; an empty want (unbundled build) trusts any recorded install.
...
// expectedMarker is empty when the build carries no payload.
...
// Recorded reports whether any device driver was installed on this machine.
func Recorded() bool {
