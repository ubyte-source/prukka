...
// AVFoundation capture; everything else keeps one-attempt semantics.
func deviceStartupRetryEnabled(goos, source string) bool {
	return goos == hostos.Darwin && deviceurl.Is(source)
}
...
func newStartupRetryFrames(
	...
	f := &startupRetryFrames{
		ctx:     ctx,
		cancel:  cancel,
		open:    open,
		policy:  policy,
		current: newFrameAttempt(initial, nil),
	}
	f.closeOnce = sync.OnceValue(f.retire)
...
// nextAttempt bounds only the first media read: a live AVFoundation process can
// survive device negotiation without ever writing PCM, leaving no process error
// for the ordinary startup classifier to inspect.
...
func (f *startupRetryFrames) Close() error { return f.closeOnce() }

// retire stops every retry path, then closes whichever attempt is published.
func (f *startupRetryFrames) retire() error {
	f.cancel()
	f.mu.Lock()
	f.closed = true
	attempt := f.current
...
func newFrameAttempt(frames core.Frames, cancel context.CancelFunc) *frameAttempt {
	a := &frameAttempt{frames: frames, cancel: cancel}
	a.closeOnce = sync.OnceValue(a.retire)

	return a
}

func (a *frameAttempt) close() error { return a.closeOnce() }

// retire cancels the attempt's process context and closes whatever it opened;
// a failed open leaves either or both absent.
func (a *frameAttempt) retire() error {
	if a.cancel != nil {
		a.cancel()
	}
	if a.frames == nil {
