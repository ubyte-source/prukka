...
// native capture fragment; network and file sources are unaffected.
...
// Open implements core.Ingress; a non-empty VideoDir adds the passthrough HLS
// video rendition alongside the PCM tap.
...
// Next implements core.Frames; PTS is the amount of audio delivered so far.
...
func (f *frames) deferReadError(n int, readErr error) int {
	if n%2 != 0 {
		...
		// A Reader may return data and a terminal error together.
...
// closeForCancel is the cancellation entry point; Close returns the same
// cached result.
func (f *frames) closeForCancel() { besteffort.Ignore(f.closeOnce()) }

// release interrupts a blocked pipe read and reaps the supervised source.
...
// Close is safe for Next cancellation and the engine owner to call
// concurrently.
