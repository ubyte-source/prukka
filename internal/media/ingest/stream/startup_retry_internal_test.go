...
func (f *deadlineProbeFrames) Close() error { return nil }
...
func TestDeviceStartupRetryIsDarwinDeviceOnly(t *testing.T) {
	...
	cases := []struct {
		...
		{goos: hostos.Darwin, source: "device://audio/1", want: true},
		{goos: hostos.Darwin, source: "device://av/0|1", want: true},
		{goos: hostos.Darwin, source: "file:///tmp/audio.aiff", want: false},
		{goos: hostos.Darwin, source: "rtmp://127.0.0.1/live", want: false},
...
func TestStartupRetryCloseRunsOnceAndCachesItsResult(t *testing.T) {
	t.Parallel()

	want := errors.New("attempt close failed")
	initial := newBlockingRetryFrames(want)
	frames := newStartupRetryFrames(
		t.Context(), initial, func(context.Context) (core.Frames, error) {
			return nil, errors.New("no reopen expected")
		}, immediateRetryPolicy(1, io.ErrUnexpectedEOF),
	)
	...
	if got := initial.closes.Load(); got != 1 {
		t.Fatalf("attempt closed %d times, want exactly once", got)
	}
}

func TestFrameAttemptCloseRunsOnceAndCachesItsResult(t *testing.T) {
	t.Parallel()

	want := errors.New("abandoned attempt close failed")
	source := newBlockingRetryFrames(want)
	canceled := 0
	attempt := newFrameAttempt(source, func() { canceled++ })

	first, second := attempt.close(), attempt.close()
	if !errors.Is(first, want) || !errors.Is(second, want) {
		t.Fatalf("close results = %v / %v, want the first result cached", first, second)
	}
	if got := source.closes.Load(); got != 1 || canceled != 1 {
		t.Fatalf("underlying closes = %d, cancels = %d, want exactly one of each", got, canceled)
	}
}

func TestFrameAttemptCloseToleratesAnEmptyAttempt(t *testing.T) {
	t.Parallel()

	if err := newFrameAttempt(nil, nil).close(); err != nil {
		t.Fatalf("close of an empty attempt = %v, want nil", err)
	}
}
