...
// WithPCMQuantum sets the amount of decoded audio returned by each Frames.Next
// call; it must be positive and a whole number of reference-rate samples.
...
func (i Ingress) Open(ctx context.Context, src core.SourceSpec) (core.Frames, error) {
	...
	info, err := input.Stat()
	if err != nil {
		return nil, closeInput(input, fmt.Errorf("file ingress: stat: %w", err))
	}
	if !info.Mode().IsRegular() {
		return nil, closeInput(input, errors.New("file ingress: source is not a regular file"))
	}

	spec, err := inspectWAV(input, info.Size())
	if err != nil {
		return nil, closeInput(input, fmt.Errorf("file ingress: %w", err))
	}

	return newFrames(input, spec, loop, i.quantumSamples), nil
}

// newFrames wires the reader over an inspected WAV handle it now owns.
func newFrames(input *os.File, spec wavSpec, loop bool, quantumSamples int) *frames {
	f := &frames{
		input:      input,
	...
	f.closeOnce = sync.OnceValue(f.release)
...
func sourcePath(rawURL string) (string, url.Values, error) {
	...
	if !ok {
		return "", nil, fmt.Errorf("file ingress: unsupported source %s", redact.URL(rawURL))
	}
...
func (f *frames) Next(ctx context.Context) (core.PCM, error) {
	...
	if f.pos >= f.dataBytes {
		...
		// Looping keeps PTS monotonic: the file restarts, the clock does not.
		f.pos = 0
	}
...
// Close releases the WAV handle; it may be called repeatedly and races
// cancellation cleanup safely.
func (f *frames) Close() error { return f.closeOnce() }

// release retires the handle and stops Next from reading past it.
func (f *frames) release() error {
	f.closed.Store(true)
