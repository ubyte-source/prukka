...
func TestFramesDeliverPacedPCMUntilEOF(t *testing.T) {
	...
	path := writeWAV(t, core.SampleRate, core.SampleRate/2)
	...
	if total != core.SampleRate/2 || chunks != 5 {
		t.Fatalf("delivered %d samples in %d chunks, want %d in 5", total, chunks, core.SampleRate/2)
	}
...
func TestPCMQuantumControlsDeliverySizeAndPTS(t *testing.T) {
	...
	path := writeWAV(t, core.SampleRate, core.SampleRate/25)
	...
	if len(first.Data) != core.SampleRate/50 || first.PTS != 0 {
	...
	if len(second.Data) != core.SampleRate/50 || second.PTS != quantum {
...
func TestPCMQuantumRejectsInvalidDurations(t *testing.T) {
	...
	for name, quantum := range map[string]time.Duration{
		...
		"not aligned": time.Second/core.SampleRate + time.Nanosecond,
		"zero":        0,
	} {
...
func drain(t *testing.T, frames core.Frames) (total, chunks int) {
	...
	for {
		...
		if chunk.Rate != core.SampleRate || chunk.Ch != 1 {
			t.Fatalf("chunk format = %d Hz × %d ch", chunk.Rate, chunk.Ch)
		}

		if want := time.Duration(total) * time.Second / core.SampleRate; chunk.PTS != want {
...
func TestLoopKeepsPTSMonotonic(t *testing.T) {
	...
	path := writeWAV(t, core.SampleRate, core.SampleRate/10)
...
func TestLoopRejectsEmptyInput(t *testing.T) {
	...
	path := writeWAV(t, core.SampleRate, 0)
...
func TestShortLoopAlwaysAdvances(t *testing.T) {
	...
	path := writeWAV(t, core.SampleRate, 1)
	frames, err := file.New().Open(t.Context(), core.SourceSpec{URL: "file://" + path + "?loop=true"})
	if err != nil {
		t.Fatalf("Open returned error: %v", err)
	}
...
func TestNextHonorsCancellation(t *testing.T) {
	...
	path := writeWAV(t, core.SampleRate, core.SampleRate)
...
func TestCloseRunsOnceAndEveryCallerReadsItsResult(t *testing.T) {
	t.Parallel()

	frames, err := file.New().Open(t.Context(), core.SourceSpec{URL: "file://" + writeWAV(t, 16000, 160)})
	if err != nil {
		t.Fatalf("open: %v", err)
	}
	...
	for range closers {
		if closeErr := <-results; closeErr != nil {
			t.Fatalf("concurrent Close = %v, want the one clean cached result", closeErr)
		}
	}

	if _, nextErr := frames.Next(t.Context()); !errors.Is(nextErr, io.EOF) {
		t.Fatalf("Next after Close = %v, want io.EOF", nextErr)
	}
}

// TestOpenErrorsNameNoLocalPath pins that a rejected file source is named by
// its scheme alone: the local path never reaches the session status.
func TestOpenErrorsNameNoLocalPath(t *testing.T) {
	t.Parallel()

	dir := t.TempDir()
	notWAV := filepath.Join(dir, "Alice Private Recording.wav")
	if err := os.WriteFile(notWAV, []byte("not a riff file at all"), 0o600); err != nil {
		t.Fatalf("write: %v", err)
	}

	for _, raw := range []string{"file://" + notWAV, "https://alice.example/private.wav"} {
		_, err := file.New().Open(t.Context(), core.SourceSpec{URL: raw})
		if err == nil {
			t.Fatalf("Open(%q) accepted an unusable source", raw)
		}
		for _, secret := range []string{dir, "Alice Private Recording", "private.wav"} {
			if strings.Contains(err.Error(), secret) {
				t.Fatalf("Open(%q) error exposes %q: %v", raw, secret, err)
			}
		}
	}
}
