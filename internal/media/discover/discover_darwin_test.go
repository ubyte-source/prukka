...
// list — playback enumeration is native. (Enumeration cannot fail by
// contract, hence no error to assert on.)
...
// TestOutputSnapshotUniqueResolvesOnlyUnambiguousLabels: duplicate-label
// rejection, pinned on a synthetic snapshot so it holds on hosts whose real
// hardware never has two devices sharing a name — a regression here would
// silently route a dub into the wrong speaker.
...
// TestOutputSnapshotDevicesLabelsAndFlags: a unique label carries the
// rebinding hint, duplicates keep the bare positional URL, a Prukka UID
// marks the virtual loopback, and a nil snapshot (no inventory yet) lists
// nothing.
