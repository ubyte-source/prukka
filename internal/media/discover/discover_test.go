...
// TestListingBudgetBoundsAWedgedListing: one wedged ffmpeg listing must fail
// on its own budget instead of starving the sibling probes — the discipline
// the darwin leg always had (a pending camera-consent decision wedges the
// avfoundation listing) and every leg must inherit.
func TestListingBudgetBoundsAWedgedListing(t *testing.T) {
	t.Parallel()

	sleepBin, err := exec.LookPath("sleep")
	if err != nil {
		t.Skip("no sleep binary to wedge a listing with")
	}

	start := time.Now()
	_, listErr := listCaptureRaw(t.Context(), sleepBin, "30")
	if elapsed := time.Since(start); elapsed >= 4*captureListBudget {
		t.Fatalf("wedged listing held its caller %v; the %v budget must end it", elapsed, captureListBudget)
	}
	if listErr == nil {
		t.Fatal("a listing ended by its budget must report the failure, not fake an empty inventory")
	}
}
