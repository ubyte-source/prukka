...
func (c *LiveCue) write(text string) {
	tmp, err := os.CreateTemp(filepath.Dir(c.path), "cue-*")
	if err != nil {
		c.log.Debug("overlay scratch", "path", c.path, "err", err)
	...
	if writeErr != nil || closeErr != nil {
		c.log.Debug("overlay write", "path", c.path, "write", writeErr, "close", closeErr)

		return
	}

	if renameErr := os.Rename(tmp.Name(), c.path); renameErr != nil {
		c.log.Debug("overlay rename", "path", c.path, "err", renameErr)
	}
}
