...
func (s *Store) Open(slug, rest string) (io.ReadSeekCloser, bool) {
	...
	case len(parts) == 2 && parts[0] == videoDir:
		path, valid = rebuild(tree.dir, videoDir, "", parts[1], segTS)
	case len(parts) == 3 && parts[0] == audioDir:
		path, valid = rebuild(tree.dir, audioDir, tree.owned(parts[1]), parts[2], segTS)
	case len(parts) == 3 && parts[0] == subsDir:
		path, valid = rebuild(tree.dir, subsDir, tree.ownedSubtitle(parts[1]), parts[2], segVTT)
	}
...
func (t *Tree) ownedSubtitle(tag string) string {
...
// unknown marker.
func (t *Tree) owned(tag string) string {
