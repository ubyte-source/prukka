...
// newSegmenter builds a segmenter through the store, with its on-disk dir.
func newSegmenter(t *testing.T) (segmenter *hls.Segmenter, dir string) {
	t.Helper()

	store := newStore(t)

	tree, err := store.Create("demo", []core.Lang{"en"})
	if err != nil {
		t.Fatalf("Create: %v", err)
	}

	return tree.Subtitles("en"), filepath.Join(filepath.Dir(tree.VideoDir()), "subs", "en")
}
...
func TestWindowRollsAndEvicts(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "seg00000.vtt")); !errors.Is(err, fs.ErrNotExist) {
...
func TestWindowEvictsCuelessGapParts(t *testing.T) {
	t.Parallel()

	segmenter, dir := newSegmenter(t)
	segmenter.Append(seg("First late cue.", 62*time.Second, time.Second))

	gap := filepath.Join(dir, "seg00008.vtt")
	if _, err := os.Stat(gap); err != nil {
		t.Fatalf("initial gap part not written: %v", err)
	}

	segmenter.Append(seg("Later cue.", 122*time.Second, time.Second))

	if _, err := os.Stat(gap); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("evicted gap part still on disk: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, "seg00018.vtt")); err != nil {
		t.Fatalf("current gap part not written: %v", err)
	}

	playlistBody, err := os.ReadFile(filepath.Clean(filepath.Join(dir, "index.m3u8")))
	if err != nil {
		t.Fatalf("playlist: %v", err)
	}
	playlist := string(playlistBody)
	if strings.Contains(playlist, "seg00008.vtt") {
		t.Fatalf("playlist retained an evicted gap part:\n%s", playlist)
	}
	if !strings.Contains(playlist, "#EXT-X-MEDIA-SEQUENCE:16") {
		t.Fatalf("media sequence did not advance:\n%s", playlist)
	}
}

func TestPartEscapesCueMetacharacters(t *testing.T) {
	t.Parallel()

	segmenter, dir := newSegmenter(t)

	// Both sinks must escape WebVTT cue metacharacters or a conformant parser
	// mangles the cue.
	segmenter.Append(seg("Tom & <Jerry>", 8*time.Second, 2*time.Second))
	...
	if !strings.Contains(text, "Tom &amp; &lt;Jerry&gt;") {
		t.Errorf("cue metacharacters not escaped:\n%s", text)
	}
	if strings.Contains(text, "<Jerry>") || strings.Contains(text, "Tom & ") {
		t.Errorf("raw metacharacters leaked into the rendition:\n%s", text)
	}
}
...
func TestEmptyTextIsIgnored(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, "index.m3u8")); !errors.Is(err, fs.ErrNotExist) {
