...
func (s *Segmenter) Append(seg *core.TranslatedSegment) {
	...
	for idx := range touched {
		if err := s.writePart(idx); err != nil {
...
func (s *Segmenter) writePart(idx int) error {
	...
	for i := range s.cues[idx] {
		...
		b.WriteString("\n")
		b.WriteString(vtt.CueText(cue))
		b.WriteString("\n")
	}

	if err := writeAtomic(s.partPath(idx), []byte(b.String())); err != nil {
		return err
	}
...
func (s *Segmenter) writePlaylist() {
	first := max(s.top-windowSegments+1, 0)
	...
	if err := writeAtomic(filepath.Join(s.dir, playlist), []byte(b.String())); err != nil {
...
func (s *Segmenter) evict() {
	...
	for idx := range s.parts {
		if idx < first {
			...
			if err := os.Remove(s.partPath(idx)); err != nil && !errors.Is(err, fs.ErrNotExist) {
...
// partName renders the package's segment filename grammar (resolve.go owns
// it), so the writer and the request resolver cannot drift apart.
func partName(idx int) string {
	return fmt.Sprintf(segVTT, idx)
}
