...
type Registry struct {
	...
	// relaunches tracks the route replays Create schedules, so a teardown
	// can drain them instead of racing their process starts.
	relaunches sync.WaitGroup
}
...
func (r *Registry) Create(
	...
	// The session gate bounds the replay; Reset drains it before teardown.
	gateCtx := r.gates[session].ctx
	r.relaunches.Go(func() {
...
func (r *Registry) Reset(session string) {
	// Drain scheduled route replays BEFORE taking startMu: a replay launching
	// a push blocks on startMu, so waiting under the lock would deadlock.
	r.relaunches.Wait()
