...
// TestDropSparesThePrefixSharingSibling: session slugs may share a prefix —
// "cast" and "cast-2" are both legal — and the ownership predicates separate
// them only because they match on "session/". Dropping one must leave the
// other streaming.
func TestDropSparesThePrefixSharingSibling(t *testing.T) {
	t.Parallel()

	log := slog.New(slog.DiscardHandler)
	// A supervisor over a missing binary keeps ServeTS a pair-existence probe:
	// with none at all it answers false for every pair, sibling or not.
	sup := ffmpeg.NewSupervisor(filepath.Join(t.TempDir(), "missing-ffmpeg"), log)
	r := audio.NewRegistry(t.Context(), sup, nil, log)
	r.Create("cast", "en", mixer())
	r.Create("cast-2", "en", mixer())

	r.Drop("cast")

	if r.ServeTS(t.Context(), io.Discard, "cast", "en") {
		t.Fatal("dropped pair still served")
	}
	if !r.ServeTS(t.Context(), io.Discard, "cast-2", "en") {
		t.Fatal(`dropping "cast" tore down its prefix-sharing sibling "cast-2"`)
	}
}
...
func TestStartHLSWritesARollingRendition(t *testing.T) {
	...
	testkit.Eventually(t, 20*time.Second, func() bool {
		_, err := os.Stat(dir + "/index.m3u8")

		return err == nil
	}, "HLS playlist never appeared")
}
