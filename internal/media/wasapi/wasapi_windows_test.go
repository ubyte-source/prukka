...
// TestWriteAfterComThreadFailureReturnsTheRealError: once the COM thread has
// died on its own, Write must deterministically surface the queued failure —
// never a bare io.ErrClosedPipe and never success — and only after the one
// real error is consumed do further writes degrade to the closed-pipe shape.
