...
//nolint:gosec // G103/G115: COM interop; COM interop pointer arithmetic; audited, maintainer-approved.
package wasapi
...
// open runs one endpoint id on its own COM thread and hands back the sink
// that feeds it.
func open(id string, config openConfig) (io.WriteCloser, error) {
	w := &writer{
	...
	go w.run(id, config.bufferDuration, ready)
...
func awaitOpen(w *writer, ready <-chan openResult) (io.WriteCloser, error) {
	result := <-ready
	if result.err != nil {
		// Wait for the worker's deferred COM releases before reporting the
		// failure.
		<-w.done
...
func (w *writer) run(id string, bufferDuration time.Duration, ready chan<- openResult) {
	...
	client, render, wfx, err := openEndpoint(id, bufferDuration)
	if err != nil {
...
func openEndpoint(
	id string, bufferDuration time.Duration,
) (client, render com, wfx *waveFormatEx, err error) {
	var enumerator com
	...
	ret = client.call(3, sharemodeShared, 0, uintptr(referenceTime(bufferDuration)), 0,
...
func (w *writer) Write(p []byte) (int, error) {
	...
	// Enqueueing into a dead COM thread would report success for up to
	// frameQueueDepth writes.
	if channelClosed(w.done) {
		return 0, w.terminalError(io.ErrClosedPipe)
	}
...
// terminalError decides what a dead COM thread means. play queues its failure
// on errs BEFORE run closes done, so a caller that saw done closed cannot miss
// it; a clean exit leaves errs empty and yields fallback.
func (w *writer) terminalError(fallback error) error {
	select {
	case err := <-w.errs:
		return err
	default:
