...
func (w *writer) Write(p []byte) (int, error) {
	...
	// A COM thread that already unwound must surface its real failure now:
	// enqueueing below would report success into a dead thread for up to
...
// terminalError is the one place that decides what a dead COM thread means:
// play queues its failure on errs BEFORE run unwinds and closes done, so a
// caller that observed done closed cannot miss the real error here. A clean
// exit leaves errs empty and yields fallback — nil for Close, ErrClosedPipe
// for Write.
