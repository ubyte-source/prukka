...
// helper can genuinely take longer than the 5s production bound to observe
// EOF, and killing it there fails the close with a false negative. The
// production constant keeps its own kill-branch test.
...
func TestPlaybackSinkCloseKillsAStuckHelper(t *testing.T) {
	...
	sink := &playbackSink{stdin: stdin, cmd: cmd, tree: tree, drain: 20 * time.Millisecond}
