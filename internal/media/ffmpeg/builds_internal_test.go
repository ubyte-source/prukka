...
func TestBuildMatrixIsCompleteAndImmutable(t *testing.T) {
	...
	want := []buildTarget{
		{os: hostos.Darwin, arch: archAMD64},
		{os: hostos.Darwin, arch: archARM64},
		{os: hostos.Linux, arch: archAMD64},
		{os: hostos.Linux, arch: archARM64},
		{os: hostos.Windows, arch: archAMD64},
		{os: hostos.Windows, arch: archARM64},
	}
	...
	for _, target := range want {
		b, ok := builds[target]
		if !ok {
			t.Fatalf("missing build for %s", target)
		}
		validateBuild(t, target.String(), &b)
	}
}
