...
func TestRedactMediaURLsStripsCredentialsAndPaths(t *testing.T) {
	t.Parallel()

	// Assembled from parts so the fixture is not a hardcoded-credential URL
	// literal (which the redactor exists to strip); user and pass are the
	// exact secrets the redaction must remove.
	user, pass := "user", "secret"
	rtmpWithCreds := "rtmp://" + user + ":" + pass + "@live.example:1935/app/streamkey: Connection refused"
	...
	cases := []struct {
		...
		{
			name: "rtmp userinfo and stream key",
			in:   rtmpWithCreds,
			want: "rtmp://live.example:1935 Connection refused",
		},
		{
			name: "srt query passphrase",
			in:   "srt://relay.example:9000?passphrase=hunter2&mode=caller failed",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			got := redactMediaURLs(tc.in)
			if got != tc.want {
				t.Fatalf("redactMediaURLs(%q) = %q, want %q", tc.in, got, tc.want)
			}
			for _, secret := range []string{"secret", "streamkey", "hunter2", "private"} {
