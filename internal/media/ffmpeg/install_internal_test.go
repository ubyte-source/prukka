...
// TestUnpackTarXzRejectsDuplicateBinary: two entries named ffmpeg are an
// ambiguity, not a choice. The tar half must refuse the archive the zip half
// already refuses, instead of installing whichever one the tar ordered first.
func TestUnpackTarXzRejectsDuplicateBinary(t *testing.T) {
	t.Parallel()

	archive := tarXZPayload(t, []tarEntry{
		{name: "first/" + binaryName(), body: []byte("FIRST-BINARY")},
		{name: "second/" + binaryName(), body: []byte("SECOND-BINARY")},
	})
	archivePath := filepath.Join(t.TempDir(), "ffmpeg.tar.xz")
	if err := os.WriteFile(archivePath, archive, 0o600); err != nil {
		t.Fatalf("write duplicate tar.xz: %v", err)
	}

	if err := unpack(archivePath, kindTarXz, filepath.Join(t.TempDir(), binaryName())); err == nil ||
		!strings.Contains(err.Error(), "duplicate") {
		t.Fatalf("duplicate tar.xz error = %v", err)
	}
}
...
func TestDownloadRejectsHTTPRedirect(t *testing.T) {
	...
	if _, err := download(context.Background(), &b); err == nil || !strings.Contains(err.Error(), "non-https redirect") {
...
func TestInstallBuildPublishesCompleteDeterministicMetadata(t *testing.T) {
	...
	if _, err = os.Stat(bareInstalledPath(stateDir)); !errors.Is(err, fs.ErrNotExist) {
		t.Fatalf("bare state/bin executable was not removed: %v", err)
	}
...
func plantInstallDebris(t *testing.T, stateDir, finalDir string) {
	...
	if err := os.WriteFile(bareInstalledPath(stateDir), []byte("bare"), 0o600); err != nil {
		t.Fatalf("plant bare state/bin executable: %v", err)
	}
