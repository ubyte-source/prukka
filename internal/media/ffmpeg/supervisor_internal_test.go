...
func TestPCMArgsPerScheme(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			args, err := pcmArgs(tc.src, "", 0)
			if err != nil {
				t.Fatalf("pcmArgs: %v", err)
			}
...
func TestPCMArgsVideoTap(t *testing.T) {
	...
	args, err := pcmArgs("rtmp://0.0.0.0:1935/in/x", "/media/x/video", 8*time.Second)
	if err != nil {
		t.Fatalf("pcmArgs: %v", err)
	}
...
func TestPCMArgsEncodesAVSources(t *testing.T) {
	...
	args, err := pcmArgs("device://av/0|1", "/tmp/video", 0)
	if err != nil {
		t.Fatalf("pcmArgs: %v", err)
	}
	joined := strings.Join(args, " ")
...
// TestPCMArgsRejectsUntranslatableDeviceSources: a device URL the package
// cannot turn into capture arguments surfaces its precise parse error before
// any process exists, instead of spawning an ffmpeg child that cannot work
// and reporting an opaque exit.
func TestPCMArgsRejectsUntranslatableDeviceSources(t *testing.T) {
	t.Parallel()

	for _, src := range []string{
		"device://av/0|?label=Mic", // microphone id hidden empty behind the query
		"device://av/0",            // no microphone at all
		"device://video/0",         // video-only capture is not a session source
	} {
		if args, err := pcmArgs(src, "", 0); err == nil {
			t.Fatalf("pcmArgs(%q) = %v, want the device translation error", src, args)
		}
	}
}
