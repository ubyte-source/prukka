...
func TestMicCaptureCommandRoutesOnlyDarwinAudioCaptures(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "darwin audio device", goos: hostos.Darwin, helper: helper, src: mic, wantOK: true},
		{name: "no helper configured", goos: hostos.Darwin, helper: "", src: mic, wantOK: false},
		{name: "video tap keeps ffmpeg", goos: hostos.Darwin, helper: helper, src: mic, videoDir: "/tmp/v", wantOK: false},
		{name: "non-darwin platform", goos: hostos.Linux, helper: helper, src: mic, wantOK: false},
		{name: "network source", goos: hostos.Darwin, helper: helper, src: "srt://host:9000", wantOK: false},
	}
...
// micCaptureHelperPCM is the fake helper's fixed s16le payload: samples 100,
// 200, 300, 400 little-endian.
...
// helper's name, so StartPCM re-execs an already-executable file.
