...
func installBuild(
	...
	if err := recoverInterruptedInstall(stateDir, platform, b); err != nil {
		return "", err
	}
...
func recoverInterruptedInstall(stateDir, platform string, b *build) error {
	root := managedRoot(stateDir)
	...
	if err != nil {
		return fmt.Errorf("read ffmpeg install recovery state: %w", err)
	}

	for _, entry := range entries {
		name := entry.Name()
		if !entry.IsDir() || !strings.HasPrefix(name, ".install-") ||
			!strings.HasSuffix(name, ".previous") {
			continue
		}
		backup := filepath.Join(root, name)
		stage := strings.TrimSuffix(backup, ".previous")
		if recoverErr := recoverInstallPair(
			installDirFor(stateDir, b), stage, backup, platform, b,
		); recoverErr != nil {
			return recoverErr
		}
	}
...
func recoverInstallPair(finalDir, stage, backup, platform string, b *build) error {
	candidate, err := recoveryCandidate(finalDir, stage, backup, platform, b)
	if err != nil {
		return err
	}
	if candidate == "" {
		return errors.Join(os.RemoveAll(stage), os.RemoveAll(backup))
	}
	if err = os.RemoveAll(finalDir); err != nil {
		return fmt.Errorf("remove interrupted ffmpeg install: %w", err)
	}
	if err = os.Rename(candidate, finalDir); err != nil {
		return fmt.Errorf("recover interrupted ffmpeg install: %w", err)
	}

	return errors.Join(os.RemoveAll(stage), os.RemoveAll(backup))
}

func recoveryCandidate(finalDir, stage, backup, platform string, b *build) (string, error) {
	finalComplete, err := installComplete(finalDir, platform, b)
	if err != nil {
		return "", err
	}
	stageComplete, err := installComplete(stage, platform, b)
	if err != nil {
		return "", err
	}
	backupComplete, err := installComplete(backup, platform, b)
	if err != nil {
		return "", err
	}
	if finalComplete {
