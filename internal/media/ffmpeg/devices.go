...
// kindVideo is the device:// kind that maps onto a video injection node.
...
func deviceOutputArgs(goos, url string, resolve OutputIndexResolver) ([]string, error) {
	...
	case kind == kindVideo && goos == hostos.Linux:
		// The webcam node's writer contract is plain write(2) of packed
		// frames: raw yuyv422 through ffmpeg's file protocol, never the v4l2
		// muxer — its S_FMT(OUTPUT) probe is rejected by the capture-only
		// node before a single byte is written.
		return []string{"-pix_fmt", "yuyv422", flagFormat, "rawvideo", id}, nil
	default:
...
// drivers/linux/webcam — before raw frames hit write(2): the node cannot
// negotiate, so an unscaled rendition would shear inside its staging buffer.
...
func deviceVideoFilter(goos, target string) string {
	kind, _, _, err := deviceParts(target)
	if err != nil || kind != kindVideo || goos != hostos.Linux {
		return ""
	}
...
// Close seals the helper's input, then bounds the drain: past it the whole
// process group is killed, because a descendant that outlived the helper
// would keep both the output device and the reaping Wait occupied. The kill
// therefore precedes the reap and the handle is only released after it —
// procio.Tree's one legal order.
func (s *playbackSink) Close() error {
	s.once.Do(func() {
		...
		select {
		case err := <-done:
			s.closeErr = errors.Join(sealErr, err, s.tree.Release())
		case <-time.After(s.drain):
			killErr := s.tree.Kill()
			s.closeErr = errors.Join(sealErr, killErr, <-done, s.tree.Release())
		}
	})
...
func StartDevicePlayback(
	...
	return &playbackSink{stdin: stdin, cmd: cmd, tree: tree, drain: playbackDrainTimeout}, nil
}
