...
// daemon executable inside the macOS app bundle.
const micCaptureBinaryName = "prukka-miccapture"

// MicCaptureBinary resolves the native audio-device helper: beside the
...
// and every non-darwin platform). The one binary serves both device
// directions. Capture: ffmpeg's raw AVFoundation input is delivered silent to
// a process launchd started, because it never asks CoreAudio for capture
// authorization; the helper opens the device through an AVCaptureSession that
// does. Playback: invoked with --play (audio.WithPlaybackHelper wires
// this resolver, StartDevicePlayback spawns it), it renders labeled push
// targets to an output device bound by NAME instead of an audiotoolbox array
// index that reshuffles whenever a device appears or vanishes.
