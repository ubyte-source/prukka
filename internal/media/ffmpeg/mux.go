...
func (s *sink) Close() error {
	...
	return errors.Join(inErr, s.proc.wait())
}
...
func (s *Supervisor) StartSink(ctx context.Context, output []string) (io.WriteCloser, error) {
	...
	s.log.Info("ffmpeg sink started", "pid", cmd.Process.Pid, "output", endpointLabel(target))

	return &sink{in: stdin, proc: &process{
		cmd: cmd, out: nopReader{}, log: s.log, stderr: stderr, src: "sink", done: ctx.Done(),
	}}, nil
}
...
func (s *Supervisor) StartAVSink(
	...
	s.log.Info("ffmpeg av sink started", "pid", cmd.Process.Pid, "video", "hls",
		"burn", vf != "", "output", endpointLabel(target))

	return &sink{in: stdin, proc: &process{
		cmd: cmd, out: nopReader{}, log: s.log, stderr: stderr, src: "av sink", done: ctx.Done(),
	}}, nil
}
