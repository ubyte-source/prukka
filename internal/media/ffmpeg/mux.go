...
func (s *Supervisor) StartMux(ctx context.Context) (*Mux, error) {
	...
	proc := &process{
		cmd: cmd, out: stdout, log: s.log, stderr: stderr, tree: tree, src: "mux", done: ctx.Done(),
	}
...
// bounded wait: if the child does not finish within sinkDrainTimeout its whole
// process group is killed (proc.wait's reap is once-guarded and never kills on
// the drain path, so Close would otherwise hang on a wedged encoder). Killing
// the group and not the leader is what makes the bound real — a wrapper
// script's grandchild would keep both the target and the reaping Wait alive.
...
func (s *Supervisor) startSink(
	...
	return &sink{in: stdin, proc: &process{
		cmd: cmd, out: nopReader{}, log: s.log, stderr: stderr, tree: tree, src: src, done: ctx.Done(),
	}}, nil
}
