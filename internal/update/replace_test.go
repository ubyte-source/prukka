...
// TestReplaceBinaryFailsWhenStagingCannotLand: the destination is untouched
// and no staging residue survives when the ".new" sibling cannot be written.
func TestReplaceBinaryFailsWhenStagingCannotLand(t *testing.T) {
	...
	// A directory squatting on the staging name defeats O_CREATE|O_WRONLY on
	// every OS and for every user: an unwritable-dir fixture would be a no-op
	// on Windows (Mkdir perm is ignored) and writable when run as root.
