...
func replaceBinary(dest string, binary []byte) (err error) {
	...
	defer func() {
		...
		if removeErr := os.Remove(next); removeErr != nil && !errors.Is(removeErr, fs.ErrNotExist) {
...
// stageBinary lands the new image at the FIXED dest+".new" name — like the
// ".old" name in activateWindows, it is a contract: deploy/uninstall.sh
// sweeps crash-time residue by those exact names, so a unique-temp landing
// would leave debris the uninstaller neither removes nor reports.
func stageBinary(path string, binary []byte) error {
	//nolint:gosec // G302: the staged prukka image must be owner-executable.
