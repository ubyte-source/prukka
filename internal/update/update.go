...
// ErrChecksumMissing reports a checksums.txt that carries no entry for the
// platform archive, so its integrity cannot be verified at all.
...
func extractTar(archive []byte, want string) (data []byte, err error) {
	...
	for {
		hdr, nextErr := tr.Next()
		// io.EOF is the clean end of the archive, not a read failure: it must
		// not leak into the "not in archive" error's chain.
