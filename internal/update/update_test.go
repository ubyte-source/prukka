...
// fixture serves a fabricated release: one platform archive, its
// checksums.txt, and the /releases/latest index.
...
// platformArchive builds the archive shape goreleaser publishes.
...
func executableFixture(payload string) []byte {
	switch runtime.GOOS {
	case "windows":
		return append([]byte{'M', 'Z'}, payload...)
	case "darwin":
		return append([]byte{0xcf, 0xfa, 0xed, 0xfe}, payload...)
	default:
		return append([]byte{'\x7f', 'E', 'L', 'F'}, payload...)
	}
}
...
func TestApplyReplacesTheBinary(t *testing.T) {
	...
	binary := executableFixture("new build")
	server := fixture(t, "v9.9.9", binary)
...
func TestApplyIsUpToDateOnSameVersion(t *testing.T) {
	...
	server := fixture(t, "v0.6.0", executableFixture("same"))
	client := update.New(server.URL)
...
// checksumsFixture serves a v9.9.9 release whose checksums.txt is verbatim.
func checksumsFixture(t *testing.T, name string, archive []byte, checksums string) *httptest.Server {
	t.Helper()
...
func TestApplyRejectsTamperedArchive(t *testing.T) {
	...
	// This checksums.txt covers a different payload than the served archive.
	name, archive := platformArchive(t, []byte("tampered"))
	sum := sha256.Sum256([]byte("what the release signed"))
	server := checksumsFixture(t, name, archive, hex.EncodeToString(sum[:])+"  "+name+"\n")
...
func TestApplyRejectsArchiveAbsentFromChecksums(t *testing.T) {
	t.Parallel()

	name, archive := platformArchive(t, executableFixture("unverifiable"))
	server := checksumsFixture(t, name, archive, "0123  another-asset.tar.gz\n")
	...
	err = client.Apply(t.Context(), release, "0.6.0", dest)
	if !errors.Is(err, update.ErrChecksumMissing) {
		t.Fatalf("Apply = %v, want ErrChecksumMissing", err)
	}

	if _, statErr := os.Stat(dest); !errors.Is(statErr, fs.ErrNotExist) {
		t.Fatal("unverified update touched the destination")
	}
}

func TestApplyRejectsNonExecutablePayload(t *testing.T) {
	t.Parallel()

	server := fixture(t, "v9.9.9", []byte("not an executable"))
	client := update.New(server.URL)
	release, err := client.Latest(t.Context())
	if err != nil {
		t.Fatalf("Latest: %v", err)
	...
	dest := filepath.Join(t.TempDir(), "prukka")
	if err := client.Apply(t.Context(), release, "0.6.0", dest); err == nil {
		t.Fatal("Apply accepted a non-executable payload")
	}
	if _, err := os.Stat(dest); !errors.Is(err, fs.ErrNotExist) {
		t.Fatal("invalid executable touched the destination")
	}
}
