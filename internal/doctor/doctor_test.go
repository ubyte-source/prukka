...
func TestRunProducesEveryProbe(t *testing.T) {
	...
	for _, name := range []string{"speech-engine", "ffmpeg", "state-dir"} {
...
func TestSpeechEngineProbeChecksBundleModels(t *testing.T) {
	...
	// Losing one voice of a bidirectional bundle degrades a two-way call.
...
func TestSpeechEngineProbeChecksOptionalCallModel(t *testing.T) {
	...
	// An omitted override, or one equal to the primary model, reuses the
	// already-checked primary artifact.
...
func assertStaticEngineWarning(t *testing.T, check doctor.Check, bin string) {
	...
	// EvalSymlinks also expands Windows 8.3 short names, and %q escapes
	// backslashes there, so the fixture must be put through both.
...
// engineBinPath builds the path to a fake engine binary inside dir, with the
// .exe suffix Windows needs for exec.LookPath to find it.
