...
func TestSpeechEngineProbeRejectsInvalidBundleManifest(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "unknown field",
			content: `{"schema":"prukka.engine.bundle","version":1,"kind":"native","extra":true}`,
		},
		{
			name:    "duplicate field",
			content: `{"schema":"prukka.engine.bundle","version":1,"kind":"native","kind":"native"}`,
		},
		{
			name:    "wrong schema",
			content: `{"schema":"other","version":1,"kind":"native"}`,
		},
		{
			name:    "wrong version",
			content: `{"schema":"prukka.engine.bundle","version":2,"kind":"native"}`,
		},
		{
			name:    "wrong kind",
			content: `{"schema":"prukka.engine.bundle","version":1,"kind":"single-binary"}`,
		},
		{
...
func engineModelFiles(cfg *config.Config) []string {
	files := make([]string, 0, 3+5*len(cfg.Providers.Local.MT.Pairs))
	files = append(files,
		cfg.Providers.Local.STT.Model,
		cfg.Providers.Local.TTS.Voice,
		cfg.Providers.Local.TTS.Voice+".json",
	)
...
func TestSpeechEngineProbeResolvesSymlinkedBundle(t *testing.T) {
	...
	assertStaticEngineWarning(t, check, resolved)
}

func assertStaticEngineWarning(t *testing.T, check doctor.Check, resolved string) {
	t.Helper()

	if check.Status != doctor.StatusWarn {
		t.Fatalf("static engine check = %+v, want warning", check)
	}
	for _, fragment := range []string{resolved, "not tested"} {
