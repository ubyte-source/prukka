...
func TestRunEngineLaneWaitsForTranscriptionBeforeOpeningIngress(t *testing.T) {
	...
	s := &session.Session{
		...
		Langs:      []core.Lang{"it"},
		SourceLang: "it", Subs: session.SubsOff, Bed: session.Off,
	}
...
// assertCapability runs one capability case: the source hint goes through
// ApplyFlags so the tag is canonicalized exactly as the wire would.
...
func TestCaptionSinksDiscardWhenSubtitlesAreOff(t *testing.T) {
	...
	s := &session.Session{
		Slug: "dub-only", Langs: []core.Lang{"it"}, Subs: session.SubsOff,
	}
	sinks := captionSinks(s, &laneDeps{out: Outputs{vtt: registry}}, nil)
...
func TestCallProvidersWarmExactPairAndSelectedVoice(t *testing.T) {
	...
	s := &session.Session{
		...
		SourceLang: "it", DubLangs: session.DubOnly("en"),
	}
...
// TestBuildLaneProvidersRollbackWithVoicesOffDoesNotPanic: the warm-failure
// rollback must tolerate the "voices off" synthesizer, a nil *native.TTS —
// boxed into engine.Closer without a guard it defeats closeLaneProvider's nil
// check and Close runs on a nil receiver, killing the daemon.
func TestBuildLaneProvidersRollbackWithVoicesOffDoesNotPanic(t *testing.T) {
	...
	defer pool.Close()
	s := &session.Session{
		Slug: "voices-off-rollback", Profile: session.ProfileCall,
		Langs: []core.Lang{"en"}, SourceLang: "it",
	}

	if _, err := buildLaneProviders(t.Context(), cfg, pool, s, nil, discard(), nil); err == nil {
...
func TestBuildEngineDub(t *testing.T) {
	...
	if dub := buildEngineDub(s, &laneDeps{out: Outputs{audio: registry}}, nil); dub != nil {
		t.Fatalf("captions-only dub = %v, want none", dub)
	}

	deps := &laneDeps{synth: nopSynth{}, out: Outputs{audio: registry}, voices: []core.Voice{{ID: "v"}}}
	...
	englishOnly := &laneDeps{
		synth: nopSynth{}, out: Outputs{audio: registry},
...
func TestBuildEngineDubBindsOneVoicePerTarget(t *testing.T) {
	...
	deps := &laneDeps{
		synth: nopSynth{}, out: Outputs{audio: registry}, log: discard(),
