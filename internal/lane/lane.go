...
func buildLaneProviders(
	...
	// A nil *native.TTS is the "voices off" state. Every interface view of it
	// is built under a nil guard: a typed nil boxed bare would slip past
	// closeLaneProvider's check and Close would run on a nil receiver.
	var synthWarmer ttsWarmer
	var synthCloser engine.Closer
	if rawSynth != nil {
	...
	if err := warmCallProviders(
		...
		return laneProviders{}, errors.Join(
			err, closeLaneProvider(synthCloser), closeLaneProvider(rawTranslator),
		)
	}
...
func installedEngine(providers *config.Providers) (local *config.Local, engineRoot string, err error) {
	...
	if err != nil {
		// Keep the remediation matchable and the diagnosis reachable: Resolve
		// distinguishes not-installed, incompatible and incomplete bundles.
		return nil, "", fmt.Errorf("%w: %w", errEngineNotInstalled, err)
	}
...
func newTranscriber(
	...
	return native.NewSTT(&native.STTConfig{
		...
		Tuning:  tuning, FastDecode: fastDecode, Inference: observeInference,
	}), nil
}
...
// sttTuning pins call endpointing explicitly — 300 ms hang, sentence-sized
// 5 s windows, 250 ms minimum speech. The daemon owns call policy: the values
// match the helper's broadcast defaults today, and the pin is what lets those
// defaults move without silently retuning calls. Broadcast returns the zero
// tuning, which serializes no flags and leaves the helper's defaults in
// force; fast-decode's bounded Whisper context is the only other call delta.
...
func runEngineLane(ctx context.Context, d *laneDeps, running func()) (retErr error) {
	...
	// Box the concrete metrics under a nil guard: a nil *observability.Metrics
	// boxed bare is a non-nil engine.Metrics, which would defeat engine.New's
	// noop fallback and panic on the first recorded latency.
...
func buildEngineDub(s *session.Session, d *laneDeps, media *hls.Session) *engine.Dub {
	...
	bedDB := bedLevel(string(s.Bed), d.defaultBed)
