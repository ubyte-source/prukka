...
// TestTranslatorHonorsSharedWorkerLimit runs in a bubble: the pool and its
// workers are pure in-process concurrency, so synctest.Wait answers "every
// worker that can start has started" exactly, where the deadline loop it
// replaces could only assert "at least two overlapped within a second".
...
func (s nilStreamSynthesizer) Speak(
	...
	if !s.nilAudio {
		// This provider deliberately breaks the contract: no stream AND no
		// error — the very shape the wrapper must convert into a failure.
		var missing *engine.AudioStream
...
// echoTranslator answers with a marked transform so the wrapper's pass-through
// of the inner translation is observable.
...
func TestTranslatorReturnsInnerTranslationVerbatim(t *testing.T) {
	...
	got, err := wrapped.Translate(t.Context(), engine.Segment{Text: "ciao", Lang: "it"}, "en")
	if err != nil || got != "mt:ciao>en" {
		t.Fatalf("Translate = (%q, %v), want the inner translation verbatim", got, err)
	}
}

// Supports is documented to answer "without consuming a worker slot": it must
// respond while the pool's only worker is parked inside a Translate.
func TestTranslatorSupportsAnswersWhileWorkerBusy(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		go func() {
			_, err := wrapped.Translate(t.Context(), engine.Segment{Text: "occupa"}, "en")
			done <- err
		}()
		synctest.Wait()
		if next.live.Load() != 1 {
		...
		// "Blocked on the worker slot" is now a fact, not a timeout: the
		// bubble settles, and a Supports still waiting has simply not
		// answered.
		synctest.Wait()
		select {
		case ok := <-answered:
...
// posts the terminal result — the provider half of the release contract.
...
// cancellation: engine's voice path abandons oversized takes mid-stream and
// relies on forward's blocked-send cancellation exit. Losing that exit would
// leak one of the few workers per oversized take until all lanes deadlock.
func TestSynthesizerAbandonedStreamReleasesWorkerOnCancel(t *testing.T) {
	...
	<-starts
	// The stream is deliberately never drained: cancel while forward is
	// parked on the blocked send, the only exit that frees the worker.
	cancel()
