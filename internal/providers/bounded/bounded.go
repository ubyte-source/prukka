...
func (l *lifecycle) close(next engine.Closer) error {
	l.closeOnce.Do(func() {
		...
		// Inverted from the usual drain-then-release on purpose: closing next
		// is what unblocks tasks still parked inside the provider, so it must
		// precede the drain. That requires next.Close to be safe against — and
		// to unblock — in-flight calls; do not copy this ordering onto a
		// Closer that does not guarantee it.
