...
func (blockingTreeFrames) Close() error { return nil }
...
func TestSTTCloseReportsProcessTreeFailure(t *testing.T) {
	...
	session.tree = &failingReleaseTree{Tree: session.tree, err: wantCleanup}
...
func TestLaneCancellationWaitsForNativeProcessTree(t *testing.T) {
	ctx, cancel := context.WithCancel(t.Context())
	defer cancel()

	pidResult := make(chan int, 1)
	lane := realtime.New(&realtime.Config{
		Stream: realtime.Stream{Slug: "tree", Source: "it"},
		Providers: realtime.Providers{Transcriber: NewSTT(&STTConfig{
			Bin:   os.Args[0],
			Model: fakeTreeModel,
			Rate:  16000,
		})},
		Output: realtime.Output{Sinks: map[core.Lang]realtime.Sink{
