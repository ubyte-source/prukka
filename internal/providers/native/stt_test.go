...
func TestSTTStreamsTranscripts(t *testing.T) {
	...
	want := []engine.Transcript{
		...
		{
			Text: "Buongiorno a tutti.", Lang: "it", SourceEnd: 3 * chunkSpan,
			Stable: true, Final: true, HasSourceEnd: true,
		},
		// The fourth line is the empty partial, dropped: its chunk still moves
		// the helper's cumulative offset, which is why the empty final that
		// follows resolves to the fifth chunk and not the fourth.
		...
		{
			Text: "Il ponte è aperto.", Lang: "it", SourceEnd: 7 * chunkSpan,
			Stable: true, Final: true, HasSourceEnd: true,
		},
	}
...
// writeProbe stands in for the helper's stdin: it observes the session
// timeline at the exact instant Push hands a frame to the pipe.
...
// TestSTTRecordsSourceFramesBeforeWritingThemToTheHelper pins the ordering the
// timeline documents: a helper that answers the instant a chunk lands must
// find that chunk's boundary already recorded, or the lane dies mid-call on
// "has no recorded source frame". A spawned fake cannot pin this — the window
// a swapped record/write opens is a mutex against a full process round trip —
// so the writer itself does the observing.
