...
// TestProcessTreeKillsBeforeWaitAndReleasesAfter: the signal lands while the
// child PID is still reserved, and the handle is freed only after the reap.
...
func TestWarmProcessShortWriteWaitsForTerminalDiagnostic(t *testing.T) {
	...
	proc := &warmProcess{
		spawnedHelper: &spawnedHelper{
			...
			stderr: stderr,
			tree:   inertProcessTree{},
		},
		log:   discardTestLogger(),
		stop:  make(chan struct{}),
		done:  make(chan struct{}),
		stage: "short-write helper",
	}
	go func() {
		<-proc.stop
		proc.exitErr = procio.WithStderr(errors.New("short-write helper exited"), stderr.String())
		close(proc.done)
	}()
