...
func translateOne(t *testing.T, mt *MT, text string, to core.Lang) string {
	...
	got, err := mt.Translate(t.Context(), realtime.Segment{Text: text, Lang: fakeLang}, to)
	if err != nil {
...
func TestMTEnforcesDeclaredPairs(t *testing.T) {
	...
	mt := newTestMT(t, &MTConfig{Bin: filepath.Join(t.TempDir(), "missing"),
	...
	_, err := mt.Translate(t.Context(), realtime.Segment{Text: "hello", Lang: "en"}, "it")
