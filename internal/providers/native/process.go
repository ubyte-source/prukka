...
// helperWaitDelay bounds how long cmd.Wait blocks for the stderr/stdout copier
// goroutines after the child exits: a group-escaping descendant that inherited
// a pipe would otherwise keep Wait wedged forever. After this, Wait force-closes
// the parent-side fds so the copiers return.
...
func (p *warmProcess) finish(scanErr error) {
	...
	p.exitErr = withHelperStderr(fmt.Errorf("%s exited: %w", p.stage, cause), p.stderr)

	close(p.done)
}

// helperResult carries one decoded reply or the failure that ended the read
// pump.
...
// exit. Each line is decoded onto responses; a stop mid-publish reaps first,
// and the channel closes once no further reply can arrive.
