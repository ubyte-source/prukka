...
// TTSConfig configures synthesis through an external engine helper, one process
// per voice; core.Voice.ID names the configured voice model.
...
// TTS implements realtime.Synthesizer over warm per-voice synthesis helpers.
type TTS struct {
	cache      *procCache[string, *voiceProc]
	bin        string
	engineRoot string
	rate       int
}
...
func NewTTS(cfg *TTSConfig) *TTS {
	return &TTS{
		cache:      newProcCache[string, *voiceProc]("native tts: synthesizer", cfg.Log),
		bin:        cfg.Bin,
...
var (
	_ realtime.Synthesizer = (*TTS)(nil)
	_ realtime.Closer      = (*TTS)(nil)
)

// Speak streams synthesized speech for one turn through the voice's warm
// process: clauses in on text, PCM chunks out on the returned channel.
func (t *TTS) Speak(
...
// warm returns the synthesis process for the voice, spawning it on first use.
...
// discard removes proc only if it is still the cached process for voice.
...
// serializes turns, the read pump owns stdout and reaping.
type voiceProc struct {
	*warmProcess

	responses chan helperResult[nativewire.AudioReply]
	gate      chan struct{}
	rate      int
}
...
func startVoiceProc(
	...
	// .golangci.yml).
	cmd := exec.CommandContext(ctx, bin, nativewire.SubTTS, flagModel, voice, flagRate, strconv.Itoa(rate))
	...
	proc := &voiceProc{
		warmProcess: child,
		responses:   make(chan helperResult[nativewire.AudioReply], 1),
		gate:        make(chan struct{}, 1),
		rate:        rate,
	}
	proc.gate <- struct{}{}

	go readLines(child, proc.responses, decodeTTSResponse)
	go proc.watch(ctx)
...
func (p *voiceProc) synth(ctx context.Context, clause string, out chan<- core.PCM) error {
	req, err := json.Marshal(nativewire.TextLine{Text: clause})
	if err != nil {
...
func decodeTTSResponse(line []byte) (nativewire.AudioReply, error) {
	var msg nativewire.AudioReply
	if err := json.Unmarshal(line, &msg); err != nil {
		return nativewire.AudioReply{}, fmt.Errorf("decode native tts response: %w", err)
	}

	hasAudio := msg.Audio != ""
	if hasAudio == msg.Done {
		return nativewire.AudioReply{}, errors.New(
