...
func BenchmarkFrameNativeSTTPush(b *testing.B) {
	...
	for b.Loop() {
		// Advance PTS by the frame's own duration, exactly as every production
		// ingress does: the timeline then coalesces into one span, which is the
		// steady state this benchmark (and the 0-allocs gate) must measure.
