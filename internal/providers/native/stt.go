...
const nativeSTTProtocolVersion = 2
...
const (
	...
	flagPartialStride = "--partial-stride"
	flagFastDecode    = "--fast-decode"
	flagProtocol      = "--protocol-version"
)

// STTTuning carries profile-specific endpoint and partial-inference pacing.
// Zero values leave the helper's broadcast-safe defaults in force.
	...
	if readyErr := session.waitReady(ctx, sttReadyTimeout); readyErr != nil {
		...
		return nil, errors.Join(
			fmt.Errorf(
				...
				nativeSTTProtocolVersion, readyErr,
			),
			session.Close(),
		)
	}
...
func (s *STT) args(hint core.Lang) []string {
	...
	args := []string{
		subSTT,
		flagProtocol, strconv.Itoa(nativeSTTProtocolVersion),
	...
	args = appendDurationArg(args, flagPartialStride, s.cfg.Tuning.PartialStride)
	if s.cfg.Tuning.FastDecode {
...
func (s *sttSession) observeInference(msg *sttMessage) {
...
func validateSTTMessageShape(msg *sttMessage, fields map[string]json.RawMessage) error {
	if raw, hasReady := fields["ready"]; hasReady {
		return validateReadyField(raw, len(fields))
	}

	if err := validateTranscriptText(msg, fields); err != nil {
		return err
	}
...
func validateTranscriptText(msg *sttMessage, fields map[string]json.RawMessage) error {
...
func transcriptUpdate(
	msg *sttMessage, language core.Lang, sourceEnd time.Duration,
