...
func NewMT(cfg *MTConfig) *MT {
	...
	for _, pair := range cfg.Pairs {
		pairs[pairKey(string(pair.From.Base()), string(pair.To.Base()))] = true
	}
...
func (t *MT) Supports(from, to core.Lang) bool {
	return !t.declared || t.pairs[pairKey(string(from.Base()), string(to.Base()))]
}
...
func (t *MT) Translate(
	...
	from, target := string(source.Lang.Base()), string(to.Base())
	var lastErr error
