...
func (*signalWriteCloser) Write(p []byte) (int, error) { return len(p), nil }
...
func blockedWarmProcess() (proc *warmProcess, entered <-chan struct{}, release func()) {
	signal := make(chan struct{})
	done := make(chan struct{})
	var releaseOnce sync.Once

	return &warmProcess{
		stdin:  &signalWriteCloser{entered: signal},
		stdout: io.NopCloser(strings.NewReader("")), tree: inertProcessTree{},
		stop: make(chan struct{}), done: done, stage: "blocked test helper",
		log: slog.New(slog.DiscardHandler),
	}, signal, func() { releaseOnce.Do(func() { close(done) }) }
}
...
func runFakeSTT() {
	lines := []string{
		`{"partial":"buon"}`,
		`{"partial":"buongiorno"}`,
		`{"text":"Buongiorno a tutti.","final":true}`,
		`{"partial":""}`,
		`{"text":"","final":true}`,
		`{"partial":"il ponte"}`,
		`{"text":"Il ponte è aperto.","final":true}`,
	}
...
func waitStderrMarker(t *testing.T, stderr *tailBuffer, marker string) {
	t.Helper()
	...
	for !strings.Contains(stderr.String(), marker) {
		select {
		case <-ctx.Done():
			t.Fatalf("wait for helper marker %q: %v", marker, ctx.Err())
		case <-ticker.C:
		}
	}
}
