...
// fakeSTTChunkSamples is the mono sample count the fake consumes per reply.
// A real helper frames PCM by its own decode window rather than by the
// producer's write sizes, so a fixed frame keeps the fixture's cumulative
// offsets deterministic instead of depending on how a pipe read splits.
const fakeSTTChunkSamples = 320

// fakeSTTChunkSpan is the source time one such chunk covers at the reference
// rate: the step between the SourceEnd values the fake's offsets resolve to.
...
func pushFakeSTTChunks(t *testing.T, transcription engine.Transcription, count int) {
	t.Helper()

	for i := range count {
		frame := core.PCM{
			Data: make([]int16, fakeSTTChunkSamples), Rate: 16000, Ch: 1,
...
// runFakeSTT answers each consumed PCM chunk with one scripted transcript
// line stamped with the cumulative mono samples it has ACTUALLY read — the
// helper's real wire contract. Emitting only after a read is what drives the
// adapter's timeline through its frame walk instead of the end_samples==0
// shortcut, so the mapping of helper offsets onto source PTS is exercised
// across a real process boundary. The empty partial and empty final must
// still be dropped and kept respectively.
func runFakeSTT() {
	lines := []string{
