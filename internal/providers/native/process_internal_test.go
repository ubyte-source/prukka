...
func newStubCache() *procCache[string, *stubProc] {
	return newProcCache[string, *stubProc]("test cache", nil)
}
...
func TestProcCacheDiscardReapsOutsideTheLock(t *testing.T) {
	...
	// The entry leaves the map under the lock but the reap runs unlocked, so a
	// concurrent Close and every other key keep making progress.
	if !cache.mu.TryLock() {
...
// TestWarmProcessCloseSurfacesTreeCleanupFailure: close returns only after the
// read pump reaped the child, and reports the tree retirement failure.
func TestWarmProcessCloseSurfacesTreeCleanupFailure(t *testing.T) {
	...
	pair.tree = &failingReleaseTree{Tree: pair.tree, err: wantCleanup}
...
// cacheAdapterHarness drives one warm-helper adapter through its public surface,
// so procCache behaviors shared by MT and TTS are pinned once.
...
func cacheSize[K comparable, P helperProc](cache *procCache[K, P]) int {
	cache.mu.Lock()
...
// en-US target also pins base-tag keying onto the warmed zz>en pair.
func newMTCacheHarness(t *testing.T) *cacheAdapterHarness {
	...
	pair := func() *pairProc {
		...
		return mt.cache.procs[basePair(fakeLang, "en")]
	}
	...
	return &cacheAdapterHarness{
		...
		altTrip: func(ctx context.Context) error {
			_, err := mt.Translate(ctx, realtime.Segment{Text: "other", Lang: fakeLang}, "de")
...
func translateTrip(ctx context.Context, mt *MT, text string) error {
	got, err := mt.Translate(ctx, realtime.Segment{Text: text, Lang: fakeLang}, "en-US")
	if err != nil {
		return err
	}
	if got != "mt:"+text {
...
// speakTurn runs one synthesis turn and returns its PCM and stream result.
...
// audio is reported as the corruption, not as its stream error.
...
// TestNativeAdapterWarmHelperOutlivesItsFirstContext: Warm loads the helper into
// the cache lifetime, so the warming request's context cannot kill it.
...
// TestNativeAdapterSurfacesHelperFailures: the diagnostic reaches the caller and
...
// TestNativeHelperWriteFailureWaitsForDiagnostic: a broken request pipe surfaces
// the helper's stderr diagnostic within the bounded stop.
...
// TestHelperCrashDeclaresItsStderr closes the loop with the published session
// status: the tail stays in the error a caller reads, and the error names it as
// the child's own text so the control plane drops exactly that span.
func TestHelperCrashDeclaresItsStderr(t *testing.T) {
	h := newMTCacheHarness(t)

	err := h.roundTrip(t.Context(), fakeCrash)
	if err == nil {
		t.Fatal("crashing helper returned no error")
	}

	var foreign redact.Untrusted
	if !errors.As(err, &foreign) {
		t.Fatalf("helper error does not declare its stderr as foreign: %v", err)
	}
	if !strings.Contains(foreign.Untrusted(), "fake mt crash") {
		t.Fatalf("declared span = %q, want the helper stderr", foreign.Untrusted())
	}
}
