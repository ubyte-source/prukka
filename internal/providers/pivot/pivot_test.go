...
// ExampleSupported shows routing over a hub-and-spoke catalog: a pair with no
// direct model is still supported when both hub legs exist, and only a pair
// missing a hub leg is refused.
func ExampleSupported() {
	// Every installed pair touches English, the hub: no direct it<->de model
	// exists, and no fr model exists at all.
	installed := map[route]bool{
		{"it", "en"}: true, {"en", "it"}: true,
		{"en", "de"}: true,
	}
	...
	emit(pivot.Supported(direct, pivot.English, "it", "en")) // direct model
	emit(pivot.Supported(direct, pivot.English, "it", "de")) // bridged it->en->de
	emit(pivot.Supported(direct, pivot.English, "fr", "fr")) // same language is a free copy
	emit(pivot.Supported(direct, pivot.English, "de", "it")) // no de->en leg, no bridge
	// Output:
	// true
	// true
	// true
