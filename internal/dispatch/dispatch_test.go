...
func TestNewClampsNonPositiveArgs(t *testing.T) {
	...
	// Non-positive worker/queue values fall back to defaults and still run.
	p := dispatch.New(0, -1)
	defer p.Close()

	done := make(chan struct{})
	if err := p.Submit(context.Background(), func() { close(done) }); err != nil {
		t.Fatalf("Submit on a defaulted pool: %v", err)
	}

	select {
	case <-done:
	case <-time.After(time.Second):
		t.Fatal("defaulted pool never ran the job")
	}
}

func TestMetricsReflectThroughput(t *testing.T) {
	t.Parallel()
	...
	m := p.Metrics()
	if m.EnqueueCount != jobs || m.DequeueCount != jobs {
		t.Fatalf("metrics enq/deq = %d/%d, want %d/%d", m.EnqueueCount, m.DequeueCount, jobs, jobs)
	}
}
