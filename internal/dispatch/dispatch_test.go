...
// TestNewRejectsNonPositiveSizing pins the constructor's documented panic
// guard. The zero rows are the load-bearing ones — without the guard a zero
// pool constructs fine and hangs later; negative sizes would panic anyway,
// so asserting the guard's own message keeps every row discriminating.
...
// TestConcurrencyNeverExceedsWorkers runs in a bubble, which upgrades the
// assertion from a safety bound to an equality: instead of sleeping inside
// each job to make overlap observable and then checking "at most workers",
// the jobs park on a channel and synctest.Wait settles the pool, so the
// observed peak is exactly the worker count.
// recordPeak raises peak to n when n is the highest concurrency seen.
...
func TestConcurrencyNeverExceedsWorkers(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		// Submitting from its own goroutine: with every worker parked the
		// queue fills and Submit blocks, which is the backpressure this pool
