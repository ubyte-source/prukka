...
// Release closes the job handle, whose kill-on-close limit retires whatever is
// still enrolled without naming a PID. terminateLocked's process-table walk
// does name PIDs, so it must not run here.
...
func (t *windowsTree) terminateLocked() error {
	...
	// A grandchild spawned between the child's start and its job assignment
	// escaped the job, and once the helper dies nothing can link it back to the
	// helper PID — so snapshot before terminating anything.
