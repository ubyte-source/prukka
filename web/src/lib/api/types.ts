...
export interface Session {
  slug: string;
  profile: string;
  /** Sanitized source identity. The raw URL is write-only input: read
   *  responses and lifecycle events never carry it. */
  sourceLabel?: string;
  langs: string[];
  flags?: Record<string, string>;
  delaySeconds?: number;
  /** Observed lane state; the daemon reports one on every session. */
...
export interface LocalConfig {
  /** sttModel is not edited by the dashboard, but Settings posts the whole
   *  config back: an update that omits it is rejected as a missing model. */
  sttModel?: string;
...
export interface EngineStatus {
  ...
  catalogError?: string;
  packs?: EnginePack[];
  /** operation is absent while the engine manager is idle. The gateway
   *  (EmitUnpopulated) renders that absence as JSON null; the client
   *  normalizes it away before the status reaches the app. */
