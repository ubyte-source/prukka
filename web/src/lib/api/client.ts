...
//
// Hosted mode: the bundle sends API calls directly to the local daemon. The
// hosting origin still controls executable JavaScript and is a privileged
// trust boundary. Loopback is exempt from mixed-content blocking.
...
import { normalizeEngineStatus } from "../enginePacks";
...
export async function getEngine(token: string, signal?: AbortSignal): Promise<EngineStatus> {
  ...
  return normalizeEngineStatus(reply.engine ?? {});
}
