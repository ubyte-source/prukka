<script lang="ts">
  ...
  // then a success line — or a plain, recoverable error. All state comes from
  // the single-flight engineInstall controller.

  import type { LanguagePlan } from "../enginePacks";
  import { mb } from "../enginePacks";
  ...
  const { tag, plan, needVoice, addBytes, onAdded }: Props = $props();
  ...
  function start() {
    if (controlToken === "" || engineInstall.busy) return;
    void engineInstall.start(tag, plan, needVoice, controlToken, () => {
      onAdded?.(tag);
      added = true;
      clearTimeout(addedTimer);
      addedTimer = setTimeout(() => (added = false), 6000);
    });
  }
</script>
...
{:else if catalogError}
  <div class="flex flex-wrap items-center gap-2 text-xs text-warn">
    ...
    <button
      ...
      class="rounded border border-line px-2 py-1 text-ink hover:border-brand"
    >
      {i18n.m.add.retry}
    </button>
  </div>
{:else if mine && engineInstall.phase === "error"}
  <div class="flex flex-col gap-1">
    ...
    <button
      type="button"
      onclick={start}
      class="self-start rounded border border-line px-2 py-1 text-xs text-ink hover:border-brand"
    >
      {i18n.m.add.tryAgain}
    </button>
  </div>
...
{:else if added}
  <p class="text-xs text-ok">{fill(i18n.m.add.done)}</p>
{:else if controlToken === ""}
  <p class="text-xs text-ink-dim">{i18n.m.add.connect}</p>
{:else if engineInstall.busy}
  <button
    type="button"
    disabled
    class="rounded-lg border border-line px-3 py-1.5 text-xs text-ink-dim opacity-60"
  >
