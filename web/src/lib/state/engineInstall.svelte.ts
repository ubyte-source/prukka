...
import { coreInstalled, packsToAdd, totalSizeBytes, type LanguagePlan } from "../enginePacks";
import { daemon } from "./daemon.svelte";
import { liveEngineProgress } from "./engineProgress";

export type InstallPhase = "idle" | "preparing" | "running" | "done" | "error";

/** ensureEngineCore installs the managed runtime, then the STT core pack,
 *  waiting out each daemon operation — the prerequisite ladder shared by the
 *  first-run hero, the pack manager and every guided language add. */
export async function ensureEngineCore(token: string, signal: AbortSignal): Promise<void> {
...
class EngineInstall {
  ...
  /** liveBytes is this op's freshest byte progress, from the shared selector. */
  private liveBytes(): { done: number; total: number; phase: EnginePhase | null } {
    const live = liveEngineProgress();
    if (live === null) return { done: 0, total: 0, phase: null };
    return { done: live.done, total: live.total, phase: live.phase };
  }
  ...
  async start(
    ...
    try {
      if (!coreInstalled(daemon.engine)) {
        ...
        await ensureEngineCore(token, signal);
      }
