...
class DaemonState {
  ...
  /** engineSupported turns false when the daemon answers 404 for /api/v1/engine. */
    ...
    getEngine(tokenSnapshot, controller.signal)
      ...
      .catch((e: unknown) => {
        ...
        if (e instanceof ApiError && e.status === 404) {
          // A daemon without the pack manager answers 404; the section stays hidden.
