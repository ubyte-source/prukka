<script lang="ts">
  ...
  // adopted from the launch URL and is never displayed; this button reports
  // whether one is held, and opens a small field to paste (or change) the
  // access code, or to disconnect.
...
<div class="relative">
  <button
    ...
    class={`inline-flex items-center gap-1.5 rounded-lg border px-2.5 py-1 text-xs font-medium
            text-ink focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand ${
              connected
                ? "border-ok/80 bg-ok/10 hover:border-ok"
                : "border-warn/80 bg-warn/10 hover:border-warn"
            }`}
  >
  ...
  {#if open}
    <form
      onsubmit={apply}
      class="absolute right-0 z-20 mt-2 flex w-72 flex-col gap-2 rounded-2xl border border-hairline
             bg-panel p-3 shadow-pop"
    >
      ...
      <label class="flex flex-col gap-1 text-xs font-medium">
        ...
        <input
          ...
          class="rounded-[10px] border border-line bg-panel-2 px-2 py-1.5 font-mono text-sm
