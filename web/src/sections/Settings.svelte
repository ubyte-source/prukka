...
{#snippet numberField(
  ...
  <label class="flex flex-col gap-1 text-sm">
    ...
    <input
      ...
      class="rounded-[10px] border border-line bg-panel-2 px-3 py-2 font-mono
...
>
  <h2 id="settings-title" class="mb-4 text-lg font-semibold tracking-tight">{i18n.m.settings.title}</h2>
    ...
    <form onsubmit={save} oninput={markDirty} autocomplete="off" class="flex flex-col gap-6">
      ...
      {#if config.providers}
        ...
        <fieldset class="flex flex-col gap-3">
          <legend class="mb-2 text-sm font-semibold">{i18n.m.settings.voices}</legend>
          <div class="max-w-lg">
            <Switch
              label={i18n.m.settings.voicesLabel}
              hint={i18n.m.settings.voicesHint}
              checked={p.voices !== "off"}
              onchange={(checked) => {
                p.voices = checked ? "local" : "off";
                markDirty();
              }}
            />
          </div>
      ...
      <div class="flex flex-wrap items-center gap-3">
        <button
          type="submit"
          disabled={busy}
          class="rounded-full bg-brand-dim px-5 py-2 font-medium text-white
