<script lang="ts">
  ...
  // route-less languages as caption targets rather than downloads.
  ...
  function callDownloads(): AddablePack[] {
    const seen = new Set<string>();
    const out: AddablePack[] = [];
    for (const tag of [youLang, themLang]) {
  ...
  // Broadcast: a language the audience wants that needs a download first. The
  // picker mirrors Sessions' "add a language you don't have yet".
  let broadcastAdd = $state<AddablePack | null>(null);

  function broadcastAddable(): { tag: string; label: string }[] {
    if (broadcastAdd !== null) return [];
    return languages
      .filter((language) => !targets.includes(language.tag) && !captionTargets.includes(language.tag))
      .filter((language) => !translationTargetSupported(language.tag))
      .filter((language) => addablePack(language.tag, dub) !== null)
      .map((language) => ({ tag: language.tag, label: language.label }));
  }

  function beginBroadcastAdd(tag: string) {
    const pack = addablePack(tag, dub);
    if (pack !== null) broadcastAdd = pack;
  }

  function finishBroadcastAdd(tag: string) {
    ...
    if (dub && voiceSupports(tag)) toggleTarget(tag);
    else toggleCaptionTarget(tag);
    broadcastAdd = null;
  }

  // Languages added inline during the call flow. Their download panel vanishes
  // the moment the packs land (addablePack returns null), so the confirmation
  // must live here in the owner that survives the panel.
  let callAdded = $state<string[]>([]);

  function finishCallAdd(tag: string) {
    if (!callAdded.includes(tag)) callAdded = [...callAdded, tag];
  }

  const callDownloadList = $derived(profile === "call" ? callDownloads() : []);
...
<section aria-labelledby="wizard-title" class="rounded-xl border border-line bg-panel p-5">
  ...
  <div class="mb-4">
    <Stepper
      steps={[i18n.m.wizard.stepType, i18n.m.wizard.stepConnect, i18n.m.wizard.stepLanguages]}
      current={step - 1}
    />
  </div>
  ...
  >
    ...
    {:else}
      ...
      {/if}
      {#if callDownloadList.length > 0}
        <div class="flex flex-col gap-3 rounded-lg border border-brand/30 bg-brand/5 p-3">
          <p class="text-sm font-medium text-ink">{i18n.m.wizard.callNeedsDownload}</p>
          {#each callDownloadList as dl (dl.tag)}
            <LanguageDownload
              tag={dl.tag}
              plan={dl.plan}
              needVoice={dl.needVoice}
              addBytes={dl.addBytes}
              onAdded={(tag) => finishCallAdd(tag)}
            />
          {/each}
        </div>
      {/if}
      {#each callAdded as tag (tag)}
        <p role="status" class="text-sm text-ok">
          {i18n.m.add.done.replace("{language}", languageLabel(tag))}
        </p>
      {/each}
      {:else}
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
    ...
    <fieldset class="rounded-lg border border-line p-3">
      ...
      {#if broadcastAdd}
        <div class="mt-3 max-w-xs">
          <LanguageDownload
            tag={broadcastAdd.tag}
            plan={broadcastAdd.plan}
            needVoice={broadcastAdd.needVoice}
            addBytes={broadcastAdd.addBytes}
            onAdded={(tag) => finishBroadcastAdd(tag)}
          />
        </div>
      {:else if daemon.engineSupported && broadcastAddable().length > 0}
        <div class="mt-3 max-w-xs">
          <Select
            compact
            label={i18n.m.wizard.addMissingBroadcast}
            options={broadcastAddable().map((l) => ({ value: l.tag, label: l.label }))}
            onchange={(tag) => beginBroadcastAdd(tag)}
          />
        </div>
      {/if}
    </fieldset>
