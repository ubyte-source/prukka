<script lang="ts">
  ...
  import { languagePlans, planForLanguage, type LanguagePlan } from "../lib/enginePacks";
  import { languageInfo, type LangCtx } from "../lib/languageAvailability";
  ...
  // downloadFor holds the missing language a running session is downloading,
  // keyed by slug, so its inline progress bar renders under that row.
  let downloadFor = $state<Record<string, { tag: string; plan: LanguagePlan; needVoice: boolean; addBytes: number }>>({});

  // langCtx captures the live capability a session's add-a-language uses.
  function langCtx(s: Session): LangCtx {
    return {
      pairs: daemon.config.providers?.local?.mt?.pairs ?? [],
      dubbedLangs: daemon.config.providers?.local?.dubbedLangs ?? [],
      voicesOff: daemon.config.providers?.voices === "off",
      plans: daemon.engine === null ? [] : languagePlans(daemon.engine),
      engineSupported: daemon.engineSupported,
      catalogAvailable: (daemon.engine?.catalogError ?? "") === "",
      source: s.flags?.source ?? "auto",
    };
  }

  // addableLanguages lists languages the session lacks that need a quick
  // download to become usable — those the plain add-picker cannot offer because
  // their translation route is not installed yet (disjoint from that picker).
  function addableLanguages(s: Session): { tag: string; label: string }[] {
    if (downloadFor[s.slug] !== undefined) return [];
    const ctx = langCtx(s);
    return daemon.languages
      .filter((language) => !s.langs.includes(language.tag))
      .filter((language) => !canTranslateTo(s, language.tag))
      .filter((language) => languageInfo(ctx, language.tag, true).status === "addable")
      .map((language) => ({ tag: language.tag, label: language.label }));
  }

  function beginDownload(s: Session, tag: string) {
    const ctx = langCtx(s);
    const info = languageInfo(ctx, tag, true);
    const plan = planForLanguage(ctx.plans, tag);
    if (info.status !== "addable" || plan === undefined) return;
    downloadFor = { ...downloadFor, [s.slug]: { tag, plan, needVoice: info.needVoice, addBytes: info.addBytes } };
  }

  function finishDownload(s: Session, tag: string) {
    addLang(s, tag);
    const next = { ...downloadFor };
    delete next[s.slug];
    downloadFor = next;
  }
...
<section aria-labelledby="sessions-title" class="rounded-2xl border border-hairline bg-panel p-6 shadow-card">
  <h2 id="sessions-title" class="mb-4 text-lg font-semibold tracking-tight">{i18n.m.sessions.title}</h2>
  ...
  {:else}
    <div class="overflow-x-auto">
      <table class="w-full text-left text-sm" aria-labelledby="sessions-title">
        <thead>
          <tr class="border-b border-hairline text-xs uppercase tracking-wide text-ink-dim">
          ...
          {#each daemon.sessions as s (s.slug)}
            <tr class="border-b border-line/50 align-top">
              ...
              <td class="py-3 pr-4">
                ...
                {#if downloadFor[s.slug]}
                  {@const dl = downloadFor[s.slug]}
                  <div class="mt-2 max-w-xs">
                    <LanguageDownload
                      tag={dl.tag}
                      plan={dl.plan}
                      needVoice={dl.needVoice}
                      addBytes={dl.addBytes}
                      onAdded={(tag) => finishDownload(s, tag)}
                    />
                  </div>
                {:else if daemon.engineSupported && addableLanguages(s).length > 0}
                  <div class="mt-2">
                    <Select
                      compact
                      disabled={pending.has(s.slug)}
                      label={`${i18n.m.sessions.addMissing} ${s.slug}`}
                      options={addableLanguages(s).map((l) => ({ value: l.tag, label: l.label }))}
                      onchange={(tag) => beginDownload(s, tag)}
                    />
                  </div>
                {/if}
              </td>
