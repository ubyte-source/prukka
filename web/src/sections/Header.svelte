<script lang="ts">
  ...
  const cost = $derived((daemon.stats.costEurPerHour ?? 0).toFixed(4));
...
>
  <div class="flex items-center gap-3">
    <span
      class={`h-2.5 w-2.5 rounded-full ${dot[daemon.status]}`}
      title={`daemon ${daemon.status}`}
      role="status"
      aria-label={`daemon ${daemon.status}`}
    ></span>
  ...
  <div class="flex items-center gap-5">
    <dl class="flex items-center gap-5 text-sm">
      <div class="text-center">
        <dd class="font-mono font-semibold">{daemon.sessions.length}</dd>
        <dt class="text-xs text-ink-dim">{i18n.m.stats.sessions}</dt>
      </div>
      <div class="text-center">
        <dd class="font-mono font-semibold">{cost}</dd>
        <dt class="text-xs text-ink-dim">€/h</dt>
      </div>
      <div class="text-center">
        <dd class="font-mono font-semibold">{uptime}</dd>
        <dt class="text-xs text-ink-dim">{i18n.m.stats.uptime}</dt>
      </div>
      <div class="text-center">
        <dd class="font-mono font-semibold">{daemon.stats.version ?? "–"}</dd>
        <dt class="text-xs text-ink-dim">{i18n.m.stats.version}</dt>
      </div>
    </dl>
