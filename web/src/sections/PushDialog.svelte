...
<dialog
  ...
  class="m-auto w-full max-w-md rounded-2xl border border-hairline bg-panel p-0 shadow-pop
...
>
  <form onsubmit={submit} aria-busy={busy} class="flex flex-col gap-4 p-5">
    ...
    <p id="push-target-notice" class="rounded-xl border border-hairline bg-panel-2 p-3 text-xs text-ink-dim">
    ...
    <div class="flex items-center justify-end gap-3">
      <button
        ...
        class="rounded-full border border-line px-5 py-2 text-sm text-ink-dim
      ...
      <button
        type="submit"
        disabled={busy}
        class="rounded-full bg-brand-dim px-5 py-2 text-sm font-semibold text-white
