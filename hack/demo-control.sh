...
curl -fs "http://$PRUKKA_HTTP/api/v1/sessions" | grep -q '"slug":"demo"'
curl -fs "http://$PRUKKA_HTTP/api/v1/languages" | grep -q '"label":"Italiano — it"'
code=$(curl -s -o /dev/null -w '%{http_code}' -X POST "http://$PRUKKA_HTTP/api/v1/sessions" -d '{}')
[ "$code" = "401" ]
TOKEN=$(cat "$PRUKKA_STATE/control.token")
curl -fs -X DELETE -H "Authorization: Bearer $TOKEN" "http://$PRUKKA_HTTP/api/v1/sessions/demo" >/dev/null

echo "==> dashboard is served"
curl -fs "http://$PRUKKA_HTTP/ui/" | grep -qi prukka
