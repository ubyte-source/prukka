...
# Shared load-gate helpers, sourced — never executed — by hack/loadgen.sh,
# hack/pgo.sh and hack/ci/loadgen-gate-test.sh.

LOAD_MAX_SESSIONS=64
# shellcheck disable=SC2034 # read only by the sourcing scripts, not here.
...
# The bound checks below compare digit strings by length, then
# lexicographically, never with (( )): a value past int64 wraps there —
# 18446744073709551626 becomes 10 — so an arithmetic minimum check would
# spuriously reject a genuinely huge value, and an arithmetic maximum check
# would accept one far beyond its cap.
...
load_validate_percentage() {
  ...
  load_validate_positive_integer "$name" "$value" || return
  # shellcheck disable=SC2071 # string compare is the point: see the note above.
  if (( ${#value} > 3 )) ||
...
# load_has_voiced_audio proves a completed HLS segment carries a non-trivial
# waveform. Load configs mute the source bed, so any signal here is the
