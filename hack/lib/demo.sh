...
# demo_await_snapshot_exit prints whatever is still in the process table when
# the deadline passes. The wait is bounded rather than single-shot: a TERM'd
# tree needs a scheduling beat to unwind on a loaded runner.
...
# demo_require_engine exposes the local speech-engine executable to the daemon.
demo_require_engine() {
  local mode=${2:-skip}
