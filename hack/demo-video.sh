#!/usr/bin/env bash
# Video demo: a real audio+video source streams in, /{session}/master.m3u8
# serves the passthrough video rendition alongside the dubbed-audio and
# live-subtitle ones, and a `subs=burn` push is checked in pixels.
# Needs a local speech engine and ffmpeg (prukka setup); skips cleanly without
# either, but never converts a runtime failure into a skip.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# The listener starts before the session: the live cue window (PTS+D) must
...
# The cue window (source 0–4.4, shown at output 8–12.4 on the wall clock)
# must land while the push encodes.
...
# Captions lag the video by provider latency plus the session delay D=8s.
...
# The caption band (bottom 80 px) during the cue window must differ from
# before it.
for _ in $(seq 1 60); do
