...
# the dubbed-audio and live-subtitle renditions; a burn-in push (subs=burn,
# subs=burn) draws the live captions onto the pushed video and the pixels
...
for _ in $(seq 1 20); do
  if grep -q "lane unavailable" "$PRUKKA_STATE/daemon.log"; then
    ...
    echo "==> video demo SKIPPED (no usable OpenRouter key): $reason"
    exit 0
  fi
...
for _ in $(seq 1 40); do
  if curl -fs "http://$PRUKKA_HTTP/video-demo/master.m3u8" 2>/dev/null | grep -q "video/index.m3u8"; then
    found=yes
    break
  fi
  sleep 0.5
...
curl -fs "http://$PRUKKA_HTTP/video-demo/master.m3u8"
...
seg=$(curl -fs "http://$PRUKKA_HTTP/video-demo/video/index.m3u8" | grep -m1 '\.ts$')
curl -fs "http://$PRUKKA_HTTP/video-demo/video/$seg" -o "$PRUKKA_STATE/video-seg.ts"
...
for _ in $(seq 1 60); do
  for part in $(curl -fs "http://$PRUKKA_HTTP/video-demo/subs/en/index.m3u8" 2>/dev/null | grep '\.vtt$' || true); do
    body=$(curl -fs "http://$PRUKKA_HTTP/video-demo/subs/en/$part" 2>/dev/null || true)
...
audio=""
for _ in $(seq 1 40); do
  if curl -fs "http://$PRUKKA_HTTP/video-demo/audio/en/index.m3u8" 2>/dev/null | grep -q '\.ts$'; then
    audio=yes
    break
  fi
  sleep 0.5
