...
// TestCreateArchiveRecordsSymlinkToDirectoryAsLink pins the macOS-bundle
// shape (Versions/Current-style links): a requested path that is a symlink to
// a directory must be archived as one TypeSymlink entry, never walked into.
func TestCreateArchiveRecordsSymlinkToDirectoryAsLink(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("symlink creation requires privileges on Windows")
	}
	t.Parallel()
	root := t.TempDir()
	writeTestFile(t, filepath.Join(root, "target", "child.txt"), "data", 0o644)
	if err := os.Symlink("target", filepath.Join(root, "link")); err != nil {
		t.Fatal(err)
	}
...
// TestCreateArchiveRecordsDanglingSymlink pins the mirror case: a dangling
// requested link is stored as a link instead of aborting the archive.
func TestCreateArchiveRecordsDanglingSymlink(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("symlink creation requires privileges on Windows")
	}
	t.Parallel()
	...
	output := filepath.Join(t.TempDir(), "archive.tar.gz")
	if err := createArchive(options{output: output, root: root, paths: []string{"broken"}}); err != nil {
		t.Fatal(err)
	}
	data := readTestFile(t, filepath.Dir(output), filepath.Base(output))
	entries := archiveEntries(t, data)
	if len(entries) != 1 || entries[0].Typeflag != tar.TypeSymlink || entries[0].Linkname != "missing" {
		t.Fatalf("entries = %+v", entries)
	}
}
