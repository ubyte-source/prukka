// Command deterministic-tar writes a tar.gz whose bytes depend only on the
// archived files' names, contents and execute bit. Ownership, timestamps, the
// producing umask and the directory walk order are all erased, so two builds
// of the same tree produce the same archive.
package main
...
// options is one invocation of the archiver.
type options struct {
	output string
	root   string
	paths  []string
...
// writeOutput renames a completed temporary over the destination, so an
// interrupted build leaves no archive rather than a truncated one.
...
// syncAfterWrite skips the flush when the write already failed: the temporary
// is about to be removed, and a sync error there would only mask the real one.
...
func collect(root fs.FS, paths []string) ([]string, error) {
	...
	for _, name := range paths {
		...
		// fs.WalkDir follows a root that is itself a symlink, storing the
		// link's children under the link's name; makeHeader uses Lstat, so only
		// a real directory may be walked.
		info, err := fs.Lstat(root, clean)
		if err != nil {
			return nil, fmt.Errorf("stat %q: %w", name, err)
		}
		if !info.IsDir() {
			if err := record(clean); err != nil {
				return nil, err
			}
...
// cleanArchivePath returns a slash-separated name because a tar entry name is
// slash-separated on every platform, including the Windows host this
...
// writeArchive pins the gzip header's modification time and OS byte rather
// than inheriting them. compress/gzip happens to default to these same two
// values, but neither default is part of its documented contract.
...
// makeHeader strips what the producing machine would otherwise leave in the
// entry — owner, owner name, access and change times, PAX records — and pins
// FormatPAX, because archive/tar otherwise picks a format per header from
// whichever ones the values happen to fit.
...
// canonicalMode keeps only the execute intent, so archives do not remember
// the producing machine's umask.
...
// writeContents copies exactly the size the header declared, so a file whose
// length changed between the stat and the read cannot desynchronize the stream
