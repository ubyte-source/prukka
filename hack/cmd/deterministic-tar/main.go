...
func collect(root fs.FS, paths []string) ([]string, error) {
	...
	for _, name := range paths {
		...
		// Classify before walking: fs.WalkDir follows a root that is itself a
		// symlink, which would store the link's children under the link's name
		// and fail outright on a dangling link. makeHeader uses Lstat, so the
		// walk must agree with it: only a real directory is walked.
