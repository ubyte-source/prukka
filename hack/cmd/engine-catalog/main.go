...
// Artifact names are a fixed contract with the publishing workflow: a file
// matching none of these patterns is a mistake.
var (
...
// options is one invocation of the catalog builder.
type options struct {
	dir      string
	baseURL  string
	output   string
...
// packMeta is the sidecar a pack tarball ships beside it.
...
// inventory is the classified content of the artifact directory.
...
func main() {
	...
	// Defaulting to the daemon's own constant keeps the published number
	// derived from the tagged tree, never a workflow literal that can drift.
	flag.IntVar(&opts.protocol, "protocol", speech.SupportedProtocol,
		"engine protocol version the artifacts implement")
...
// classify refuses an unrecognized name rather than skipping it, so an
// artifact the publishing workflow renamed fails the build instead of quietly
...
// checkComplete pairs every pack archive with its sidecar in both directions:
// an archive alone publishes bytes the daemon cannot describe, and a sidecar
// alone describes bytes there are none of.
...
// runtimeEntries sorts by OS then arch so the same artifact directory yields a
// byte-identical catalog whatever order the directory was read in.
...
// readMeta decodes strictly, so a sidecar carrying a field this builder does
// not model fails the build rather than losing it into the catalog silently.
...
// requiredFields maps a pack kind to the fields it must carry; the second
// result reports whether the kind exists at all.
...
// validateMeta requires the fields the kind defines and rejects every other
// one, so a pack cannot ship a value the daemon will never read.
...
func writeCatalog(output string, doc *speech.Catalog) error {
	...
	// A catalog built for another protocol version is legitimately rejected
	// by this daemon's parser, so only a matching one is round-tripped.
