...
// writeChecksums emits one manifest line per release subject. archiveDigests
// carries digests computed while the archives were certified; a cached entry
// still matches the bytes on disk because nothing rewrites an archive after
// certification. Subjects outside the cache (the SBOM documents) are hashed
// here.
