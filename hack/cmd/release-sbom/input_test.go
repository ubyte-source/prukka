...
func TestReadTarBinary(t *testing.T) {
	t.Parallel()
	want := []byte("runtime")
	var tarOutput bytes.Buffer
	gz := gzip.NewWriter(&tarOutput)
	tw := tar.NewWriter(gz)
	header := &tar.Header{Name: "prukka", Mode: 0o755, Size: int64(len(want)), Typeflag: tar.TypeReg}
	...
	got, err := readTarBinary(bytes.NewReader(tarOutput.Bytes()), "linux")
	if err != nil || !bytes.Equal(got, want) {
		t.Fatalf("readTarBinary() = %q, %v", got, err)
	}
}

func TestReadZIPBinary(t *testing.T) {
	t.Parallel()
	want := []byte("runtime")
	var zipOutput bytes.Buffer
	zw := zip.NewWriter(&zipOutput)
	entry, err := zw.Create("prukka.exe")
	if err != nil {
		t.Fatal(err)
	}
	if _, writeErr := entry.Write(want); writeErr != nil {
		t.Fatal(writeErr)
	}
	if closeErr := zw.Close(); closeErr != nil {
		t.Fatal(closeErr)
	}
	got, err := readZIPBinary(bytes.NewReader(zipOutput.Bytes()), int64(zipOutput.Len()), "windows")
	if err != nil || !bytes.Equal(got, want) {
		t.Fatalf("readZIPBinary() = %q, %v", got, err)
	}
}

func TestVerifyArchiveMembers(t *testing.T) {
	t.Parallel()
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := verifyArchiveMembers(root, &target, testOptions().createdEpoch); err != nil {
