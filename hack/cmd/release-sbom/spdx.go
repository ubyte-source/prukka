...
func makeComponentSPDX(
	...
	relationships := []spdxRelationship{
		...
		{From: dashboard.ID, To: archiveSPDXFileID(rootLicenseFile), Type: relationOther, Comment: "License evidence."},
	...
	for _, file := range inventory.embedded {
		...
		relationships = append(relationships,
			...
			spdxRelationship{
				From: pkg.ID, To: archiveSPDXFileID(licenseFile), Type: relationOther, Comment: "License evidence.",
			},
		)
	}
...
func verifyComponentSPDX(
	...
	if err := verifyDashboardSPDX(packageByID, relationships, opts.version); err != nil {
		return err
	}
	if err := verifyEmbeddedComponents(packageByID, relationships, opts, target, inventory); err != nil {
		return err
	}
	if err := verifyNPMComponents(packageByID, relationships, inventory); err != nil {
		return err
	}
	return verifyNoUnexpectedComponents(packages, opts, target, inventory)
}

func verifyDashboardSPDX(
	packages map[string]spdxPackage,
	relationships []spdxRelationship,
	version string,
) error {
	dashboard := dashboardSPDXPackage(version)
	if err := verifyExactPackage(packages, &dashboard); err != nil {
		return err
	}
	if err := requireRelationship(relationships, spdxDocumentID, dashboard.ID, relationDescribes, ""); err != nil {
		return err
	}
	if err := requireRelationship(
		relationships, dashboard.ID, archiveSPDXFileID(rootLicenseFile), relationOther, "License evidence.",
	); err != nil {
		return err
	}
	return requireRelationship(relationships, archiveSPDXFileID("README.md"), dashboard.ID, "DOCUMENTATION_OF", "")
}

func verifyEmbeddedComponents(
	packages map[string]spdxPackage,
	relationships []spdxRelationship,
	opts *options,
	target *releaseTarget,
	...
	for _, file := range inventory.embedded {
		fileID := embeddedSPDXFileID(file)
		if strings.HasPrefix(file.name, "internal/webui/dist/") {
			if err := requireRelationship(relationships, dashboard.ID, fileID, relationContains, ""); err != nil {
				return err
			}
			continue
		}
		...
		driver := driverSPDXPackage(opts.version, target, file)
		if err := verifyExactPackage(packages, &driver); err != nil {
			return err
		}
		licenseFile := rootLicenseFile
		if driver.LicenseDeclared == licenseGPL2 {
			licenseFile = linuxLicenseFile
		}
		for _, relationship := range []spdxRelationship{
			{From: spdxDocumentID, To: driver.ID, Type: relationDescribes},
			{From: driver.ID, To: fileID, Type: relationContains},
			{From: driver.ID, To: archiveSPDXFileID(licenseFile), Type: relationOther, Comment: "License evidence."},
		} {
			if err := requireRelationship(
				relationships, relationship.From, relationship.To, relationship.Type, relationship.Comment,
			); err != nil {
				return err
			}
		}
...
func verifyNPMComponents(
	packages map[string]spdxPackage,
	relationships []spdxRelationship,
	inventory *componentInventory,
) error {
	dashboardID := spdxID("Package-Dashboard", projectName)
	for index := range inventory.npm {
		pkg := npmSPDXPackage(&inventory.npm[index])
		if err := verifyExactPackage(packages, &pkg); err != nil {
			return err
		}
		if err := requireRelationship(relationships, pkg.ID, dashboardID, "BUILD_DEPENDENCY_OF", ""); err != nil {
			return err
		}
		if err := requireRelationship(
			relationships, pkg.ID, archiveSPDXFileID("NOTICE.txt"), relationOther, "Notice evidence.",
		); err != nil {
			return err
		}
	}
...
func verifyNoUnexpectedComponents(
	packages []spdxPackage,
	opts *options,
	target *releaseTarget,
	inventory *componentInventory,
) error {
	expected := map[string]struct{}{dashboardSPDXPackage(opts.version).ID: {}}
	for _, file := range inventory.embedded {
		if strings.HasPrefix(file.name, "internal/devices/assets/") {
			expected[driverSPDXPackage(opts.version, target, file).ID] = struct{}{}
		}
	}
	for index := range inventory.npm {
		expected[npmPackageID(&inventory.npm[index])] = struct{}{}
	}
	for index := range packages {
		pkg := &packages[index]
		if !strings.HasPrefix(pkg.ID, "SPDXRef-Package-Dashboard-") &&
			!strings.HasPrefix(pkg.ID, "SPDXRef-Package-Driver-") &&
			!strings.HasPrefix(pkg.ID, "SPDXRef-Package-NPM-") {
			continue
		}
		if _, ok := expected[pkg.ID]; !ok {
			return fmt.Errorf("unexpected component package in SPDX: %s", pkg.ID)
		}
	}
	return nil
}
