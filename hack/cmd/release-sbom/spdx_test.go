...
// Golden identifiers computed outside the producer (sha256 over the ID
// inputs, NUL-terminated), so they cannot follow a producer regression.
const (
...
func TestMakeComponentSPDXGoldenContract(t *testing.T) {
	...
	wantLicenses := map[string]string{
		goldenDashboardID: "GPL-3.0-or-later",
...
// decodedTestSPDX is a generated document decoded back for the assertions.
	...
	inventory := map[string]spdxPackage{
		"example.com/module\x00v1.2.3": {
			...
			External: []spdxExternalRef{{
				...
				Type:     "purl",
				Locator:  "pkg:golang/example.com/module@v1.2.3",
			}},
		},
	}
