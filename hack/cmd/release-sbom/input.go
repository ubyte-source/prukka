...
// archiveContents is the single-pass inventory of one release archive: every
// member name after per-entry validation, the payloads of the repository
// files the SBOM certifies, and the runtime binary.
...
// add admits one member and, when the SBOM needs its payload, captures the
// bytes that open yields under the member's size limit. The member is always
// closed, and a close failure is reported even when the read succeeded.
...
// claim admits one member after path-safety and duplicate checks, and
// reports the payload byte limit for members whose contents the SBOM needs.
