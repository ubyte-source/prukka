// Command release-sbom generates one SPDX 2.3 document per release archive and
// verifies each against the inputs it claims to describe, then writes the
// checksum manifest that covers both.
package main
...
// Release identity, the SBOM's own limits, and the SPDX 2.3 vocabulary the
// document is written in. NOASSERTION, the checksum algorithm names, the
// relationship verbs, PACKAGE-MANAGER, purl and SPDXRef-DOCUMENT are the
// specification's spellings, not this program's choices.
const (
...
// releaseTarget is one published archive.
type releaseTarget struct {
	name, archive, goos, goarch string
	drivers                     []string
}

// The published archive set. verifyArchiveSet refuses a dist/ holding anything
// else, so a new platform is added here before it can ship.
var releaseTargets = []releaseTarget{
	{
		name:    "prukka_darwin_amd64",
		archive: "prukka_darwin_amd64.tar.gz",
		goos:    "darwin",
		goarch:  archAMD64,
		drivers: []string{"darwin/microphone.tar.gz", "darwin/speaker.tar.gz", "darwin/webcam.tar.gz"},
	},
	{
		name:    "prukka_darwin_arm64",
		archive: "prukka_darwin_arm64.tar.gz",
		goos:    "darwin",
		goarch:  "arm64",
		drivers: []string{"darwin/microphone.tar.gz", "darwin/speaker.tar.gz", "darwin/webcam.tar.gz"},
	},
	{
		name:    "prukka_linux_amd64",
		archive: "prukka_linux_amd64.tar.gz",
		goos:    osLinux,
		goarch:  archAMD64,
		drivers: []string{"linux/src.tar.gz"},
	},
	{
		name:    "prukka_linux_arm64",
		archive: "prukka_linux_arm64.tar.gz",
		goos:    osLinux,
		goarch:  "arm64",
		drivers: []string{"linux/src.tar.gz"},
	},
	{
		name:    "prukka_windows_amd64",
		archive: "prukka_windows_amd64.zip",
		goos:    "windows",
		goarch:  archAMD64,
...
// options is one invocation of the SBOM generator.
type options struct {
...
// embeddedFile is one file the binary carries or the archive ships, held
// with its bytes so its digest is computed here rather than trusted.
type embeddedFile struct {
	name string
	data []byte
}

// npmPackage is one dashboard dependency as it will be declared.
type npmPackage struct {
	path, name, version, resolved, integrity, license string
	sha512                                            [sha512.Size]byte
}

// componentInventory is everything one target declares.
...
// spdxFile is one entry of the SPDX document's files array.
...
// spdxChecksum is one algorithm/value pair on an SPDX file or package.
type spdxChecksum struct {
	Algorithm string `json:"algorithm"`
	Value     string `json:"checksumValue"`
}

// spdxPackage is one entry of the SPDX document's packages array.
...
// spdxExternalRef is an external identifier attached to an spdxPackage.
...
// spdxRelationship is one edge of the SPDX graph.
...
// run verifies every release input before producing anything, so a bad archive
// set fails before dist/ gains documents that describe it.
...
// validateOptions requires the full 40-hex object ID because an abbreviation
// is ambiguous in a repository that keeps growing, and an absolute HTTPS
// namespace base because the SPDX namespace built on it must be an absolute
// URI.
...
// verifySyft refuses to run against any Syft but the pinned one: normalization
// is written against a single scanner's output, and a different version may
// change the shape it has to normalize.
...
// generateTarget returns the SHA-256 of the archive bytes its SBOM certifies.
// The scan runs twice and the normalized bytes are compared: reproducibility
// is a property of the release, so it is proven per target rather than
// assumed.
...
// scanArchive disables Syft's update check, so a scan does not vary with what
// the update endpoint answers, and reads the result back through os.OpenRoot
// so only the directory this call created can be read from.
func scanArchive(ctx context.Context, opts *options, target *releaseTarget) (raw []byte, err error) {
	...
	cmd := exec.CommandContext(ctx, syftExecutable,
		"scan", "file:"+archive, "--source-name", target.name,
