...
func run(ctx context.Context, opts *options) error {
	...
	for index := range releaseTargets {
		...
		if err := generateTarget(ctx, opts, target); err != nil {
	...
	return writeChecksums(opts.dist)
}
...
func generateTarget(ctx context.Context, opts *options, target *releaseTarget) error {
	if err := verifyArchiveMembers(opts.dist, target, opts.createdEpoch); err != nil {
		return err
	}
	binary, err := readReleaseBinary(opts.dist, target)
	if err != nil {
		return err
	}
	inventory, err := loadComponentInventory(opts, target, binary)
	if err != nil {
		return err
	}
	...
	first, err := scanAndNormalize(ctx, opts, target, binary, inventory, archiveDigest)
	if err != nil {
		return err
	}
	second, err := scanAndNormalize(ctx, opts, target, binary, inventory, archiveDigest)
	if err != nil {
		return err
	}
	...
	return writeAtomic(filepath.Join(opts.dist, target.name+".spdx.json"), first)
}

func loadComponentInventory(opts *options, target *releaseTarget, binary []byte) (*componentInventory, error) {
	embedded, err := loadEmbeddedFiles(opts.repo, target)
	if err != nil {
		return nil, err
	}
	if verifyErr := verifyEmbeddedBytes(binary, embedded); verifyErr != nil {
		return nil, verifyErr
	}
	archived, err := loadArchiveFiles(opts.dist, opts.repo, target)
	if err != nil {
		return nil, err
	}
