...
// TestSTTProtocolRejectsFlagsOutsideTheRealContract pins the accept-set to
// the real engine's: a flag internal/speechengine does not define (the old
// --partial-stride), a rate outside its 8000..192000 band, and a tuning
// outside nativewire's 20ms..30s bounds must all be refused here too.
func TestSTTProtocolRejectsFlagsOutsideTheRealContract(t *testing.T) {
	t.Parallel()

	base := []string{subcommandSTT, "--protocol-version", "2", "--model", "fixture"}
	for _, args := range [][]string{
		{"--partial-stride", "250ms"},
		{"--rate", "4000"},
		{"--rate", "192001"},
		{"--silence-hang", "10ms"},
		{"--max-window", "31s"},
		...
		var out, stderr bytes.Buffer
		if code := realMain(append(base, args...), strings.NewReader(""), &out, &stderr); code == 0 {
			t.Fatalf("args %v were accepted; the real engine rejects them", args)
		}
	}
}
