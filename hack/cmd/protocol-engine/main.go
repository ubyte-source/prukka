...
//
// Its flag accept-set comes from internal/nativewire so the double cannot
// accept an invocation the real engine would refuse; the wire frames stay
// hand-written so the demo gate exercises the daemon's encoding, not shares it.
package main
...
const (
	...
	// Mirrors the band that lives unexported in internal/speechengine.
	minSampleRate = 8000
...
// runSTT writes the readiness line the daemon waits for before it sends any
// PCM, then one final transcript. Its end_samples must equal the samples
// consumed: the daemon resolves that count against the audio it wrote and
// rejects a value that moved backward or ran past it.
...
// sttConfig is one parsed STT invocation of the protocol double.
...
// parseSTTConfig binds only the flags nativewire names, so a flag the shipped
// helper does not declare is an error here too and the demo gate cannot pass
// on an invocation the real engine would refuse.
...
// validate checks the protocol version rather than ignoring it, so a change to
// nativewire.ProtocolVersion that leaves this double behind fails the gate
// instead of testing the daemon against a contract nothing speaks.
...
func encodeSTTResult(encoder *json.Encoder, language string, endSamples int64) error {
	...
	return encoder.Encode(struct {
		...
		Text:       "ciao dal motore di protocollo",
		Language:   lang,
		Final:      true,
...
// runMT tags the text with the target language instead of translating it, so a
// test can prove the routing and the framing without a model behind it.
...
// runTTS answers each non-empty request with one audio frame and one boundary
// frame, the two-line shape nativewire.AudioReply allows and the daemon closes
// a turn on.
...
// writeTone emits a square wave rather than silence, so a lane that dropped or
// muted the audio is distinguishable downstream from one that carried it.
