...
func parseSTTConfig(args []string) (sttConfig, error) {
	...
	config := sttConfig{}
	flags.StringVar(&config.model, "model", "", "model path")
	flags.IntVar(&config.protocol, "protocol-version", 0, "daemon/helper protocol version")
	flags.StringVar(&config.language, "language", defaultLanguage, "source language")
	flags.IntVar(&config.rate, "rate", defaultRate, "sample rate")
	flags.IntVar(&config.threads, "threads", defaultSTTThreads, "Whisper computation threads")
	flags.DurationVar(&config.silenceHang, "silence-hang", 300*time.Millisecond, "trailing silence endpoint")
	flags.DurationVar(&config.maxWindow, "max-window", 5*time.Second, "maximum STT window")
	flags.DurationVar(&config.minSpeech, "min-speech", 250*time.Millisecond, "minimum voiced audio")
	flags.DurationVar(&config.partialStride, "partial-stride", 700*time.Millisecond, "partial inference stride")
	_ = flags.Bool("fast-decode", false, "bounded-context conversational decode")
...
func (c *sttConfig) validate() error {
	...
	if c.protocol != sttProtocolVersion {
		return fmt.Errorf("stt requires --protocol-version %d", sttProtocolVersion)
	}
	if c.threads < 1 || c.threads > maxSTTThreads {
		return fmt.Errorf("stt requires --threads between 1 and %d", maxSTTThreads)
	}
	if c.invalidTiming() {
		return errors.New("stt requires positive, internally consistent timing flags")
	}

	return nil
}

func (c *sttConfig) invalidTiming() bool {
	return c.silenceHang <= 0 || c.maxWindow <= 0 || c.minSpeech <= 0 || c.partialStride <= 0 ||
		c.minSpeech > c.maxWindow || c.partialStride > c.maxWindow
}
