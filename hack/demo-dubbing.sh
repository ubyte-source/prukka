#!/usr/bin/env bash
# Dubbing demo: the Italian speech fixture streams in and
# /{session}/{lang}/audio.ts serves the English dub mixed over the ducked
# original bed. The result lands in /tmp for listening. Needs a local speech
# engine and ffmpeg (prukka setup); skips cleanly without either.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# The dub lands at the session delay (D=8s) and then drains, so the capture
# starts immediately; 25 s covers the fixture plus D plus provider latency.
...
capture_pid=$!
demo_register_job "$capture_pid" dub-capture

for _ in $(seq 1 20); do
  if grep -q "lane unavailable" "$PRUKKA_STATE/daemon.log"; then
    reason=$(grep "lane unavailable" "$PRUKKA_STATE/daemon.log" | tail -1)
    if ! demo_stop_owned_job "$capture_pid" dub-capture; then
      echo "FAIL: dubbing capture cleanup failed; state retained for recovery" >&2
      exit 1
    fi
    demo_forget_job "$capture_pid"
