...
EXCEPTIONS='^internal/gen/|/doc\.go$|^cmd/prukka/main\.go$|^internal/core/ports\.go$'
...
done < <(git ls-files '*.go' | grep -v '_test\.go$' | grep -vE "$EXCEPTIONS")
...
done < <(git ls-files '*helpers_test.go')
