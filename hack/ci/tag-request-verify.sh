...
# pipelines. It runs from a default-branch checkout (workflow_run always loads
# the workflow, and therefore this script, from the default branch) and treats
# the request file as attacker-controlled input: shape, object-id syntax, tag
# grammar, immutability since the request, and ancestry in protected main are
# all re-established here against a freshly fetched origin.
#
# Prints the verified reference as GITHUB_OUTPUT lines; the caller redirects
# them, so a failure appends nothing.
set -euo pipefail
...
[[ -f "$FILE" ]]
[[ $(wc -c < "$FILE") -le 512 ]]
# Read portably rather than with mapfile: every script here also runs on a
# maintainer's macOS bash 3.2.
request=()
