#!/usr/bin/env bash
# Install the pinned Node.js toolchain into .tools/node and link node/npm/npx
# into .tools/bin. Node is a BUILD-time dependency only. The tarball is
# verified against the checksum committed in tools/versions.mk before
# unpacking.
set -euo pipefail
...
[[ "$VERSION" =~ ^v(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)\.(0|[1-9][0-9]*)$ ]] || {
  echo "invalid Node.js version: $VERSION" >&2
  exit 1
}
...
[[ "$EXPECTED" =~ ^[0-9a-f]{64}$ ]] || { echo "invalid Node.js checksum" >&2; exit 1; }
...
curl --proto '=https' --tlsv1.2 -fsSL --retry 4 --retry-delay 5 \
  "$BASE/$NAME.tar.gz" -o "$TMP/$NAME.tar.gz"
