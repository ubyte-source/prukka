#!/usr/bin/env bash
# bundled-gate.sh — prove the bundleddrivers build tag, the only configuration
# that ships, compiles, lints and tests. The real driver archives exist only at
# release time, so a go overlay maps the five embedded payload paths onto one
# placeholder. The overlay rides GOFLAGS for golangci-lint, which has no
# -overlay flag; the tag rides the CLI, never .golangci.yml, which is
# byte-anchored by LINTER.sha256.
set -euo pipefail
