...
# binary, so unlike the other pinned linters it cannot be `go install`-ed:
# the release archive is downloaded and verified against the checksum
# committed in tools/versions.mk before it is unpacked.
set -euo pipefail
