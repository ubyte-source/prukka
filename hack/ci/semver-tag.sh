...
# identifiers. tag-request.sh, tag-request-verify.sh (and through them every
# release and engine workflow) and proto-breaking-gate.sh all validate through
# this file, so the contract cannot drift between call sites.
set -euo pipefail
