...
# is_semver defers to the shared release-tag contract; candidates that fail
# it are simply not release tags. Anchored to the script's own directory:
# the gate test drives this file from a scratch repository.
is_semver() {
...
# flags implicit-to-explicit presence purely as a generated-code change. The
# second pass restores the wire-relevant half of that rule: WIRE_JSON runs it
# too, admitting only the presence transition and still rejecting every
# cardinality change that alters the bytes or the JSON (repeated to singular,
# map to repeated).
"$buf_bin" breaking proto --against ".git#tag=${baseline},subdir=proto" \
