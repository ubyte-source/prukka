...
repo_root=${PRUKKA_REPO_ROOT:-"$script_dir/../.."}
cd "$repo_root"

# Anchored to the script's own directory: the gate test drives this file from
# a scratch repository.
is_semver() {
  "$script_dir/semver-tag.sh" "$1" >/dev/null 2>&1
}

# semver_compare implements SemVer precedence directly; generic "version sort"
# disagrees for alphanumeric prerelease identifiers.
semver_compare() {
...
while IFS= read -r candidate; do
  ...
  if [[ -n "$current_tag" ]]; then
    ...
    # Tags on HEAD are aliases of the sources under review.
  ...
  if (( comparison > 0 )); then
    ...
    # Build metadata has no precedence, so pick a stable spelling rather than
    # depend on ref iteration order.
...
# proto/buf.yaml excepts FIELD_SAME_CARDINALITY from the FILE category, which
# flags implicit-to-explicit presence purely as a generated-code change. This
# second pass restores the wire-relevant half: WIRE_JSON runs the rule too,
# admitting only the presence transition.
"$buf_bin" breaking proto --against ".git#tag=${baseline},subdir=proto" \
  --config '{"version":"v2","modules":[{"path":"proto"}],"breaking":{"use":["WIRE_JSON"],"ignore":["proto/google"]}}'
