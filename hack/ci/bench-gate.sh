...
run_suite() {
  ...
  out=$(GOMAXPROCS=1 go test -run '^$' -bench "$pattern" -benchmem -benchtime 2000x "$package")
  echo "$out"
...
run_suite ./internal/speechengine '^BenchmarkS16StreamDecoderDecode$' 'engine STT decoder'
