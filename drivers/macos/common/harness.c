...
int main(int argc, char **argv) {
	...
	// sample-time-to-ring mapping. Assert the value, not just the plumbing.
	{
		AudioObjectPropertyAddress periodAddr =
		    Addr(kAudioDevicePropertyZeroTimeStampPeriod, global);
		UInt32 period = 0, periodSize = 0;
		CHECK(iface->GetPropertyData(driver, kDevice, 0, &periodAddr, 0, NULL,
		                             sizeof(period), &periodSize, &period) == 0,
		      "GetPropertyData(ZeroTimeStampPeriod)");
		CHECK(period == kRing_Frames, "zero-timestamp period %u != ring frames %u",
		      period, (UInt32)kRing_Frames);
	}
	...
	AudioObjectPropertyScope scopes[2] = {input, kAudioObjectPropertyScopeOutput};
	const char *scopeLabels[2] = {"device/input", "device/output"};
	for (size_t s = 0; s < 2; s++) {
		for (size_t i = 0; i < sizeof(scopedSelectors) / sizeof(*scopedSelectors); i++) {
			checkProperty(driver, kDevice, Addr(scopedSelectors[i], scopes[s]),
			              scopeLabels[s]);
		}
	}
	...
	{
		AudioObjectPropertyAddress nameAddr = Addr(kAudioObjectPropertyName, global);
		UInt32 bogusSize = 0;
		CFStringRef bogusName = NULL;
		CHECK(!iface->HasProperty(driver, 99, 0, &nameAddr),
		      "HasProperty answered for a bogus object");
		CHECK(iface->GetPropertyDataSize(driver, 99, 0, &nameAddr, 0, NULL, &bogusSize) ==
		          kAudioHardwareBadObjectError,
		      "GetPropertyDataSize did not reject a bogus object");
		CHECK(iface->GetPropertyData(driver, 99, 0, &nameAddr, 0, NULL, sizeof(bogusName),
	...
	iface->GetPropertyData(driver, kStreamIn, 0, &dir, 0, NULL, sizeof(direction), &used, &direction);
	CHECK(direction == 1, "input stream direction = %u, want 1", direction);
	iface->GetPropertyData(driver, kStreamOut, 0, &dir, 0, NULL, sizeof(direction), &used, &direction);
	...
	// kChannels and kRing_Frames come from contract.h; only the cycle
	// length is the harness's own choice.
	enum { kFrames = 512 };
	...
	cycle.mOutputTime.mSampleTime = 4096 + kRing_Frames;
	CHECK(iface->DoIOOperation(driver, kDevice, kStreamOut, 1,
	                           kAudioServerPlugInIOOperationWriteMix, kFrames, &cycle,
	                           outBuffer, NULL) == 0,
	...
	cycle.mInputTime.mSampleTime = 4096 + kRing_Frames;
	CHECK(iface->DoIOOperation(driver, kDevice, kStreamIn, 1,
	                           kAudioServerPlugInIOOperationReadInput, kFrames, &cycle,
	                           inBuffer, NULL) == 0,
	...
	cycle.mInputTime.mSampleTime = 4096 + 3 * kRing_Frames;
	CHECK(iface->DoIOOperation(driver, kDevice, kStreamIn, 1,
	                           kAudioServerPlugInIOOperationReadInput, kFrames, &cycle,
	                           inBuffer, NULL) == 0,
	...
	CHECK(iface->StopIO(driver, kDevice, 1) == 0, "StopIO (re-anchor)");
	CHECK(iface->StartIO(driver, kDevice, 1) == 0, "StartIO (re-anchor)");
	cycle.mOutputTime.mSampleTime = -256;
	CHECK(iface->DoIOOperation(driver, kDevice, kStreamOut, 1,
	                           kAudioServerPlugInIOOperationWriteMix, kFrames, &cycle,
	                           outBuffer, NULL) == 0,
	...
	CHECK(memcmp(inBuffer, outBuffer + 256 * kChannels,
	             256 * kChannels * sizeof(Float32)) == 0,
	      "pre-anchor frames were shifted instead of dropped");
	sum = 0;
	for (int i = 256 * kChannels; i < kFrames * kChannels; i++) sum += fabsf(inBuffer[i]);
	...
	cycle.mInputTime.mSampleTime = -256;
	CHECK(iface->DoIOOperation(driver, kDevice, kStreamIn, 1,
	                           kAudioServerPlugInIOOperationReadInput, kFrames, &cycle,
	                           inBuffer, NULL) == 0,
	      "ReadInput (straddling the anchor)");
	sum = 0;
	for (int i = 0; i < 256 * kChannels; i++) sum += fabsf(inBuffer[i]);
	CHECK(sum == 0, "pre-anchor capture span not silent (residual %f)", sum);
	CHECK(memcmp(inBuffer + 256 * kChannels, outBuffer + 256 * kChannels,
	             256 * kChannels * sizeof(Float32)) == 0,
	...
	cycle.mOutputTime.mSampleTime = kRing_Frames - kFrames / 2;
	cycle.mInputTime.mSampleTime = kRing_Frames - kFrames / 2;
	iface->DoIOOperation(driver, kDevice, kStreamOut, 1, kAudioServerPlugInIOOperationWriteMix,
	                     kFrames, &cycle, outBuffer, NULL);
	iface->DoIOOperation(driver, kDevice, kStreamIn, 1, kAudioServerPlugInIOOperationReadInput,
	                     kFrames, &cycle, inBuffer, NULL);
	...
	UInt64 skewInputStart = 5 * kRing_Frames - kSkewFrames;
	...
	UInt64 resumeInput = skewInputStart + 4 * kRing_Frames;
