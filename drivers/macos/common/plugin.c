...
static Float32 gRing[kRing_Frames * kChannels];
...
static ULONG Plugin_AddRef(void *inDriver) { return ++gRefCount; }
static ULONG Plugin_Release(void *inDriver) { return gRefCount > 0 ? --gRefCount : 0; }
...
#define RETURN_VALUE(type, value)                                                                  \
	...
	if (IsDevice(inObjectID)) {
		...
		case kAudioDevicePropertyDeviceIsRunning:
			RETURN_VALUE(UInt32, gIOCount > 0 ? 1 : 0);
		case kAudioDevicePropertyDeviceCanBeDefaultDevice:
			RETURN_VALUE(UInt32, 1);
		case kAudioDevicePropertyDeviceCanBeDefaultSystemDevice:
			RETURN_VALUE(UInt32, 0);
...
// DoIOOperation is the loopback: WriteMix lands the dub in the ring at its
// sample time; ReadInput hands the same timeline to the call app and clears
// what it consumed so stale audio never loops.
static OSStatus Plugin_DoIOOperation(AudioServerPlugInDriverRef inDriver,
	...
	if (inOperationID == kAudioServerPlugInIOOperationWriteMix) {
		...
		for (UInt32 frame = 0; frame < inIOBufferFrameSize; frame++) {
			UInt64 position = ((start + frame) % kRing_Frames) * kChannels;

			for (UInt32 ch = 0; ch < kChannels; ch++) {
				gRing[position + ch] = samples[frame * kChannels + ch];
			}
		}
	...
	if (inOperationID == kAudioServerPlugInIOOperationReadInput) {
		...
		for (UInt32 frame = 0; frame < inIOBufferFrameSize; frame++) {
			...
			for (UInt32 ch = 0; ch < kChannels; ch++) {
				samples[frame * kChannels + ch] = gRing[position + ch];
				gRing[position + ch] = 0;
			}
		}
