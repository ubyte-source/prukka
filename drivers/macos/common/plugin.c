...
// lifecycle work bounded and allocation-free; 256 simultaneous clients is far
// above a practical audio-device fanout, and exhaustion fails the new client
// without disturbing existing IO.
...
static void StopIOClientLocked(IOClientState *client) {
	...
	UInt32 count = atomic_load_explicit(&gIOCount, memory_order_relaxed);
	if (count > 0) {
		atomic_store_explicit(&gIOCount, count - 1, memory_order_relaxed);
	}
}
