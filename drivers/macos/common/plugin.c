...
// Object IDs, ring geometry and the client-ledger bound are the proven
// contract: the harness includes the same header and checks these values.
#include "contract.h"
...
// gHost is retained for future PropertiesChanged/host notifications;
// currently unused.
static AudioServerPlugInHostRef gHost = NULL;
static pthread_mutex_t gStateMutex = PTHREAD_MUTEX_INITIALIZER;
// gIOMutex guards only the clock anchor, mirroring the state/IO mutex split
// in Apple's NullAudio sample: GetZeroTimeStamp runs on the HAL's timing
// thread every ring lap and must never wait behind lifecycle work — StartIO
// holds gStateMutex across a ledger scan and, at 0->1, a full ring clear.
static pthread_mutex_t gIOMutex = PTHREAD_MUTEX_INITIALIZER;
// gIOCount is written only under gStateMutex; it is atomic solely so the
// lock-free DeviceIsRunning read (GetPropertyData) can observe it.
static _Atomic UInt32 gIOCount = 0;

// CoreAudio assigns a stable ID to each device client before it can start IO.
// Keep running state by that identity rather than treating StartIO/StopIO as
// anonymous counter deltas: duplicate delivery must not leak a phantom runner
// or falsely stop/re-anchor a different live client. The fixed ledger keeps
// lifecycle work bounded and allocation-free; kMaxIOClients (contract.h)
// bounds it, and exhaustion fails the new client without disturbing
// existing IO.
typedef struct {
	UInt32 id;
	bool occupied;
...
static IOClientState gIOClients[kMaxIOClients];

static Float64 gAnchorSampleTime = 0;
static UInt64 gAnchorHostTime = 0;
static Float64 gHostTicksPerFrame = 0;
// gAnchorSeed tells the HAL the device clock re-anchored: coreaudiod detects
// zero-time-stamp discontinuities only through a changed seed, so a constant
// seed after a re-anchor desynchronizes every already-running client's sample
// times from the ring generations — permanent silence for those clients. The
// first StartIO anchors with seed 1; every later 0->1 restart bumps it.
static UInt64 gAnchorSeed = 0;

static IOClientState *FindIOClientLocked(UInt32 id) {
	for (size_t i = 0; i < kMaxIOClients; i++) {
		if (gIOClients[i].occupied && gIOClients[i].id == id) {
			return &gIOClients[i];
		}
	}
	return NULL;
...
static IOClientState *RegisterIOClientLocked(UInt32 id) {
	IOClientState *existing = FindIOClientLocked(id);
	if (existing != NULL) {
		return existing;
	}

	for (size_t i = 0; i < kMaxIOClients; i++) {
		if (!gIOClients[i].occupied) {
			gIOClients[i] = (IOClientState){.id = id, .occupied = true, .running = false};
			return &gIOClients[i];
		}
	}
	return NULL;
...
static void StopIOClientLocked(IOClientState *client) {
...
// The driver is a static singleton and is never deallocated; the refcount
// is bookkeeping only, so relaxed ordering carries no destruction hazard.
...
// Relaxed ordering is sufficient because the generation tag and the sample
// payload live in ONE atomic 64-bit word: a reader can never observe a
// generation without its matching payload, and no other memory is published
// through the ring, so no acquire/release edge is needed. A reader that has
// not yet observed a write sees the previous whole word and degrades to the
// defined semantics (silence or the prior lap). The CAS loop makes
// concurrent mixes land in modification order; a mid-mix read returning a
// partial sum for one sample is inherent and acceptable.
// (generation - currentGeneration) > UINT32_MAX / 2 is serial-number
// arithmetic (RFC 1982): "older than" across uint32 wrap.
...
// QueryInterface speaks COM, not CoreAudio: failures must be NEGATIVE
// HRESULTs. An OSStatus like kAudioHardwareBadObjectError is a positive
// fourcc, so SUCCEEDED() on it would be true and the host would use a
// garbage interface pointer.
...
static OSStatus Plugin_AddDeviceClient(AudioServerPlugInDriverRef inDriver,
	...
	IOClientState *client = RegisterIOClientLocked(inClientInfo->mClientID);
	pthread_mutex_unlock(&gStateMutex);

	return client != NULL ? kAudioHardwareNoError : kAudioHardwareUnspecifiedError;
}
...
static OSStatus Plugin_RemoveDeviceClient(AudioServerPlugInDriverRef inDriver,
	...
	pthread_mutex_lock(&gStateMutex);
	IOClientState *client = FindIOClientLocked(inClientInfo->mClientID);
	if (client != NULL) {
		StopIOClientLocked(client);
		*client = (IOClientState){0};
	}
	pthread_mutex_unlock(&gStateMutex);
...
// Plugin_HasProperty is the ONE property map: GetPropertyDataSize and
// GetPropertyData consult it before answering, so the three functions can
// never disagree on which object serves which selector.
static Boolean Plugin_HasProperty(AudioServerPlugInDriverRef inDriver,
	...
	if (!IsPlugIn(inObjectID) && !IsDevice(inObjectID) && !IsStream(inObjectID)) {
		return false;
	}
...
// PropertyGate maps a query the property map rejects onto the contract's
// error: an unknown object is kAudioHardwareBadObjectError, a known object
// without that selector is kAudioHardwareUnknownPropertyError.
static OSStatus PropertyGate(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID,
                             pid_t inClientPID, const AudioObjectPropertyAddress *inAddress) {
	if (Plugin_HasProperty(inDriver, inObjectID, inClientPID, inAddress)) {
		return kAudioHardwareNoError;
	}
	if (!IsPlugIn(inObjectID) && !IsDevice(inObjectID) && !IsStream(inObjectID)) {
		return kAudioHardwareBadObjectError;
...
static OSStatus Plugin_IsPropertySettable(AudioServerPlugInDriverRef inDriver,
	...
	OSStatus gate = PropertyGate(inDriver, inObjectID, inClientPID, inAddress);
	if (gate != kAudioHardwareNoError) {
		return gate;
	}
...
static OSStatus Plugin_StartIO(AudioServerPlugInDriverRef inDriver,
	...
	pthread_mutex_lock(&gStateMutex);
	IOClientState *client = FindIOClientLocked(inClientID);
	if (client == NULL) {
		pthread_mutex_unlock(&gStateMutex);
		return kAudioHardwareIllegalOperationError;
	}
	...
	if (atomic_load_explicit(&gIOCount, memory_order_relaxed) == 0) {
		// The anchor rewrite is the only other gIOMutex holder; the ring
		// clear stays outside it (the ring is atomic on its own) so the
		// timing thread never waits behind 131,072 stores.
...
static OSStatus Plugin_StopIO(AudioServerPlugInDriverRef inDriver,
	...
	pthread_mutex_lock(&gStateMutex);
	StopIOClientLocked(FindIOClientLocked(inClientID));
	pthread_mutex_unlock(&gStateMutex);
...
static OSStatus Plugin_DoIOOperation(AudioServerPlugInDriverRef inDriver,
	...
	if (inOperationID == kAudioServerPlugInIOOperationWriteMix) {
		// Sample times can be negative in the first cycles after an anchor.
		// Pre-anchor frames are DROPPED per frame, never clamped: clamping
		// the cycle start to 0 would relocate the whole buffer, and the
		// misplaced tail would later sum with the legitimate write for
		// those slots — doubled, shifted audio on the first lap.
		Float64 base = inIOCycleInfo->mOutputTime.mSampleTime;

		for (UInt32 frame = 0; frame < inIOBufferFrameSize; frame++) {
			Float64 t = base + (Float64)frame;
			if (t < 0) {
				continue;
			}
			UInt64 sampleFrame = (UInt64)t;
	...
	if (inOperationID == kAudioServerPlugInIOOperationReadInput) {
		// Pre-anchor frames read as silence at their own positions; the
		// same per-frame skip as WriteMix keeps capture aligned.
