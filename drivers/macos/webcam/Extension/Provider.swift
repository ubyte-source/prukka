...
class CameraDeviceSource: NSObject, CMIOExtensionDeviceSource {
    ...
    // while it stays fresh. A feeder that dies without stopping the sink
    // must not freeze its final frame on every call forever.
    private let frameLock = NSLock()
    private var latestFrame: CVPixelBuffer?
    private var latestFrameAt: CFTimeInterval = 0
    private let frameFreshness: CFTimeInterval = 1.0
    ...
    func consume(_ sampleBuffer: CMSampleBuffer) {
        ...
        latestFrameAt = CMClockGetTime(CMClockGetHostTimeClock()).seconds
        frameLock.unlock()
    }

    // emitFrame serves the feeder's latest frame while fresh, or the splash.
    private func emitFrame() {
        let now = CMClockGetTime(CMClockGetHostTimeClock()).seconds
        frameLock.lock()
        let fresh = now - latestFrameAt < frameFreshness
        let frame = fresh ? latestFrame : nil
        frameLock.unlock()
