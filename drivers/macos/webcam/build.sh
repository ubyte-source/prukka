...
# The camera extension calls CMIOExtensionProvider (macOS 12.3+), so this
# bundle ships on a higher floor than the other native components; checks.sh
# owns the shared default and gates every produced binary against whichever
# floor is in force.
...
universal() {
  out="$1"; shift
  xcrun swiftc -O -sdk "$SDK" -target "x86_64-apple-macos$MACOSX_DEPLOYMENT_TARGET" "$@" -o "$out.x86_64"
  xcrun swiftc -O -sdk "$SDK" -target "arm64-apple-macos$MACOSX_DEPLOYMENT_TARGET" "$@" -o "$out.arm64"
...
for bin in "$SYSEXT/Contents/MacOS/$EXT_ID" \
  ...
  check_minos "$bin" "$(basename "$bin")" x86_64 arm64
done
