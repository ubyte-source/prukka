...
static bool InstalledPaths(wchar_t* dir, size_t dirCount, wchar_t* dll, size_t dllCount) {
    ...
    if (swprintf_s(dir, dirCount, L"%s\\Prukka", programData) < 0 ||
...
static int Install() {
    ...
    // plugged — this process holds it.
    HANDLE mapping = nullptr;
    prukka::SharedFrame* shared = prukka::CreateShared(&mapping);
    if (shared == nullptr) {
...
static int Uninstall() {
    ...
    const bool cameraFailed = FAILED(cameraResult) &&
                              HRESULT_CODE(cameraResult) != ERROR_PROC_NOT_FOUND;
    if (cameraFailed) {
