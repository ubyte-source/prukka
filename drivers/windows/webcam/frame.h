...
// after (paired InterlockedIncrement calls, each a full barrier); a
// reader that sees an odd or changed counter retries. Readers must load
// the counter with ReadAcquire before the copy and fence before the
// recheck — plain volatile loads carry no ordering on ARM64.
struct SharedFrame {
...
inline SharedFrame* CreateShared(HANDLE* mapping) {
    ...
    *mapping = CreateFileMappingW(INVALID_HANDLE_VALUE, &security, PAGE_READWRITE, 0,
    ...
    // Captured immediately: last-error is only defined right after
    // CreateFileMappingW, and the LocalFree below may clobber it.
    ...
    const bool fresh = createError != ERROR_ALREADY_EXISTS;
