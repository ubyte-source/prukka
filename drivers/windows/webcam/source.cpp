...
// The frame server queries IKsControl on the source and on its pin, and
// neither exposes KS property, method or event sets. Both objects have to
// implement the interface themselves, so the shared answer is one function
// plus the macro that fills the three vtable slots.
static HRESULT KsSetNotFound(ULONG* written) {
    if (written != nullptr) {
        *written = 0;
    }
...
class Stream final : public RuntimeClass<RuntimeClassFlags<ClassicCom>, IMFMediaStream2,
  ...
  public:
    ...
    IFACEMETHODIMP RequestSample(IUnknown* token) override {
        ...
        // the last copy rather than stall the pipeline. The explicit
        // acquire load and fence pair with the feeder's Interlocked
        // increments (frame.h): plain volatile loads are only ordered on
        // x64's /volatile:ms default, and on ARM64 (/volatile:iso) the
        // pixel copy could be reordered across the sequence loads,
        // letting torn frames pass the check.
        for (int attempt = 0; attempt < 4; attempt++) {
            const LONG before = ReadAcquire(&shared_->sequence);
            memcpy(data, const_cast<const uint8_t*>(shared_->pixels), kFrameBytes);
            MemoryBarrier();
            if ((before & 1) == 0 && ReadNoFence(&shared_->sequence) == before) {
                break;
            }
        }
