...
STDMETHODIMP_(NTSTATUS) CWaveStream::SetState(KSSTATE state)
{
	...
	if (leavingRun) {
		...
		// KeFlushQueuedDpcs requires PASSIVE_LEVEL, so it must run
		// outside the fast mutex, which raises to APC_LEVEL. Per-stream
		// SetState calls are serialized by portcls, so the cancel/flush
		// stays ordered against any following run transition.
		KeFlushQueuedDpcs();
		Release();
	}
...
CWaveStream::AllocateBuffer(ULONG size, PPHYSICAL_ADDRESS)
{
	...
	m_Buffer = static_cast<PUCHAR>(ExAllocatePool2(
		POOL_FLAG_NON_PAGED, size, PRUKKA_POOL_TAG));
...
{
	if (m_Buffer != nullptr) {
		ExFreePoolWithTag(m_Buffer, PRUKKA_POOL_TAG);
		m_Buffer = nullptr;
		m_Allocated = 0;
		m_BufferSize = 0;
	}
...
STDMETHODIMP_(void) CWaveStream::CopyTo(PVOID dest, PVOID source, ULONG bytes)
{
	...
	// At most two bulk moves, split at the wrap: this runs at
	// DISPATCH_LEVEL under the shared ring lock, so a per-byte modulo
	// loop would hold every other stream off the ring for thousands of
	// iterations per copy-engine pass. AllocateBuffer clamps the DMA
	// buffer to the ring size, so bytes never exceeds one lap.
	const UCHAR* src = static_cast<const UCHAR*>(source);
	const ULONG pos =
		static_cast<ULONG>(g_Ring.written % PRUKKA_RING_BYTES);
	const ULONG first =
		min(bytes, static_cast<ULONG>(PRUKKA_RING_BYTES) - pos);
	RtlCopyMemory(&g_Ring.data[pos], src, first);
	RtlCopyMemory(&g_Ring.data[0], src + first, bytes - first);
					  ...
					  ULONG bytes)
{
	...
	// Bulk moves split at the wrap (see CopyTo): drain what the writer
	// produced, zero the consumed span so a stopped writer yields
	// silence instead of stale audio, and silence-fill the remainder.
	const ULONG drained = static_cast<ULONG>(
		min(static_cast<ULONGLONG>(bytes), available));
	const ULONG pos = static_cast<ULONG>(g_Ring.read % PRUKKA_RING_BYTES);
	const ULONG first =
		min(drained, static_cast<ULONG>(PRUKKA_RING_BYTES) - pos);
	RtlCopyMemory(out, &g_Ring.data[pos], first);
	RtlZeroMemory(&g_Ring.data[pos], first);
	RtlCopyMemory(out + first, &g_Ring.data[0], drained - first);
	RtlZeroMemory(&g_Ring.data[0], drained - first);
	RtlZeroMemory(out + drained, bytes - drained);
					   ...
					   PULONG resultantSize)
{
	...
	*resultantSize = need;
	...
	// Only claim a match when the client's range actually covers the one
	// fixed format; answering 48k/2ch/16-bit to a 44.1k-mono range would
	// stream garbage.
	if (clientRange->FormatSize >= sizeof(KSDATARANGE_AUDIO)) {
		const auto* audio =
			reinterpret_cast<const KSDATARANGE_AUDIO*>(clientRange);
		if (audio->MinimumSampleFrequency > static_cast<ULONG>(PRUKKA_RATE) ||
		    audio->MaximumSampleFrequency < static_cast<ULONG>(PRUKKA_RATE) ||
		    audio->MaximumChannels < static_cast<ULONG>(PRUKKA_CHANNELS) ||
		    audio->MinimumBitsPerSample > static_cast<ULONG>(PRUKKA_BITS) ||
		    audio->MaximumBitsPerSample < static_cast<ULONG>(PRUKKA_BITS)) {
			       ...
			       PSERVICEGROUP* serviceGroup)
{
	...
	// A stream may only open with the one fixed format the filter
	// advertises; SetFormat relies on this validation having happened.
	if (format == nullptr ||
	    format->FormatSize < sizeof(KSDATAFORMAT_WAVEFORMATEX)) {
		return STATUS_INVALID_PARAMETER;
	}
	const auto* wfx =
		reinterpret_cast<const KSDATAFORMAT_WAVEFORMATEX*>(format);
	if (wfx->WaveFormatEx.nChannels != PRUKKA_CHANNELS ||
	    wfx->WaveFormatEx.nSamplesPerSec != PRUKKA_RATE ||
	    wfx->WaveFormatEx.wBitsPerSample != PRUKKA_BITS) {
		return STATUS_NO_MATCH;
	}

	auto* wave = new (POOL_FLAG_NON_PAGED, PRUKKA_POOL_TAG) CWaveStream(outer);
...
{
	auto* miniport =
		new (POOL_FLAG_NON_PAGED, PRUKKA_POOL_TAG) CMiniportWaveCyclic(nullptr);
