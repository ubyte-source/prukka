...
class CWaveStream : public IMiniportWaveCyclicStream,
  ...
  private:
	...
	KTIMER m_Timer;
	KDPC m_Dpc;
	// QPC anchor from the last run transition; position derives from it.
	LARGE_INTEGER m_Anchor = {};
...
STDMETHODIMP_(NTSTATUS) CWaveStream::SetState(KSSTATE state)
{
	if (state == KSSTATE_RUN && m_State != KSSTATE_RUN) {
		m_Anchor = KeQueryPerformanceCounter(&m_QpcFrequency);
...
{
	if (m_State != KSSTATE_RUN || m_BufferSize == 0) {
		*position = 0;
	...
	ULONGLONG elapsed = now.QuadPart - m_Anchor.QuadPart;
	ULONGLONG bytes = (elapsed * PRUKKA_BYTES_PER_SEC) /
			  m_QpcFrequency.QuadPart;
	// Whole frames only, wrapped onto the cyclic buffer.
	bytes -= bytes % PRUKKA_BLOCK_ALIGN;
	*position = static_cast<ULONG>(bytes % m_BufferSize);
					  ...
					  ULONG bytes)
{
	...
	ULONGLONG available = g_Ring.written - g_Ring.read;
