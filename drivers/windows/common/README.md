...
[`../audio`](../audio) each build this core with their own INF
(their `.vcxproj` compiles `../common/*.cpp`); the core is name-agnostic, and
each driver's identity is the FriendlyName/DeviceDesc pair in its INF
`[Strings]` section. Fix the core once, both drivers
get it.
