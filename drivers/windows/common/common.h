...
// around. Identity comes from each driver's INF Strings section
// (FriendlyName/DeviceDesc); this core is name-agnostic.
//
// The format is fixed — 48 kHz, stereo, 16-bit PCM — one known-good
// shape instead of a negotiation matrix.
#pragma once

// stdunk.h ships deprecated POOL_TYPE operator new/delete declarations;
// _NEW_DELETE_OPERATORS_ keeps them out. Allocation goes through the
// pool-flags operator new below (defined in adapter.cpp), deallocation
// through stdunk.lib's operator delete plus the sized form below.
...
// Every allocation this driver makes carries one tag; pool dumps print it
// reversed, so 'kurP' reads as "Pruk".
#define PRUKKA_POOL_TAG 'kurP'
