...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			cmd.Printf("config: %s (state: %s)\n\n", configSource(flags), paths.StateDir())
...
func configSource(flags *rootFlags) string {
	...
	return paths.DefaultPath() + " (or built-in defaults)"
}
...
func deviceVerdict(results []devices.Result) doctor.Check {
	...
	// The install command comes before any per-device note: on Windows the
	// audio drivers always read manual, which would bury the first step.
	if installed == 0 {
		return doctor.Check{
			Name:   devicesName,
...
// deviceAttention words the warning a device state calls for, or "" for none.
...
func daemonCheck(cmd *cobra.Command, flags *rootFlags) doctor.Check {
	...
	if err != nil {
		return doctor.Check{Name: daemonName, Status: doctor.StatusWarn, Detail: err.Error()}
	}
