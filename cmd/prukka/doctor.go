...
func daemonCheck(cmd *cobra.Command, flags *rootFlags) doctor.Check {
	...
	if err != nil {
		// withControl already words the cause and its next step; wrapping it
		// in a second hint only produced a line long enough for the
		// actionable tail to wrap off the terminal.
