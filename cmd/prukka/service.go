...
func serviceOptions(flags *rootFlags, now bool) (*service.Options, error) {
	...
	// The unit carries the config path into a supervisor that starts the
	// daemon from its own working directory — / for launchd, $HOME for a
	// systemd user unit, system32 for a scheduled task — so a relative path
	// taken from the installing shell would resolve somewhere else and
	// crash-loop the daemon on a config it cannot find.
	config := flags.config
	if config != "" {
	...
	return &service.Options{ExecPath: exe, ConfigPath: config, Now: now}, nil
}
