...
func newServiceInstallCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if printOnly {
				path, content, renderErr := osservice.Rendered(opts)
				if renderErr != nil {
					return renderErr
				}
			...
			if installErr := osservice.Install(cmd.Context(), opts); installErr != nil {
				return installErr
			}
...
// lingerNote surfaces the systemd lingering requirement: without it the
// per-user daemon stops at logout and never starts at boot.
func lingerNote() string {
	if runtime.GOOS != hostos.Linux {
		return ""
	}
...
func serviceOptions(flags *rootFlags, now bool) (*osservice.Options, error) {
	...
	// The supervisor starts the daemon from its own working directory, so a
	// relative path taken from the installing shell would resolve elsewhere.
	config := flags.config
	if config != "" {
	...
	return &osservice.Options{ExecPath: exe, ConfigPath: config, Now: now}, nil
}
...
func newServiceRemoveCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if err := osservice.Remove(cmd.Context()); err != nil {
				return err
			}
...
func newServiceRestartCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			if err := osservice.Restart(cmd.Context()); err != nil {
...
func newServiceStatusCmd() *cobra.Command {
	return &cobra.Command{
		...
		Args:  cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			state, err := osservice.Status(cmd.Context())
			if err != nil {
				return err
			}
