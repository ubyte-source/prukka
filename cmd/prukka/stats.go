...
func newStatsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "stats",
		Short: "Show daemon statistics (sessions, uptime, cost)",
		Args:  cobra.NoArgs,
...
func printStats(cmd *cobra.Command, flags *rootFlags) error {
	return withControl(cmd, flags, func(ctx context.Context, client v1.ControlClient) error {
		...
		cmd.Printf("sessions: %d · cost: €%.4f/h · uptime: %s · daemon: %s\n",
			resp.GetSessionsActive(), resp.GetCostEurPerHour(), uptime, resp.GetVersion())
