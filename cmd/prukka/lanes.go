...
func (o laneOutputs) drop(slug string) {
	o.vtt.Drop(slug)
	o.audio.Drop(slug)
	o.hls.Drop(slug)
}
...
func (o laneOutputs) scrub(slug string) {
	o.vtt.Drop(slug)
	o.audio.Reset(slug)
	o.hls.Drop(slug)
}
...
func newTranscriber(
	...
	return native.NewSTT(&native.STTConfig{
		...
		Model: sttModel(localEngine.STT, profile), Rate: pipeline.SampleRate,
...
func newSynthesizer(cfg *config.Config, log *slog.Logger) (*native.TTS, []core.Voice, error) {
	...
	return native.NewTTS(&native.TTSConfig{
		Log: log, Bin: localEngine.Bin, EngineRoot: engineRoot, Rate: pipeline.SampleRate,
...
func ingressFor(url string, profile session.Profile, log *slog.Logger) (core.Ingress, error) {
	...
	case "rtmp", "srt", "device":
		...
		if helper := ffmpeg.MicCaptureBinary(speech.BundleRoot(config.StateDir())); helper != "" {
