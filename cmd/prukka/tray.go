...
// newTrayCmd runs the system tray companion against a separately running
// daemon.
func newTrayCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			launch, display := dashboardURL("http://" + cfg.Daemon.HTTP)
...
// controlStats adapts the control plane to tray.StatsSource, dialing per poll
// so a daemon restart is survivable.
...
func (c *controlStats) Stats(ctx context.Context) (tray.Stats, error) {
	...
	return tray.Stats{Sessions: int(resp.GetSessionsActive())}, nil
}
