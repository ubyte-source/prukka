...
func newSessionAddCmd(flags *rootFlags) *cobra.Command {
	...
	// and the daemon rejects a negative delay. Changed() carries not-set.
...
func printSessions(cmd *cobra.Command, sessions []*v1.Session) error {
	...
	// ERROR trails every fixed-width column: the daemon's failure detail is
	// unbounded, and mid-table it pushes the rest of the row past the fold
	// exactly when a session has failed and you need to read it.
...
// pushRetryable reports the daemon's tagged not-ready answer — the window
// while the dubbed track is still being built. A bare codes.Unavailable is a
// transport failure (stopped daemon, refused socket) and retrying it for the
// whole budget would just spin.
...
// newSessionLangsCmd hot-adds and hot-removes languages. Interspersed
// parsing is off so "-de" AFTER the slug reads as a removal, while inherited
// flags before the subcommand (--config, --log-level) and --help keep
// working:
//
