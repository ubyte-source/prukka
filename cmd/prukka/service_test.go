...
// TestServiceOptionsAbsolutizeTheConfigPath: the rendered unit outlives the
// installing shell's working directory, so a relative --config would send the
// supervised daemon looking for a config that is not there.
