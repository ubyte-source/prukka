...
// TestCLIErrorTranslatesOnlyTheUnreachedDaemon: gRPC mints codes.Unavailable
// both for a transport that never reached the socket and for the daemon's own
// answers, so the connection state — not the message text — has to decide
...
// daemonUnavailableMessage is a live daemon's own Unavailable: the engine
// catalog and the tagged not-ready push both answer with this code from a
// process that is running and reachable.
const daemonUnavailableMessage = "engine catalog unavailable: boom"

// answeringDaemon serves the control API and refuses Stats the way the daemon
// does.
...
// TestControlCommandKeepsTheWordingOfADaemonThatAnsweredUnavailable: the half
// of the translation no pure-function test can reach. control.Dial builds a
// LAZY connection, so before the call the state is always Idle in a one-shot
// CLI process — indistinguishable from a socket with nothing behind it. Only
// reading it after the call separates the two, which is why the state is
// never an argument evaluated next to the call: an answered Unavailable
// rewritten into "no daemon answering" sends the operator to restart a daemon
// that just replied, and buries the reason it gave.
...
// its control endpoint, returning the config path a subcommand needs. The
// directory is a short MkdirTemp, not t.TempDir, because the socket lives in it
// and t.TempDir embeds the full test name — enough to overrun Darwin's
// sockaddr_un limit.
...
// before it dials, so the failure under test is never the token or the config.
...
// runStats drives one control subcommand end to end and returns the error the
// operator would read.
...
// TestControlCommandWithoutADaemonNamesTheStartCommands: a stopped daemon is
// the most common failure the CLI has, and gRPC words it as a nested
// transport dump with no next step anywhere in it.
