...
func newRootCmd() *cobra.Command {
	...
	// Without this, cobra's Print* fall back to stderr and every command
	// RESULT lands there — `prukka doctor > report.txt` yields an empty file.
	// PrintErr* resolve through the separate error writer and stay on stderr.
	cmd.SetOut(os.Stdout)

	// pflag reads the FIRST back-quoted span of a usage string as the flag's
	// value name; every later pair stays literal prose. Spelling the
	// placeholder deliberately beats letting a cross-reference become one.
