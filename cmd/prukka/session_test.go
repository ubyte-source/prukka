...
// daemon's not-ready answer; real failures and success surface immediately.
func TestPushRetryableWaitsOnlyOnNotReady(t *testing.T) {
	t.Parallel()

	notReady := status.Error(codes.Unavailable, "media output not ready")
	if !pushRetryable(notReady) {
		t.Error("not-ready must be retried")
	}
