...
func TestSetupPrintPathIsMachineReadable(t *testing.T) {
	...
	// CI reads the path from stdout, so nothing may land on stderr.
	if errOutput.Len() != 0 {
...
func TestInstallEngineFailsAHungCatalogByDeadline(t *testing.T) {
	...
	// The production bounds are minutes long, so the test injects short ones
...
// holdConnections accepts and never answers, like a wedged mirror.
