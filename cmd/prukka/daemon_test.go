...
func TestReloadConfigReconfiguresOnlyForLiveLaneChanges(t *testing.T) {
	...
	calls := 0
	reconfigure := (&laneReconfigurer{reconfigure: func() { calls++ }}).onChange
...
// TestLaneReconfigurerClassifiesEachTransitionIndependently: every writer
// classifies its OWN ordered snapshot pair, so a concurrent additive pack
// install — whose transition never routes here at all (the engine discards
// it by provenance) — cannot swallow a lane-relevant settings save, and a
// quiet save stays quiet even after the live snapshot has moved on.
func TestLaneReconfigurerClassifiesEachTransitionIndependently(t *testing.T) {
	t.Parallel()
	...
	calls := 0
	lane := &laneReconfigurer{reconfigure: func() { calls++ }}

	// An additive pack install lands first; its transition is discarded by
	// provenance, so the reconfigurer never observes it.
	if _, installErr := holder.Update(func(c *config.Config) {
		c.Providers.Local.MT.AddPair(core.Lang("it"), core.Lang("en"))
	}); installErr != nil {
		t.Fatalf("additive install update: %v", installErr)
	}

	// A quiet settings save classifies against ITS OWN predecessor — the
	// install's growth cannot masquerade as this writer's edit.
	quiet, err := holder.Update(func(*config.Config) {})
	if err != nil {
		t.Fatalf("quiet update: %v", err)
	}
	lane.onChange(quiet)
	if calls != 0 {
		t.Fatalf("reconfigure calls after quiet settings save = %d, want 0", calls)
	}

	// A genuine lane-relevant change reconfigures.
	relevant, err := holder.Update(func(c *config.Config) {
		c.Providers.Local.STT.Model = "models/stt/large.bin"
	})
	if err != nil {
		t.Fatalf("lane-relevant update: %v", err)
	}
	lane.onChange(relevant)
	if calls != 1 {
