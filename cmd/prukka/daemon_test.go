...
func TestRunDaemonReturnsPprofBindFailure(t *testing.T) {
	// t.TempDir embeds the full test name and can push the Unix-domain
	// control socket past Darwin's short sockaddr_un limit.
...
func TestLaneReconfigurerClassifiesEachTransitionIndependently(t *testing.T) {
	...
	// An additive pack install lands first and is discarded by provenance.
	...
	// A quiet settings save classifies against ITS OWN predecessor.
...
func TestDeviceOutputStampWatchesOnlyLabeledDeviceTargets(t *testing.T) {
	...
	// A labeled target defers to the platform fingerprint, which reports
	// unwatchable where the label does not resolve uniquely.
