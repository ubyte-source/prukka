...
func newDaemonRuntime(ctx context.Context, holder *config.Holder, log *slog.Logger) *daemonRuntime {
	...
	engine := managedEngine(holder, laneCfg.onChange, log)
...
func managedEngine(
	...
	return control.NewEngine(installer, client, holder, change, log)
}
...
// clobber — the race that once let a background pack install swallow a
// lane-relevant settings save. Pack installs never route here at all: the
// engine suppresses them by provenance (strictly additive).
type laneReconfigurer struct {
	reconfigure func()
}

// onChange reconfigures live lanes when the transition changed the
// lane-relevant config. A settings save, a pack removal or a SIGHUP reload
// routes here: a removed capability can strand a lane whose languages
// resolved to it, so those lanes must restart.
...
func sessionDefaults(cfg *config.Config) control.SessionDefaults {
	// The configuration is validated on load through the same parsers, so
	// its vocabulary is already the typed one.
	return control.SessionDefaults{
		Langs: slices.Clone(cfg.Defaults.Langs),
		Subs:  session.SubsMode(cfg.Defaults.Subs),
		Bed:   session.BedMode(cfg.Defaults.Bed),
