...
func newDaemonRuntime(ctx context.Context, holder *config.Holder, log *slog.Logger) *daemonRuntime {
	...
	// Device sinks reopen when another application rewrites the device's
	// configuration (OBS switches the nominal rate on attach); a stale
	// output queue would otherwise stay silently wedged.
	audio.SetDeviceConfigStamp(deviceOutputStamp)
	// Labeled device pushes render through the native helper, which binds the
	// output device by NAME instead of an audiotoolbox array index.
	audio.SetDevicePlaybackHelper(func() string {
		return ffmpeg.MicCaptureBinary(speech.BundleRoot(config.StateDir()))
	})
	...
	outs := laneOutputs{vtt: registry, audio: audioReg, hls: hlsStore}
	...
	engine.SetChangeHook(laneChange)
...
func managedEngine(holder *config.Holder, log *slog.Logger) *control.Engine {
	...
	installer := speech.NewInstaller(config.StateDir(), client, func(p speech.Progress) { engine.Progress(p) })
