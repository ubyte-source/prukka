...
func newDaemonRuntime(ctx context.Context, holder *config.Holder, log *slog.Logger) *daemonRuntime {
	...
	engine := managedEngine(holder, log)
	...
	engine.SetChangeHook(laneCfg.onChange)
	engine.SetBaselineHook(laneCfg.syncBaseline)

	// Daemon-lifetime goroutines: SIGHUP reload, session gauge.
	go watchReload(ctx, holder, laneCfg.onChange, log)
...
// the closure hands installer progress to the engine created right after it.
func managedEngine(holder *config.Holder, log *slog.Logger) *control.Engine {
	...
	installer := speech.NewInstaller(paths.StateDir(), client, func(p speech.Progress) { engine.Progress(p) })
	engine = control.NewEngine(installer, client, holder, log)
