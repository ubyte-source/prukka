...
// restartedNote sees the requested daemon state.
func fakeServiceManagers(t *testing.T, running, restartOK bool) {
	...
	t.Setenv("PATH", bin)
	// The darwin status probe falls back to the agent plist under HOME.
...
func TestRestartedNoteFlagsAStaleDaemon(t *testing.T) {
	...
	if !strings.Contains(got, osservice.RestartHint()) {
