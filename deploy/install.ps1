...
try {
    ...
    Write-Host ""
    Write-Host "  Speech lanes use native tools and model packs that prukka setup"
    Write-Host "  downloads (or install them from the dashboard). Native builds are"
    Write-Host "  macOS-only today, so out-of-box speech is not available on Windows yet."
    Write-Host "  To use your own engine, set providers.local.bin, then run: prukka doctor"
    Write-Host ""
