...
func TestInstallRejectsDuplicateMemberArchive(t *testing.T) {
	t.Parallel()
	...
	binary := fmt.Appendf(nil, fakePrukka, "duplicate")
	archive := writeDuplicateMemberArchive(t, binary, readUninstaller(t))
	env := lifecycleEnv(root, filepath.Join(root, "bin"), tools, archive, "0")
	cmd := scriptCommand(env, "install.sh")
	if output, err := cmd.CombinedOutput(); err == nil ||
		!strings.Contains(string(output), "archive must contain exactly one prukka entry") {
		t.Fatalf("duplicate prukka member was not rejected: %v\n%s", err, output)
	}
}

// writeDuplicateMemberArchive builds a release archive carrying the prukka
// member twice — the shape assert_single_regular_member must refuse, because
// sequential extraction would let the last entry decide what lands on disk.
func writeDuplicateMemberArchive(t *testing.T, binary, uninstaller []byte) string {
	t.Helper()

	f, err := os.CreateTemp(t.TempDir(), "release-duplicate-*.tar.gz")
	if err != nil {
	...
	entries := []struct {
		...
		{name: "prukka", data: binary},
		{name: "prukka", data: binary},
		{name: "deploy/uninstall.sh", data: uninstaller},
	}
...
func uninstallAndKeepData(t *testing.T, env []string, root, binDir string) (state, config string) {
	...
	if _, statErr := os.Stat(filepath.Join(binDir, "prukka")); !errors.Is(statErr, fs.ErrNotExist) {
