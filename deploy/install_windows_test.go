...
// TestPowerShellServiceFallbackTreatsAbsentTaskAsRemoved drives the real
// Remove-ServiceFallback against a schtasks stub that, like the real tool on
// an absent task, writes an ERROR line to stderr and exits nonzero on every
// call. Under the file-wide $ErrorActionPreference = "Stop" that stderr must
// not be mistaken for residue: the /Query exit code alone is the verdict.
func TestPowerShellServiceFallbackTreatsAbsentTaskAsRemoved(t *testing.T) {
	t.Parallel()

	root := t.TempDir()
	logPath := filepath.Join(root, "schtasks.log")
	stub := writeSchtasksStub(t, root, "@echo off\r\n"+
		">> \"%PRUKKA_SCHTASKS_LOG%\" echo %*\r\n"+
		"echo ERROR: The system cannot find the file specified. 1>&2\r\n"+
		"exit /b 1\r\n")
	script := uninstallScript(t)
	program := strings.Join([]string{
		`$env:PRUKKA_SCHTASKS_LOG='` + psQuote(logPath) + `'`,
		`. '` + psQuote(script) + `'`,
		`function Get-TaskSchedulerPath { '` + psQuote(stub) + `' }`,
		`Remove-ServiceFallback`,
	}, "; ")
	output, runErr := runPowerShell(t, program)
	if runErr != nil {
		t.Fatalf("absent scheduled task was reported as residue: %v\n%s", runErr, output)
	}

	log, err := os.ReadFile(filepath.Clean(logPath))
	if err != nil {
		t.Fatalf("read schtasks log: %v", err)
	}
	got := strings.ReplaceAll(string(log), "\r\n", "\n")
	want := "/End /TN Prukka\n/Delete /TN Prukka /F\n/Query /TN Prukka\n"
	if got != want {
		t.Fatalf("schtasks calls = %q, want %q", got, want)
	}
}

// TestPowerShellServiceFallbackReportsSurvivingTask covers the opposite
// verdict: when /Query still finds the task after /Delete, the fallback must
// report residue.
func TestPowerShellServiceFallbackReportsSurvivingTask(t *testing.T) {
	t.Parallel()
	...
	program := strings.Join([]string{
		...
		`. '` + psQuote(script) + `'`,
		`function Get-TaskSchedulerPath { '` + psQuote(stub) + `' }`,
		`try { Remove-ServiceFallback; exit 9 } catch { ` +
			`if ($_.Exception.Message -notlike '*Prukka is still registered*') { ` +
			`Write-Error $_; exit 8 }; exit 0 }`,
	}, "; ")
	if output, runErr := runPowerShell(t, program); runErr != nil {
		t.Fatalf("surviving scheduled task was not reported as residue: %v\n%s", runErr, output)
	}
}

// writeSchtasksStub materializes a cmd script that stands in for schtasks.exe.
func writeSchtasksStub(t *testing.T, dir, body string) string {
	t.Helper()

	stub := filepath.Join(dir, "schtasks.cmd")
	// Windows executability is ACL-based, not mode-bit-based: 0o600 runs fine.
	if err := os.WriteFile(stub, []byte(body), 0o600); err != nil {
		t.Fatalf("write schtasks stub: %v", err)
	}
