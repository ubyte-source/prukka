...
# Invoke-NativeStatus runs a native tool with its output discarded and returns
# its exit code. Windows PowerShell turns redirected native stderr into error
# records, and the file-wide $ErrorActionPreference = "Stop" would make the
# first such line terminating before $LASTEXITCODE could ever be read; this
# helper's scope-local "Continue" makes the exit code the only verdict.
function Invoke-NativeStatus([string] $File, [string[]] $Arguments) {
...
function Get-TaskSchedulerPath {
...
function Remove-ServiceFallback {
    $scheduler = Get-TaskSchedulerPath
    # /End is the uninstaller's only daemon shutdown — schtasks /Delete never
    # interrupts a running program. Both calls are best-effort because each
    # legitimately fails on an idle or absent task; the /Query exit code is
    # the sole residue verdict, so removal failure and "already gone" cannot
    # be confused.
    Invoke-NativeBestEffort $scheduler @("/End", "/TN", "Prukka")
    Invoke-NativeBestEffort $scheduler @("/Delete", "/TN", "Prukka", "/F")
    if ((Invoke-NativeStatus $scheduler @("/Query", "/TN", "Prukka")) -eq 0) {
        throw "residue: scheduled task Prukka is still registered"
    }
}
