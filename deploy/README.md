...
there is no container or orchestration layer. `prukka setup` downloads FFmpeg,
the managed native speech tools and the model packs the configuration needs;
an operator-built helper replaces them only through an explicit
`providers.local.bin`. The release publishes checksum-verified tar.gz/zip
archives with build-provenance attestations; native OS packages
(deb/rpm/pkg/msi) do not exist, and the one-command installers
([`install.sh`](install.sh), [`install.ps1`](install.ps1)) are the supported
install path: they wrap the verified daemon download, `prukka setup`, service
install and staged device install. The hosted dashboard, if you run one,
is a static bundle — see [`web/`](web/). To watch the daemon's `/metrics` in
Grafana, import [`grafana/prukka.json`](grafana/prukka.json) against a
...
whose final component is `prukka` or `Prukka` (Windows accepts only the
capitalized form). Custom config files are removed only when
