...
# release checksums, installs ffmpeg and the local speech runtime (prukka
...
# assert_single_regular_member refuses an archive whose named member is
# absent, duplicated or not a regular file — a directory or symlink entry
# with that name would otherwise decide what lands on disk.
assert_single_regular_member() {
  archive=$1
  member=$2

  entries=$(/usr/bin/tar -tzf "$archive" |
    /usr/bin/awk -v name="$member" '$0 == name { n++ } END { print n + 0 }')
  if [ "$entries" -ne 1 ]; then
    echo "archive must contain exactly one $member entry" >&2
    return 1
  fi
  entry_type=$(LC_ALL=C /usr/bin/tar -tvzf "$archive" "$member" |
    /usr/bin/awk 'NR == 1 { print substr($1, 1, 1) }')
  if [ "$entry_type" != - ]; then
    echo "archive $member entry must be a regular file" >&2
    return 1
  fi
}

# extract_member writes one named archive member to dest. The member must
# pass the single-regular-member assertion and be non-empty.
extract_member() {
  archive=$1
  member=$2
  dest=$3

  assert_single_regular_member "$archive" "$member" || return 1
  if ! /usr/bin/tar -xOzf "$archive" "$member" > "$dest"; then
    echo "archive $member entry could not be extracted" >&2
    return 1
  fi
  if [ ! -s "$dest" ]; then
    echo "archive contains an empty $member entry" >&2
    return 1
  fi
}

# discard_privileged_stage removes a root-owned staging directory and clears
# the handle the EXIT trap would otherwise retry.
discard_privileged_stage() {
  privileged rm -rf "$1" >/dev/null 2>&1 || true
  privileged_stage=
}
...
privileged_prukka() {
  ...
  # The staged copy is digest-identical to $archive, so asserting the member
  # shape on the user-readable copy covers the root-owned one too.
  assert_single_regular_member "$archive" prukka || return 1
  ...
  if ! privileged chmod 0700 "$root_stage" ||
    ...
    discard_privileged_stage "$root_stage"
    return 1
  fi
...
main() {
  ...
  extract_member "$tmp/prukka.tar.gz" prukka "$tmp/prukka" || exit 1
  extract_member "$tmp/prukka.tar.gz" deploy/uninstall.sh "$tmp/uninstall.sh" || exit 1
  ...
  echo "==> installing ffmpeg and the local speech runtime"
...
Prukka is ready.
  ...
  Speech lanes use the local runtime "prukka setup" downloads: the
  whisper/mt/piper tools and their model packs. Rerun setup or use
  the dashboard to add or update models:
      "$BIN_DIR/prukka" setup

  Operators can instead point providers.local.bin at their own engine
  binary, then verify:
