...
A bare `state/bin/ffmpeg` executable, outside the content-addressed layout,
has no manifest or executable hash and is not trusted as a managed install.
`Resolve` fails closed and instructs a `prukka setup` run to replace it. Demo
fixtures expose a known external FFmpeg through `PATH`; they never manufacture
a managed install with a symlink.
...
version-3 components. The Evermeet macOS builds are not used because no
immutable build recipe matching those archives is available.
