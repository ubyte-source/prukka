...
The engine assets follow the same shape, for the same reason — they are native
code every install downloads and executes: `Engine build request` is the
...
`internal/speechengine` package, run as hidden `stt`/`mt`/`tts` subcommands that the
single `prukka` binary self-executes. `prukka setup` and the dashboard download
