...
| `internal/engine` | Speech-engine orchestrator, run as hidden `stt`/`mt`/`tts` subcommands the single `prukka` binary self-executes; resolves native tools and models relative to its executable |
