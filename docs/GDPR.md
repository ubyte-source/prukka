...
The standard setup installs FFmpeg, the managed native speech tools and the
configured STT/MT/TTS model packs, each downloaded once, at explicit
operator command, from the project's published release catalog and SHA-256
verified; language packs added or removed later from the dashboard follow the
same explicit, verified download path. No session media is ever part of these
transfers. On platforms without published native tools, speech lanes stay
unavailable until an operator supplies a compatible helper through
`providers.local.bin`. The statements below describe the runtime once the
native tools are installed.

- STT, translation and supported-language TTS are designed to run in a local
  subprocess. Translation is limited to routes the installed models serve — a
  declared directed MT pair, or a bridge through English as two local legs
  backed by declared pairs; same-base
